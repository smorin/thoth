# Product Requirements Document – Doxa Research v1.7
...
- Updated F-20: Combined reports now require --combined flag (not generated by default)
- Updated F-38: Added --quiet mode implementation requirement
- Added F-39: Provider flag short form (-P) requirement
- Updated F-40: Prioritized --config flag implementation
- Added clarification for mock provider API key behavior
- Updated progress display specification to show elapsed time, next check time, and timeout
- Removed PDF/HTML export from gaps (intentionally out of scope)
- Added test IDs throughout PRD for formal test plan alignment
- Added Rich console stdout behavior documentation
...
| F-39 | Support -P as short form for --provider flag | Must | T-CLI-02 |
...
doxa "database optimization techniques" --quiet
...
OPTIONS:
    ...
    --quiet           Minimal output during execution
...
- Next check time: `Next check in Xs` (for async operations)
- Timeout countdown: `timeout in MM:SS`

This provides users with clear visibility into operation timing without overwhelming detail.
...
### Test Suite Status
As of v1.7, the test suite achieves **100% pass rate** with 28/28 tests passing when using the mock provider. This represents complete resolution of all critical issues identified in v1.6.
...
- Progress display shows elapsed time, next check, and timeout
- Provider flag supports -P short form

---

## End of Doxa Research v1.7 PRD
