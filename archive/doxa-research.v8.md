# Product Requirements Document – Doxa Research v0.8
...
| F-16 | All diagnostics to stderr; DEBUG with `--verbose` or `DOXA_DEBUG=1` | Must |
...
| F-21 | Support project-based and date-based output organization | Must |
...
| status ID | Show detailed operation status | `doxa-research status research-20240729-143022-a1b2c3d4` |
| list | List all operations | `doxa-research list` |

### 10.5 Validation Rules
- `--async` cannot be used with `--resume`
- `--structured` requires either `--project` or a default project in config
...
? Default output directory [~/Documents/doxa-research-research]:
? Default project name [research_notes]:
? Output organization style:
  ❯ Project-based (organized by project)
...
class Config:
    ...
    def _default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "paths": {
                "output_dir": str(Path.home() / "Documents" / "doxa-research-research"),
...
[paths]
output_dir = "~/Documents/doxa-research-research"
...
Output Files:
────────────
~/Documents/doxa-research-research/quantum_crypto/research/
