# Product Requirements Document – Doxa Research v1.2
...
Doxa Research v1.2 is a command-line interface (CLI) tool that automates deep technical research using multiple LLM providers. It orchestrates parallel execution of OpenAI's Deep Research API and Perplexity's research models to deliver comprehensive, multi-perspective research reports. Built as a single-file Python script with UV inline dependencies for zero-configuration deployment, Doxa Research provides both synchronous and asynchronous operation modes, intelligent provider selection, and robust handling of long-running operations (5-30+ minutes).
...
- Single configuration file at `~/.doxa-research/config.toml`
...
| Config file | Single TOML file at `~/.doxa-research/config.toml` containing all settings |
...
|----|-------------|----------|
| F-01 | Single config file `~/.doxa-research/config.toml` with all settings | Must |
...
| F-15 | All diagnostics to stderr; DEBUG with `--verbose` or `DOXA_DEBUG=1` | Must |
...
```bash
doxa [COMMAND] [OPTIONS]
doxa MODE QUERY [OPTIONS]
...
doxa deep_research "quantum cryptography" --project quantum_research
# Creates:
...
doxa deep_research "quantum algorithms" --provider openai
# Creates only:
#   ./2024-08-03_143022_deep_research_openai_quantum-algorithms.md

# Read query from file
doxa deep_research --query-file ./research_query.txt

# Read query from stdin (max 1MB)
echo "quantum computing security" | doxa-research deep_research --query-file -

# Mode chaining example
doxa clarification "quantum computing security implications"
# Creates: ./2024-08-03_143022_clarification_openai_quantum-computing.md

doxa exploration --input-file ./2024-08-03_143022_clarification_openai_quantum-computing.md
# Or with auto-input (if configured):
doxa exploration --auto

# Async submission (returns immediately)
doxa deep_research "quantum cryptography" --async
...
|---------|-------------|---------|
| init | Run interactive setup wizard | `doxa-research init` |
| status ID | Show detailed operation status | `doxa-research status research-20240803-143022-a1b2c3d4e5f6g7h8` |
| list | List all operations | `doxa-research list` |
...
### 11.1 Setup Wizard (`doxa-research init`)

```
Welcome to Doxa Research Research Assistant Setup!
...
Configuration file will be created at: ~/.doxa-research/config.toml
...
Configuration saved to ~/.doxa-research/config.toml (version 1.0)

You can now run: doxa-research deep_research "your query"
```
...
class Config:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        self.config_path = config_path or Path(user_config_dir("doxa-research")) / "config.toml"
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        if config_version != CONFIG_VERSION:
            ...
            console.print("[yellow]Some settings may not work as expected. Run 'doxa-research init' to update.[/yellow]")
...
def cli(ctx, mode, query, mode_opt, query_opt, query_file, async_mode, resume_id,
    ...
    Run research with: doxa-research MODE "QUERY"
...
async def run_research(mode: str, query: str, async_mode: bool,
    ...
    if not check_disk_space(output_path):
        raise DoxaError("Insufficient disk space", "Free up at least 100MB", exit_code=8)
    ...
    if async_mode:
        ...
        console.print(f"\nCheck status: [dim]doxa-research status {operation_id}[/dim]")
    else:
...
checkpoint_dir = "~/.doxa-research/checkpoints"  # Checkpoint storage (symlinks resolved)
...
log_file = "~/.doxa-research/doxa-research.log"
max_log_size = "10MB"
```
...
class APIKeyError(DoxaError):
    ...
    def __init__(self, provider: str):
        super().__init__(
            ...
            f"Set {provider.upper()}_API_KEY or run 'doxa-research init'",
            exit_code=2
        )
...
[yellow]Warning:[/yellow] Checkpoint file corrupted: ~/.doxa-research/checkpoints/research-20240803-143022-a1b2c3d4e5f6g7h8.json
...
2. **"Configuration version mismatch"**
   - Run `doxa-research init` to update your configuration
...
5. **"Operation ID not found"**
   ...
   - List active operations with `doxa-research list`
...
- Add `doxa-research list --clean` option to remove old operations
- Configurable retention policies

---

## End of Doxa Research v1.3 PRD

This production-ready specification provides a complete foundation for implementing Doxa Research as a robust research automation tool. All major design decisions have been made, with comprehensive coverage of:
