# Product Requirements Document: Doxa Research v0.5

## Executive Summary

Doxa Research v0.5 is a command-line interface (CLI) tool that automates deep technical research using multiple LLM providers. It orchestrates parallel execution of OpenAI's Deep Research API and Perplexity's research models to deliver comprehensive, multi-perspective research reports. Built with UV script dependencies for zero-configuration deployment, Doxa Research provides both synchronous and asynchronous operation modes, intelligent provider selection, and robust handling of long-running operations (5-30+ minutes).
...
# Asynchronous
doxa research "impact of quantum computing on cryptography" --no-wait
# Returns: research-20240115-143022-a1b2c3d4

# Status checking
doxa status research-20240115-143022-a1b2c3d4
doxa logs research-20240115-143022-a1b2c3d4 --follow
```
...
@click.group()
def cli():
    """Doxa Research - AI-Powered Deep Research Automation"""
    pass
...
def research(query: str, mode: str, wait: bool, output_dir: Optional[str]):
    ...
    if wait:
        ...
        console.print("Check status with: doxa-research status {result.operation_id}")
```
...
if not api_key:
    raise DoxaError(
        ...
        "Set OPENAI_API_KEY environment variable or run 'doxa-research config'"
    )
```
...
This PRD provides a comprehensive specification for implementing Doxa Research v0.5 with all discovered API details, best practices, and implementation patterns integrated into a cohesive product vision.
