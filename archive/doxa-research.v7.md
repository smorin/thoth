# Product Requirements Document – Doxa Research v0.7
...
- Persistent defaults stored in `~/.doxa-research/` configuration files
...
| Defaults file | `~/.doxa-research/defaults.toml`; user-set system-wide defaults |
...
|----|-------------|----------|
| F-01 | Built-in modes hard-coded; merge user file `~/.doxa-research/modes.toml` if exists | Must |
| F-02 | Built-in model-slots hard-coded; merge `~/.doxa-research/models.toml` if exists | Must |
| F-03 | Defaults file `~/.doxa-research/defaults.toml` overrides built-in defaults when present | Must |
...
| F-14 | Config directory (`~/.doxa-research/`) auto-created on first run | Should |
| F-15 | Interactive wizard: plain-text prompts; escape with Ctrl-C or Ctrl-D | Must |
| F-16 | All diagnostics to stderr; DEBUG enabled with `--verbose` or `DOXA_DEBUG=1` | Must |
...
| F-21 | Support `doxa-research status <id>` command to check operation status | Must |
...
doxa deep_research "impact of quantum computing on cryptography"
...
# Resume operation
doxa --resume research-20240729-143022-a1b2c3d4

# Check status
doxa status research-20240729-143022-a1b2c3d4
...
- ~/.doxa-research/defaults.toml
- ~/.doxa-research/models.toml (created with defaults)
- ~/.doxa-research/modes.toml (created with defaults)

Run 'doxa-research deep_research "your query"' to start researching!
```

### 11.2 Interactive Research Wizard (`doxa-research interactive`)

```
Doxa Research Interactive Research Assistant
...
All configuration files are stored in `~/.doxa-research/` directory, which is created automatically on first run.

### 14.1 `~/.doxa-research/models.toml`
...
### 14.3 `~/.doxa-research/defaults.toml`
...
[paths]
output_dir = "~/Documents/doxa-research-research"  # Expands to full path
...
class DoxaOrchestrator:
    ...
    def __init__(self, config_dir: Path = None):
        self.config_dir = config_dir or Path(user_config_dir("doxa-research"))
...
├─────────────────────────────────────────────────────────────────┤
│ Saving to: ~/Documents/doxa-research-research/quantum_crypto_2024/      │
...
class APIKeyError(DoxaError):
    ...
    def __init__(self, provider: str):
        ...
        suggestion = f"Set {provider.upper()}_API_KEY environment variable or run 'doxa-research init'"
...
def display_error(error: DoxaError):
...
try:
    ...
    console.print("\n[dim]Please report this issue at: https://github.com/doxa-research/issues[/dim]")
    sys.exit(127)
```
...
## End of Doxa Research v0.7 PRD

This corrected specification resolves all identified inconsistencies, completes missing implementations, and provides a clear, coherent design for the Doxa Research research automation tool. All conflicts have been resolved, naming has been standardized, and the document now provides complete implementation guidance.
