...
**Goal**: Ship the automation-and-scripting half of v3.0.0. Add `doxa-research completion {bash,zsh,fish}` (with `--install`) backed by dynamic completers in `completion/sources.py`. Add `--json` to every data/action admin command via the B-deferred per-handler `get_*_data() -> dict` extraction pattern, with envelope contract centralized in `json_output.py`. Completion script success stays raw shell output for `eval "$(doxa-research completion zsh)"`; `completion --json` is only for structured errors/install metadata. `help` stays human-only. Closes PRD F-70 and Plan M21-07. Lands as the final commit before release-please opens the v3.0.0 PR.
...
- **Critical invariant** (spec §7.2): the subcommand wrapper is the ONLY place that knows about `--json`. Handlers below never branch on the flag. CI lint rule: `! grep -rnE "as_json" src/doxa_research/commands.py src/doxa_research/config_cmd.py src/doxa_research/modes_cmd.py`. If a future PR adds `as_json=True` plumbing, lint fails.
...
- **`completion <shell> --install`** writes to conventional shell rc location (e.g., `~/.zshrc`, `~/.bashrc`, `~/.config/fish/completions/doxa-research.fish`) with prompt-before-overwrite in tty; refuses with a helpful error in non-tty unless `--force`. Detect existing `_doxa_completion` block; preview + prompt y/n.
...
- [x] [P16-PR3-T01] Create `src/doxa_research/json_output.py` with `emit_json(data)` and `emit_error(code, message, details=None, exit_code=1)`. Stdlib only.

**Phase B — `completion` subcommand**
- [x] [P16-PR3-TS04] `tests/test_completion.py`: `doxa-research completion bash` emits a script containing `_DOXA_COMPLETE=bash_source doxa-research`
- [x] [P16-PR3-TS05] Same for `zsh` and `fish`; `doxa-research completion zsh-bogus --json` exits 2 with `UNSUPPORTED_SHELL`
- [x] [P16-PR3-TS06] `tests/test_completion_install.py`: `doxa-research completion bash --install` writes to `~/.bashrc` (tmp-home fixture); rerun detects existing block and prompts before overwrite
...
- [x] [P16-PR3-T03] Create `src/doxa_research/completion/__init__.py`, `script.py` (init script generation), `sources.py` (completer data functions: `operation_ids`, `mode_names`, `config_keys`, `provider_names`)
- [x] [P16-PR3-T04] Add `src/doxa_research/cli_subcommands/completion.py` with `@click.command("completion")` + string `shell` arg validated in the command body against `{bash,zsh,fish}` + `--install/--force/--json` flags. Do not use a raw `click.Choice`, because invalid-shell errors must be emit-able as `UNSUPPORTED_SHELL` JSON.
...
- [x] [P16-PR3-T14] Add CI check: `! grep -rnE "as_json" src/doxa_research/commands.py src/doxa_research/config_cmd.py src/doxa_research/modes_cmd.py` (handlers must not branch on JSON flag)
...
- [x] [P16-PR3-T16] Update `planning/doxa-research.prd.v24.md:96` ("Added shell completion support") from aspirational to actually-shipped (spec §13 stale-PRD note)
...
- CI lint rule: `! grep -rnE "as_json" src/doxa_research/commands.py src/doxa_research/config_cmd.py src/doxa_research/modes_cmd.py` exits 0
- `doxa-research status NONEXISTENT_ID --json | jq .status` returns `"error"`; `.error.code` returns `"OPERATION_NOT_FOUND"`

### Manual Verification
- `eval "$(doxa-research completion zsh)"` then `doxa-research resume <TAB>` shows live op-ids from `~/.doxa-research/operations/`
- `doxa-research completion bash --install` writes to `~/.bashrc`; rerun prompts before overwrite
- `doxa-research init --json --non-interactive` emits success envelope; `doxa-research init --json` (no flag) emits `JSON_REQUIRES_NONINTERACTIVE` exit 2
- `doxa-research providers list --json | jq '.data[].name'` returns provider names
- `doxa-research config edit --json` emits success envelope after vim closes

### Acceptance criteria for v3.0.0 release (cumulative across PR1+PR2+PR3, per spec §11)
- `doxa-research --help` shows two-section layout + epilog
- Every admin command is a real Click subcommand
- `doxa-research ask` works as canonical scripted form; `doxa-research resume OP_ID` is the only resume form
- `doxa-research --resume OP_ID` and `doxa-research providers -- --list` both exit 2 with migration hints
- `doxa-research completion bash|zsh|fish` ships working init scripts
