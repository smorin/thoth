...
- **Code:** `src/doxa_research/providers/openai.py:166` (`submit`), `:255` (`check_status`), `:355` (`cancel`), `:393` (`stream`), `:452` (`get_result`); `src/doxa_research/run.py` background dispatcher and `_poll_display`.
...
**Out of scope**
- Re-implementing the existing background path (already shipped via P02 / P03 / P18; full surface in `src/doxa_research/providers/openai.py`).
...
| 2 | `submit()` transport retry on transient HTTP errors | tenacity `@retry` retries `openai.APITimeoutError` / `openai.APIConnectionError` with exponential backoff before re-raising | source `src/doxa_research/providers/openai.py:182-188`; runtime evidence covered under live-API workflow |
...
**Coverage summary (post-T01 verification):** 13 of 14 rows `✓` offline; 1 of 14 `✓ live` (nightly / weekly GitHub Actions). Row `#2` (submit-time tenacity retry) is verified by source inspection plus the live-API workflow; offline coverage is intentionally absent because retry timing is a tenacity-library guarantee, not a doxa-research contract.
...
Existing `ResearchProvider` background contract (`src/doxa_research/providers/base.py`):
...
Provider-agnostic runtime (`src/doxa_research/run.py:588` `_run_polling_loop`): owns jitter (±10%), refresh interval, exponential backoff on transient errors (capped at 60 s), `max_transient_errors` counter, checkpoint write on every status transition, timeout via `max_wait`. Providers DO NOT implement polling themselves — they only emit status dicts.

| Concern | OpenAI (shipped) | Perplexity (P27) | Gemini (P28) |
|---|---|---|---|
| Provider file | `src/doxa_research/providers/openai.py` | `src/doxa_research/providers/perplexity.py` (immediate-only stub at present; background path planned in P27) | `src/doxa_research/providers/gemini.py` (planned in P28) |
...
**Rationale:**
1. The shared abstraction already exists. `_run_polling_loop` (`src/doxa_research/run.py:588`) owns the polling loop, jitter, exponential backoff on transient errors, transient-error counter, and checkpoint-on-every-status-transition. `ResearchProvider` base class owns the `submit / check_status / get_result / cancel` contract. `StreamEvent` (from P22's analysis) covers output translation. There is no abstraction to extract that is not already in place.
...
**No new P## reserved.** **No Protocol / ABC sketch needed** (the existing `ResearchProvider` background contract serves that role; `_run_polling_loop` serves the runtime role). **No code changes in `src/doxa_research/`.**
...
P26 closes `[x]` per the project-level close-out criterion (all three parts have closed out, no new code lands in `src/doxa_research/`).
...
- No new code lands in `src/doxa_research/` as part of P26 unless Part 3 outcome (b) requires it (and even then scope is limited to a Protocol / ABC declaration, not provider implementation changes).
