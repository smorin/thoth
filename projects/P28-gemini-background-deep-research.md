...
6. ~~**Single-agent assumption.** Doc §3 confirms `deep-research-pro-preview-12-2025` is the only built-in agent currently.~~ **Resolved 2026-05-04:** upstream docs (https://ai.google.dev/gemini-api/docs/deep-research) now list a two-tier set — `deep-research-preview-04-2026` (speed/efficiency, "ideal to be streamed back to a client UI") and `deep-research-max-preview-04-2026` (max comprehensiveness, "for automated context gathering and synthesis"). The legacy `deep-research-pro-preview-12-2025` no longer appears. **P28 v1 scope:** ship only the speed-efficiency tier (`deep-research-preview-04-2026`) across all 9 modes; defer max-tier opt-in to a successor project. Plan v2 Task 1 (API spike) verifies both IDs against the live API before committing the v1 scope.
