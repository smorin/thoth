# P11 — `doxa-research modes` Discovery Command (v2.11.0)
...
**Out of Scope**
- Editing/adding modes from the CLI (use `doxa-research config set` / `edit` as today)
- Changing how modes actually execute at runtime (other than the `is_background_mode` helper refactor)
- Reworking the existing `doxa-research --help` mode listing beyond a one-line pointer + teaser

### Design Notes
- Single derivation point `is_background_mode(mode_config) -> bool` in `src/doxa_research/config.py`: `bool(mode_config.get("async"))` if set, else `"deep-research" in mode_config.get("model", "")`. Replace ad-hoc check at `src/doxa_research/providers/openai.py:175,182`.
...
- Secret masking + TTY auto-detect parity with `doxa-research config` (reuse `_mask_in_tree` / `_is_secret_key`). No `--no-color` flag — auto-detect instead.
...
- [x] [P11-T00] Implement `is_background_mode` in `src/doxa_research/config.py`; refactor `providers/openai.py:175,182` + `providers/__init__.py:111` to call it; change `BUILTIN_MODES["thinking"]["model"]` to `"o3"`
...
- [x] [P11-T01] Implement `list_all_modes` + `ModeInfo` in `src/doxa_research/modes_cmd.py`
- [x] [P11-TS02] CLI tests for `doxa-research modes`: default table + sort order, `--json` shape with `schema_version`, `--source builtin|user|overridden|all` filter, `--show-secrets` unmasks, default masks api_key inside a mode. Use test-isolation fixture (`isolated_doxa_home` + autouse `COLUMNS=200`) — does NOT read the real `~/.doxa-research/config.toml`
...
- [x] [P11-TS05] Tests for `doxa-research modes --name <mode>` detail view: override diff, `--full` dumps entire `system_prompt`, unknown name returns exit 1
- [x] [P11-T03] Implement `--name` detail view with per-field override diff and `--full` flag
- [x] [P11-T04] Wire `modes` into `src/doxa_research/cli.py` dispatch (parallel to `config`); add `show_modes_help()` in `src/doxa_research/help.py`; replace per-mode epilog loop with names-only teaser + pointer; include JSON schema snippet in help
- [x] [P11-TS03] Test that `doxa-research help modes` prints the new help block and that help epilog still lists mode names
...
- [x] [P11-T06] Added `ignore_unknown_options=True` in root click context so `--json` / `--name` / `--source` flags pass through to subcommands; updated doxa_test EXIT-02 to assert exit 2 via `doxa-research modes bogus_op` (the `doxa-research --invalid-flag` case became "prompt" per new click behavior)
- [x] Regression Test Status — full suite 169/169 pytest + 63/64 doxa_test (1 skipped, 0 failed) green
...
$ doxa modes --json | jq '.[] | select(.kind == "background") | .name'
```
...
- `doxa-research modes --json` validates against documented schema (`schema_version: "1"`)
- API key values inside a `[modes.*]` table are masked by default

### Manual Verification
- `doxa-research modes` prints table sorted by source, kind, provider, model
- `doxa-research modes --name thinking` reflects the `o3` fix and shows `Kind: immediate`
- Running a background mode still routes to OpenAI background responses (sanity check: `doxa-research deep_research "hello" --async`)
- `doxa-research --help` still shows mode names (teaser) and points at `doxa-research modes` for details
