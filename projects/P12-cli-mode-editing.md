# P12 — CLI Mode Editing — `doxa-research modes` mutations (v2.12.0)
...
- **Code:** `src/doxa_research/config_profiles.py:107` (overlay-modes semantics)

**Status:** `[x]` Done.

**Goal**: Let users create, edit, rename, copy, and remove mode definitions from the CLI instead of hand-editing TOML. Adds the mutation half of the `doxa-research modes` surface (P11 shipped read-only `list`), mirroring the `doxa-research config profiles` precedent (P21b) where applicable and diverging deliberately where mode semantics require it (builtins, `--model`-on-create, empty-table pruning). Integrates with P18's canonical `kind` field and P21*'s profile-overlay tier.
...
- `set-default` / `unset-default` for modes — deferred to a follow-up project. Users edit `general.default_mode` via `doxa-research config set` until the follow-up lands.
- Schema migration for `[modes.*]` (no on-disk shape change beyond what P18 already established).

### Design Notes — command surface

Argument shape mirrors `doxa-research config profiles` (P21b) where applicable, with mode-specific deviations:

- `doxa-research modes add NAME --model M [--provider P] [--description D] [--kind immediate|background] [--override]` — required `--model`; default `provider=openai`, default `kind=immediate`. `--override` opts in to creating a same-named user override for a builtin in the selected tier: base `[modes.NAME]` by default, or `[profiles.X.modes.NAME]` with `--profile X`.
- `doxa-research modes set NAME KEY VALUE [--string]` — single-leaf write. `--string` forces string parsing for keys like `sk-...`.
- `doxa-research modes unset NAME KEY` — single-leaf remove. **Never overloaded** — the no-key form is `remove`, not `unset`.
- `doxa-research modes remove NAME` — whole-table delete (parallel to `config profiles remove`). Idempotent.
- `doxa-research modes rename OLD NEW` — user-mode only.
- `doxa-research modes copy SRC DST [--from-profile X] [--override]` — copies effective config (builtin layered with override) into a new user mode. `--from-profile X` reads SRC from `[profiles.X.modes.SRC]` (default reads from base `[modes.SRC]`); composes with `--profile Y` (DST) to enable all four directions: base→base, base→overlay, overlay→base, overlay→overlay. `--override` opts in when DST is a builtin name, writing a same-named override in the selected destination tier.
...
- **File axis** (mutually exclusive):
  - default → `~/.doxa-research/doxa-research.config.toml`
  - `--project` → `./doxa.config.toml` (or `./.doxa-research.config.toml`)
...
- **Tier axis** (composes with file axis):
  - default → `[modes.<NAME>]`
  - `--profile X` → `[profiles.X.modes.<NAME>]` (overlay tier; grounded in `config_profiles.py:107` semantics — overlay modes are hidden from `doxa-research modes` unless `--profile X` / `DOXA_PROFILE=X` is active)

Examples:
- `doxa-research modes add cheap --model gpt-4o-mini` → `~/.doxa-research/doxa-research.config.toml [modes.cheap]`
- `doxa-research modes add cheap --model gpt-4o-mini --profile dev` → `~/.doxa-research/doxa-research.config.toml [profiles.dev.modes.cheap]`
- `doxa-research modes add cheap --model gpt-4o-mini --project --profile ci` → `./doxa.config.toml [profiles.ci.modes.cheap]`

### Design Notes — semantics shared by every mutator

- **JSON envelope**: every mutator accepts `--json` through the Click wrapper and follows the existing Doxa Research JSON contract: `{"status": "ok", "data": {schema_version: "1", op, mode, target: {file, tier}, …op-specific keys}}` or `{"status": "error", "error": ...}`. Default human output is a one-line confirmation (e.g. `Set my_brief.temperature = 0.2 → ~/.doxa-research/doxa-research.config.toml [modes.my_brief]`). Users who want the full effective view run `doxa-research modes list --name NAME` afterward.
...
- **Effective view after writes**: not auto-printed. The write receipt is intentionally minimal; users run `doxa-research modes list --name NAME [--profile X]` for the full ModeInfo. Diverges from the original P12 design (which echoed `_render_detail` after every write) — chosen for parity with `config profiles` and to avoid double-rendering.
- **Shared secret helpers**: `src/doxa_research/_secrets.py` already exists (extracted under P13); P12 simply consumes it. The original P12-T05 / P12-TS05 "extract helpers" tasks no longer apply.
...
#### `doxa-research modes add NAME --model M [--provider P] [--description D] [--kind K] [--override] [--project] [--config PATH] [--profile X] [--string] [--json]`
- [x] [P12-TS01a] Happy path: creates `[modes.NAME]` with `model = M`, default `provider = "openai"`, default `kind = "immediate"`; appears in `doxa-research modes list --source user`
...
- [x] [P12-TS01h] Targeting matrix: `--project` → `./doxa.config.toml`; `--config PATH` → PATH; `--profile X` → `[profiles.X.modes.NAME]`; `--profile + --project` and `--profile + --config PATH` compose; `--project + --config PATH` rejected with `USAGE_ERROR` exit 2
...
- [x] [P12-T01] Implement `doxa-research modes add` — click command in `cli_subcommands/modes.py`, `get_modes_add_data` + `_op_add` in `modes_cmd.py`, `ConfigDocument.ensure_mode(profile=...)` primitive

#### `doxa-research modes set NAME KEY VALUE [--project] [--config PATH] [--profile X] [--string] [--json]`
...
- [x] [P12-TS02d] Setting on a builtin-only name implicitly creates override in the chosen tier (post: `doxa-research modes list --name NAME [--profile X]` → `source=overridden`)
...
- [x] [P12-T02] Implement `doxa-research modes set` — click command, `get_modes_set_data` + `_op_set`, `ConfigDocument.set_mode_value(profile=...)`

#### `doxa-research modes unset NAME KEY [--project] [--config PATH] [--profile X] [--json]`
...
- [x] [P12-TS03c] On overridden mode: pruning fully reverts the override; post `doxa-research modes list --name NAME` → `source=builtin`
...
- [x] [P12-T03] Implement `doxa-research modes unset` — click command, `get_modes_unset_data` + `_op_unset`, `ConfigDocument.unset_mode_value(profile=..., prune_empty=True)`

#### `doxa-research modes remove NAME [--project] [--config PATH] [--profile X] [--json]`
- [x] [P12-TS04a] Drops a user-only mode in the chosen tier; gone from `doxa-research modes list [--profile X]`
...
- [x] [P12-T04] Implement `doxa-research modes remove` — click command, `get_modes_remove_data` + `_op_remove`, `ConfigDocument.remove_mode(profile=...)`

#### `doxa-research modes rename OLD NEW [--project] [--config PATH] [--profile X] [--json]`
...
- [x] [P12-T05] Implement `doxa-research modes rename` — click command, `get_modes_rename_data` + `_op_rename`, `ConfigDocument.rename_mode(profile=...)`

#### `doxa-research modes copy SRC DST [--from-profile X] [--override] [--project] [--config PATH] [--profile Y] [--json]`
...
- [x] [P12-T06] Implement `doxa-research modes copy` — click command, `get_modes_copy_data` + `_op_copy`, `ConfigDocument.copy_mode(from_profile=..., profile=...)`
...
- [x] [P12-TS07d] `doxa-research help modes` epilog and `show_modes_help()` list every new op with examples covering `--project`, `--config PATH`, `--profile`, `--override`, and `--from-profile` — implemented as the modes-group `epilog=` string (no `show_modes_help()` exists; `doxa-research help modes` routes through `doxa-research modes --help`)
- [x] [P12-TS07e] Layering: when both `[modes.X]` and `[profiles.dev.modes.X]` exist, `doxa-research modes list --name X --profile dev` reflects overlay value and `doxa-research modes list --name X` reflects base value. Validates that P12 mutators land in the right tier and P21*'s overlay reader still resolves correctly across the full set of P12-introduced primitives.
- [x] [P12-T07] Help integration — extend the click modes-group `epilog=` (no `show_modes_help()` exists in this codebase; `doxa-research help modes` routes via `doxa-research modes --help`)
...
#### Regression
- [x] [P12-TS08] Full `uv run pytest tests/` + `./doxa_test -r` green; existing P11 read paths (`doxa-research modes list`, `--json`, `--name`, `--source`, `--full`, `--profile`) unchanged
- [x] Regression Test Status

### Deliverable
```bash
$ doxa modes add my_brief --model gpt-4o-mini --description "terse daily brief"
Added mode 'my_brief' (model=gpt-4o-mini, kind=immediate)
# → ~/.doxa-research/doxa-research.config.toml [modes.my_brief]

$ doxa modes set my_brief temperature 0.2
Set my_brief.temperature = 0.2

$ doxa modes add team_review --model o1-preview --kind background --project
Added mode 'team_review' (model=o1-preview, kind=background)
# → ./doxa.config.toml [modes.team_review]

$ doxa modes add cheap_test --model gpt-4o-mini --profile dev
Added mode 'cheap_test' under profile 'dev' (kind=immediate)
# → ~/.doxa-research/doxa-research.config.toml [profiles.dev.modes.cheap_test]

$ doxa modes copy deep_research custom_research
Copied 'deep_research' (overridden) → 'custom_research' (user)

$ doxa modes rename my_brief brief
Renamed 'my_brief' → 'brief'

$ doxa modes unset brief temperature
Unset brief.temperature

$ doxa modes remove brief
Removed mode 'brief'

$ doxa modes set deep_research parallel false --json | jq .
{ "status": "ok",
  "data": { "schema_version": "1", "op": "set", "mode": "deep_research",
    "target": {"file": "~/.doxa-research/doxa-research.config.toml", "tier": "modes"},
...
### Manual Verification
- After `doxa-research modes add my_brief --model gpt-4o-mini`, `doxa-research modes list` shows it with `source=user`
- After `doxa-research modes set deep_research parallel false`, `doxa-research modes list --name deep_research` shows `source=overridden` with `parallel: true → false`
- `doxa-research modes remove deep_research` reverts the override; `doxa-research modes list --name deep_research` shows `source=builtin`
- `--project` flag writes to `./doxa.config.toml`, not `~/.doxa-research/doxa-research.config.toml`
- `--config PATH` writes to PATH
- `--profile dev` writes to `[profiles.dev.modes.<name>]`; the mode is invisible without `--profile dev` / `DOXA_PROFILE=dev`
- `--profile dev --project` writes to `./doxa.config.toml [profiles.dev.modes.<name>]`
