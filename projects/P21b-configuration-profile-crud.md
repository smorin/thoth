...
**Goal**: Add `doxa-research config profiles list/show/current/set-default/unset-default/add/set/unset/remove` so users can manage profiles from the CLI without hand-editing TOML. Depends on P21 (uses `cm.profile_catalog`, `cm.profile_selection`, and the threaded `--profile` plumbing).
...
- Top-level `doxa-research profiles` command group (commands live under `doxa-research config profiles`).
...
- [x] [P21b-T04] Update `README.md`, `manual_testing_instructions.md`, and `src/doxa_research/help.py` with command examples (`doxa-research config profiles ...` invocations) and the `config get general.default_profile` vs `config profiles current` distinction. (Task 3 commit.)
