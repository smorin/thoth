...
- [ ] [P24-T26] Document the recognized-field registry for root `[providers.NAME]` tables. The docs should state that this root namespace intentionally mixes recognized auth/client fields (`api_key`, `timeout`, base URL/client controls) and recognized request-default fields (`temperature`, token-budget keys, provider-native request fields); for each field, record its category, provider key path, precedence, and whether it participates in request normalization. Link the matrix and inconsistency docs to `INC-002`, and include examples showing mode provider namespaces overriding root provider defaults.
