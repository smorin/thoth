...
- [ ] [P24-TS22] Add failing CLI regression tests for `--input-file` alias semantics. Cover both root-level and `thoth ask` inherited forms: `--input-file file.txt` reads the file contents as the prompt exactly like `--prompt-file file.txt`; it is mutually exclusive with positional prompts, `--prompt`, and `--prompt-file`; `--input-file -` reads stdin with the same byte cap and UTF-8 handling; the provider receives the file contents, not the path. Add checkpoint/output assertions showing `operation.prompt` and the metadata `### Prompt` block match `--prompt-file`, and `input_files:` is absent for alias usage.
- [ ] [P24-T25] Implement the alias by normalizing `input_file` into the effective prompt-file path before prompt resolution in `cli.py` and `cli_subcommands/ask.py`, then stop passing alias usage into `run_research(..., input_file=...)`. Update help text so `--input-file` is described as a deprecated/compatibility alias for `--prompt-file`, and leave any true multi-file/context ingestion for a separate explicitly named option or project. Make P24-TS22 pass.
