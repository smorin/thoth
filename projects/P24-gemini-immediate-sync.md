...
- **Code:** `src/doxa_research/providers/openai.py`, `src/doxa_research/providers/perplexity.py`, `src/doxa_research/providers/base.py`, `src/doxa_research/run.py`, `src/doxa_research/providers/__init__.py`, `src/doxa_research/config.py`, `src/doxa_research/errors.py`.
...
- Define module-level `_PROVIDER_NAME_GEMINI = "gemini"` constant; use it in every `_map_gemini_error(...)` DoxaError construction. Suffix-named per the cross-provider convention (`_PROVIDER_NAME_<PROVIDER>`); the Cross-immediate-providers consolidation prerequisite project introduces `_PROVIDER_NAME_OPENAI = "openai"` (new) and renames Perplexity's bare `_PROVIDER_NAME` to `_PROVIDER_NAME_PERPLEXITY`. The full-suffix naming makes the constants unique across modules — useful when grep'ing repo-wide or extracting future cross-provider helpers.
- All `## Sources` rendering MUST use `md_link_title()` and `md_link_url()` from `doxa-research.utils` for HTML/scheme-injection defense. Mirrors `perplexity.py:463`'s `f"- [{md_link_title(title)}]({md_link_url(url)})"` pattern. Never inline raw `f"- [{title}]({url})"` interpolation. (OpenAI currently uses raw interpolation at `openai.py:614-622`; that's a known bug tracked by the Cross-immediate-providers consolidation prerequisite project.)
- Auth-invalid `DoxaError` (`code=401, status=UNAUTHENTICATED` with `_INVALID_KEY_PHRASES_GEMINI` matching the message) MUST set `exit_code=2`, mirroring `perplexity.py`'s invalid-key path. Auth-missing maps to bare `APIKeyError("gemini")` (default exit code).
...
- `--api-key-gemini` + `GEMINI_API_KEY` only. No Vertex/ADC. Reason: keeps the auth surface uniform across OpenAI/Perplexity/Gemini for `doxa-research ask`; Vertex is a different deployment model and would warrant its own project.
...
Distilled from a `factor-dedup` pass over `src/doxa_research/providers/openai.py` (immediate path) and `src/doxa_research/providers/perplexity.py` (full file). Used at implementation time to verify nothing is forgotten and at review time to confirm symmetry. Full classification + drift bookkeeping lives in [planning/p24-immediate-providers-consolidation.v1.md](../planning/p24-immediate-providers-consolidation.v1.md).
...
- [x] [P24-T05] Implement explicit `stream = false` handling and Gemini `submit()` / `check_status()` / `get_result()` one-shot behavior. Use `md_link_title()` / `md_link_url()` from `doxa-research.utils` for the `## Sources` block. Make P24-TS05 pass.
...
- [x] [P24-T10] Gemini implementation closeout (after T01–T09 + the cross-provider extended tasks below). Run targeted pytest for new/changed tests, `just check`, `./doxa_test -r`, `just test-lint`, `just test-typecheck`, and the Gemini live-api command when `GEMINI_API_KEY` is available. Flip P24 to `[x]` only after the full gate passes — including the cross-provider extended tasks.
...
- [x] [P24-T12] Backport `md_link_title()` and `md_link_url()` from `doxa-research.utils` to `openai.py:614-622`'s `## Sources` rendering. Replace the raw `f"- [{title}]({url})"` interpolation. Make P24-TS11 pass.
...
- [x] [P24-TS13] Add failing test asserting OpenAI's `_map_openai_error` invalid-key path raises `DoxaError` with `exit_code=2`, matching Perplexity's `perplexity.py` behavior.
- [x] [P24-T14] Update `openai.py:73-80` to set `exit_code=2` on the invalid-key `DoxaError` construction. Make P24-TS13 pass.
...
**`[providers.defaults]` and `[providers.X]` defaults investigation:**

- [-] [P24-TS16] Originally punted to a focused follow-up project. The successor work activated the root-provider default tests in `tests/test_provider_config.py`, added shared normalizer coverage in `tests/test_provider_parameter_normalization.py`, and aligned the old investigation with the matrix/backlog docs. See `planning/p24-providers-root-namespace-investigation.v1.md`.
- [-] [P24-T17] Originally punted after the investigation report at `planning/p24-providers-root-namespace-investigation.v1.md`. The successor implementation now routes `[providers.defaults]`, `[providers.NAME]`, `[profiles.NAME.providers.defaults]`, and `[profiles.NAME.providers.PROVIDER]` through `src/doxa_research/providers/parameter_config.py`, suppresses the misleading OpenAI flat-mode deprecation warning for root provider defaults, and normalizes recognized fields for OpenAI, Perplexity, and Gemini.
- [x] [P24-T26] Document the matrix `Recognized Field Registry` for root `[providers.defaults]`, `[providers.NAME]`, `[profiles.NAME.providers.defaults]`, and `[profiles.NAME.providers.PROVIDER]` tables. The docs state that all-provider defaults may contain only recognized shared client controls and common request-default fields, while per-provider defaults intentionally mix recognized auth/client fields (`api_key`, `timeout`, base URL/client controls), common request-default fields (`temperature`, token-budget keys), and recognized provider-native request fields; for each field, the matrix records its category, provider key path, precedence, and whether it participates in request normalization. The matrix and inconsistency docs link this contract to `INC-002` and `DEC-007`, and include examples showing mode provider namespaces overriding provider defaults.
...
- [x] [P24-TS17] Add a failing test asserting that, given `[modes.openai_reasoning].openai.reasoning_summary = "auto"`, the streaming request to `client.responses.stream(...)` includes `reasoning={"summary": "auto"}` in its kwargs. Assert symmetric for a `web_search` opt-in flag if added.
- [x] [P24-T18] Implement the namespace passthrough: extend `_resolve_provider_config_value` consumers in `OpenAIProvider.stream()` to read `reasoning_summary` (e.g. `"auto"`) and a `web_search` flag from `[modes.X.openai]`, translating them into `request_params["reasoning"] = {"summary": ...}` and `request_params["tools"] = [...]` respectively. Define a built-in `openai_reasoning` mode (analog to `gemini_reasoning`) that sets `reasoning_summary = "auto"` by default. Add a `tests/extended/test_openai_real_workflows.py::test_openai_reasoning_emits_reasoning_section` live-api case asserting reasoning events appear when `--mode openai_reasoning` is used. Make P24-TS17 pass. See `planning/p24-openai-stream-audit.v1.md` and the inline caveat at `openai.py:501-510`.

**Implementation status.** Done in the follow-up implementation pass. `openai_reasoning` now sets `[modes.openai_reasoning.openai].reasoning_summary = "auto"` and `web_search = true`; custom OpenAI modes can set `[modes.X.openai].web_search = false` to opt out.

##### 3. Perplexity flat-key reads not migrated to `[modes.X.perplexity]` namespace

**What it is.** OpenAI received the namespace migration in Task 3.1 (`src/doxa_research/providers/openai.py:195` `_resolve_provider_config_value` reads `self.config["openai"][key]` first, falls back to flat with `DeprecationWarning`). Perplexity has *partial* namespacing: bag-of-options keys (`web_search_options`, `stream_mode`, `search_recency_filter`) already use `[modes.X.perplexity]`, but **SDK-direct kwargs still read flat** at `src/doxa_research/providers/perplexity.py:493-497`:
...
**What to achieve.** Cross-provider symmetry. After this task, `[modes.X.perplexity].temperature` flows through the same way `[modes.X.openai].temperature` does. Clarified follow-up decision: no `DeprecationWarning` bridge; namespaced Perplexity direct SDK keys are the only supported shape now, and flat direct SDK keys are ignored.

- [x] [P24-TS18] Add failing tests covering: `[modes.X.perplexity].temperature` reaches the request kwargs; flat direct SDK keys are ignored; namespace value wins over flat when both are present; direct SDK keys are excluded from `extra_body`.
- [x] [P24-T19] Refactor the `_DIRECT_SDK_KEYS_PERPLEXITY` loop at `perplexity.py:493-497` to read only from the `[modes.X.perplexity]` namespace. Update existing test fixtures using flat Perplexity direct SDK keys to use the namespace. Make P24-TS18 pass.
...
- `perplexity.py:175-181`: `openai.NotFoundError` → `ProviderError("perplexity", "Model X not found... doxa-research providers --models...")` with the CLI hint.
...
- [x] [P24-TS19] Add failing tests against `_map_perplexity_error_async` that mirror the sync mapper's NotFoundError + unsupported-parameter assertions: a 404 response surfaces the model name + `doxa-research providers models --provider perplexity` hint; a 422 response with `"unsupported parameter 'X'"` in the body extracts X via `_extract_unsupported_param`.
- [x] [P24-T20] Backport both branches to `_map_perplexity_error_async` (`perplexity.py:295-340`). For 404, produce `ProviderError("perplexity", "Model X not found...")` with the same CLI hint as the sync mapper. For the existing 422 path (already at `perplexity.py:309`), additionally try `_extract_unsupported_param(body_text)` and surface the offending parameter name when matched. Make P24-TS19 pass.
...
**What to achieve.** DRY. After Tasks 2.2 (`_invalid_key_doxaerror`) and 2.4 (`_extract_unsupported_param`), `src/doxa_research/providers/_helpers.py` is the established home for cross-provider helpers. The third + duplicate of the same pattern is now load-bearing evidence for `factor-dedup` consolidation.

- [x] [P24-TS20] Run the lightweight factor-dedup comparison over the three sites. Pin the unified contract via tests in `tests/test_provider_helpers.py`: `render_sources_block(citations: Iterable[Citation]) -> str` produces correct deduped markdown; `debug_print_empty_response(response, provider_label) -> None` emits the truncated ladder to stderr.
- [x] [P24-T21] Extract `render_sources_block` and `debug_print_empty_response` to `src/doxa_research/providers/_helpers.py`. Provider code now extracts provider-specific `Citation` objects locally and calls the shared helpers for rendering/debug output. Provider-specific code keeps only the *extraction* logic (which genuinely differs because chunk vocabularies differ: OpenAI annotation, Perplexity search_result, Gemini grounding_chunk.web). Make P24-TS20 pass.

**Implementation status.** Done in the follow-up implementation pass. The lightweight comparison kept provider-specific extraction local and moved the common Markdown source rendering plus verbose empty-response debug ladder into `src/doxa_research/providers/_helpers.py`.

##### 6. OpenAI `_PROVIDER_NAME_OPENAI` unused at non-error sites (cosmetic cleanup)

**What it is.** Task 1.2 introduced `_PROVIDER_NAME_OPENAI = "openai"` at `src/doxa_research/providers/openai.py:49` and replaced 12 inline `"openai"` literals inside `_map_openai_error()` with the constant. But 4 inline literals remain *outside* the error mapper (verified by `grep -nE '"openai"' src/doxa_research/providers/openai.py | grep -v _PROVIDER_NAME_OPENAI`):
...
- [x] [P24-T22] Replace the 4 inline `"openai"` literals listed above with `_PROVIDER_NAME_OPENAI`. No new tests required — existing tests at `tests/test_openai_errors.py::test_openai_constants_use_suffix_naming` already pin the constant's value, so any reference-site swap that breaks string-equality is caught. Pure mechanical change, no behavior delta. Effort: ~5 minutes. (No TS pair — refactor is one-liner per site, covered by existing literal-equality tests.)

##### 7. OpenAI URL-citation type-string filter is presence-based, not type-based (robustness)

**What it is.** Task 6.1's stream-event handler for `response.output_text.annotation.added` (`src/doxa_research/providers/openai.py:551-572`) filters annotations to URL-citations by checking *whether the annotation has a `url` attribute*, not by checking its `type` discriminator:
...
- [x] [P24-TS21] Add failing tests against both `OpenAIProvider.stream()` and `OpenAIProvider.get_result()` that simulate a non-URL annotation variant with `type="file_citation"` AND a `url` field; assert the stream/get_result paths skip it (no `StreamEvent("citation", ...)` emitted; no `## Sources` line rendered), and that legacy missing-type URL annotations still render with a warning.
- [x] [P24-T23] Tighten both filters to use the explicit `type` discriminator. Missing `type` with `url` remains backward-compatible but logs a warning; known non-URL types with `url` are skipped and logged. Apply at `openai.py:551-572` (streaming) AND `openai.py:602-610` (get_result) symmetrically. Make P24-TS21 pass.

##### 8. `_RESEARCH_OPTIONS` count test is a strict pin (documentation note)

**What it is.** `tests/test_p16_pr2_options_decorator.py:11-19` asserts the *exact count* of options in the shared `_RESEARCH_OPTIONS` decorator stack at `src/doxa_research/cli_subcommands/_options.py`. P24 Task 5.1 bumped it 26 → 27 because `--api-key-gemini` was added.
...
- [x] [P24-T24] Add a one-paragraph `CONTRIBUTING.md`-style note (or a docstring on the test itself) explaining the strict-pin convention for `_RESEARCH_OPTIONS`. Cross-reference from `src/doxa_research/cli_subcommands/_options.py`'s module docstring so contributors find it before they trip over the test failure. No code change required; documentation only. (No TS pair — documentation work, no behavior to test.)

##### 9. `--input-file` should be an alias for `--prompt-file`

**What it is.** The CLI currently has two file-looking prompt inputs with different behavior. `--prompt-file` is read through `_read_prompt_input(...)`, becomes the effective prompt string, is sent to the provider, and is stored in checkpoints/output metadata as `operation.prompt`. `--input-file` does not read the file into the provider payload. It is passed to `run_research(...)`, stored as `OperationStatus.input_files`, persisted in checkpoints, and rendered as an `input_files:` metadata list in output files. That creates a misleading state: the file path appears in metadata/checkpoints/output, but the provider only receives the separate positional/`--prompt`/`--prompt-file` prompt text.

**What to achieve.** Treat `--input-file PATH` as a backward-compatible alias for `--prompt-file PATH`. Both flags should use the same prompt-reading path, byte cap, UTF-8 validation, stdin `-` behavior, and mutual-exclusion rules against positional prompt text and `--prompt`. The provider payload, checkpoint `prompt`, output metadata `prompt`, and `### Prompt` section should be identical for `--input-file PATH` and `--prompt-file PATH`. Do not populate `OperationStatus.input_files` merely because `--input-file` was used as the prompt source. If the project still wants to preserve prompt-source provenance, add an explicit prompt-source metadata field in a separate decision rather than overloading `input_files`.

- [x] [P24-TS22] Add failing CLI regression tests for `--input-file` alias semantics. Cover both root-level and `doxa-research ask` inherited forms: `--input-file file.txt` reads the file contents as the prompt exactly like `--prompt-file file.txt`; it is mutually exclusive with positional prompts, `--prompt`, and `--prompt-file`; `--input-file -` reads stdin with the same byte cap and UTF-8 handling; the provider receives the file contents, not the path. Add checkpoint/output assertions showing `operation.prompt` and the metadata `### Prompt` block match `--prompt-file`, and `input_files:` is absent for alias usage.
- [x] [P24-T25] Implement the alias by normalizing `input_file` into the effective prompt-file path before prompt resolution in `cli.py` and `cli_subcommands/ask.py`, then stop passing alias usage into `run_research(..., input_file=...)`. Update help text so `--input-file` is described as a deprecated/compatibility alias for `--prompt-file`, and leave any true multi-file/context ingestion for a separate explicitly named option or project. Make P24-TS22 pass.
...
- `doxa-research ask "What's new in CRISPR?" --mode gemini_quick` returns a grounded answer with a `## Sources` section using `gemini-2.5-flash-lite`.
- `doxa-research ask "What's new in CRISPR?" --provider gemini --model gemini-2.5-pro` returns a grounded answer with a `## Sources` section.
- `doxa-research ask "hello" --provider gemini --model future-model-id` passes the model string to Gemini without local compatibility validation; any invalid model error is surfaced from the provider/API layer.
- `doxa-research ask --mode gemini_reasoning ...` exposes thought-summary content separately from answer text in a `## Reasoning` block (no `<think>` tags appear in user-visible output).
- `doxa-research ask --provider gemini --model deep-research-pro-preview-12-2025 ...` with immediate kind raises `ModeKindMismatchError` before any HTTP call, with the existing generic suggestion.
