...
- **`temperature`/`top_p` exposure**: current Perplexity schema accepts them but help-center says best-effort for grounded answers. Surface in mode config (mirrors OpenAI immediate)? Yes — but document caveat in mode-config docstrings.
