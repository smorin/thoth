...
**Goal**: Standardize Doxa Research's config filename to `doxa-research.config.toml` so the filename alone uniquely identifies a Doxa Research config file. Three canonical locations: one user-tier (`$XDG_CONFIG_HOME/doxa-research/doxa-research.config.toml`) and two project-tier (`./doxa.config.toml` or `./.doxa-research.config.toml`, mutually exclusive). Clean break — legacy filenames (`config.toml` in user dir, `doxa-research.toml`, `.doxa-research/config.toml`) are no longer read; their presence is detected only to enrich "config not found" error messages with rename guidance.
...
**Scope**
- Canonical filename is `doxa-research.config.toml`. Three accepted locations:
  - User: `$XDG_CONFIG_HOME/doxa-research/doxa-research.config.toml`
  - Project root, visible: `./doxa.config.toml`
  - Project root, dotfile: `./.doxa-research.config.toml`
...
- Legacy filenames (`config.toml` in user XDG dir, `./doxa.toml`, `./.doxa-research/config.toml`) are **not loaded**. The `.doxa-research/` directory form is retired for config purposes.
...
- `doxa-research init` writes `./doxa.config.toml` by default. A flag (`--hidden` or similar — final spelling decided in plan) writes `./.doxa-research.config.toml` instead. `doxa-research init --user` writes `$XDG_CONFIG_HOME/doxa-research/doxa-research.config.toml` without pre-loading project config, so project-tier ambiguity does not block user-tier repair. `--user` and the dotfile flag are mutually exclusive. `--force` semantics unchanged.
...
**Out of Scope**
- A `doxa-research config migrate` CLI command that renames files in place.
...
- A `DOXA_CONFIG_FILENAME` env var to override the filename.
- Project marker semantics — presence of either canonical project path still means "this is a Doxa Research project."
- Picking precedence between `./doxa.config.toml` and `./.doxa-research.config.toml`. There is none; both present → error.
- A directory-form project config (e.g. `./.doxa-research/doxa-research.config.toml`).
...
- [x] [P21c-TS02] `tests/test_config_filename.py`: user-tier loads `$XDG_CONFIG_HOME/doxa-research/doxa-research.config.toml` when present; raises `ConfigNotFoundError` when only the legacy `config.toml` exists, and the error message names the legacy file with the rename target. (Tests A1–A5.)
- [x] [P21c-TS03] Project tier: `./doxa.config.toml` only → loads. `./.doxa-research.config.toml` only → loads. Both present → `ConfigAmbiguousError`. Neither present → returns empty/no-project-config. (Tests B1–B8.)
...
- [x] [P21c-TS05] `doxa-research init` writes `./doxa.config.toml` by default. `--hidden` writes `./.doxa-research.config.toml`. `--user` writes the user-tier canonical path without loading project config. `--user` and `--hidden` are mutually exclusive. None overwrite without `--force`; JSON init emits envelopes for both refusal and `--force` overwrite paths. (Tests D1–D14.)
- [x] [P21c-T02] Update `src/doxa_research/paths.py`: `user_config_file()` returns the canonical user path. (Commit `ac84b00`.)
- [x] [P21c-T03] Update `src/doxa_research/config.py`: canonical `project_config_paths`; `ConfigAmbiguousError` if both exist; user-tier load skips legacy; `detect_legacy_paths()` helper. (Commit `0b5b5b1`.)
- [x] [P21c-T04] Update `doxa-research init`: add `--user`, `--hidden`, `--force`; enforce mutual exclusion. (Commit `6ca8ac0`.)
- [x] [P21c-T05] Add `ConfigNotFoundError` and `ConfigAmbiguousError` to `src/doxa_research/errors.py`. (Commit `ac84b00`.)
