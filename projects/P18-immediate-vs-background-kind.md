# P18 — Immediate vs Background — Explicit `kind`, Runtime Mismatch, Path Split, Streaming, Cancel (v3.1.0)
...
### Design Notes
- **`KNOWN_MODELS` derivation** (`models.py` or `config.py`): built once at import time from `BUILTIN_MODES`. Same `(provider, model)` pair appearing under two different `kind` values across builtins → `DoxaError` at module import (we shipped a broken config). User-only models are intentionally excluded from `KNOWN_MODELS` — user opted in, user owns kind correctness.
...
- [x] [P18-TS02] `tests/test_known_models_registry.py`: `derive_known_models()` returns one entry per unique `(provider, model)` across builtins; cross-mode kind conflicts raise `DoxaError` at module import
...
- [x] [P18-T03b] **(Reeval 2026-04-27)** Update `modes_cmd.py:_derive_kind` (`modes_cmd.py:46-50`) to read `cfg["kind"]` first; fall back to substring heuristic only with a warning. Pre-existing CLI surface (`doxa-research modes` table "Kind" column) keeps working unchanged.
...
- [x] [P18-T04] Add `ModeKindMismatchError` class to `errors.py` (subclass of `DoxaError`)
...
- [x] [P18-TS08] `tests/test_immediate_path.py`: an immediate-mode run produces no `Progress` rendering, no spinner, no operation-ID echo (unless `--project` or `--out FILE` set), no `doxa-research resume` hint on failure
...
- [x] [P18-T09] Audit `run.py:629,654,691,692` and `run.py:199,311,313` and `signals.py:93,99` and `commands.py:227,238` — gate every `doxa-research resume {id}` / `doxa-research status {id}` / `Operation ID:` emission on `mode_kind(mode_config) == "background"` (or persistence flag set)
- [x] [P18-T09b] **(Reeval 2026-04-27)** Migrate `cli.py:284` (`_doxa_config.is_background_model(model_name)` call) to `mode_kind(mode_config) == "background"` once a mode_cfg is in scope. If `mode_cfg` not available at that call site, leave as `is_background_model(model_name)` (model-level helper) and document.
...
**Phase D — Mode rename + `doxa-research modes --kind` filter**
...
- [x] [P18-TS11b] **(Reeval 2026-04-27)** `tests/test_modes_kind_filter.py`: `doxa-research modes --kind immediate` shows only immediate modes; `doxa-research modes --kind background` shows only background; invalid value rejected with `BadParameter`; tab-completion uses `completion/sources.py:79 mode_kind` (returns `["immediate","background"]`)
...
- [x] [P18-T15] Add `MultiSink` class (in new `src/doxa_research/sinks.py`) — fans `write(chunk)` to a list of `IO[str]` handles, lazy file open, ordered close in `finally`
- [x] [P18-T16] **(Reeval 2026-04-27)** Add `--out PATH` (repeatable, accepts `-`, comma-list also accepted) and `--append` flags to **`cli_subcommands/_options.py:_RESEARCH_OPTIONS`** so they are inherited by both top-level CLI and `doxa-research ask` (the existing `_research_options` decorator stack). Add corresponding entries to `cli_subcommands/_option_policy.py` for inheritance + validation. Wire to `MultiSink` inside `_execute_immediate` via `_run_research_default`.
...
- [x] [P18-TS18] `tests/test_provider_cancel.py`: providers that don't implement cancel raise `NotImplementedError`; the `doxa-research cancel` CLI catches it and reports "upstream cancel not supported, local checkpoint marked cancelled"
- [x] [P18-TS19] `tests/test_cancel_subcommand.py`: `doxa-research cancel <op-id>` updates the checkpoint to `cancelled`, calls `provider.cancel()` for each non-completed provider, exits 0
- [x] [P18-TS20] `tests/test_cancel_subcommand.py`: `doxa-research cancel MISSING_ID` exits 6 (matching `doxa-research resume` missing-op behavior)
...
- [x] [P18-T25] Add `src/doxa_research/cli_subcommands/cancel.py` — `@click.command("cancel")`, `OP_ID` required positional, `--json` flag, delegates to a new `cancel_operation(op_id, ctx)` in `commands.py`. Mirror the `cli_subcommands/resume.py` pattern (already shipped).
- [x] [P18-T25b] **(Reeval 2026-04-27)** Register `cancel` in `cli.py` via `cli.add_command(cancel)` and add to "Run research" help section in `DoxaGroup.format_commands`. Add `shell_complete=operation_ids` to the `OP_ID` positional (using the existing completer from `completion/sources.py:26`).
...
- [x] [P18-T30] **(Reeval 2026-04-27)** Add a NEW `[tool.pytest.ini_options]` section to `pyproject.toml` (none present today): register `extended` marker; add `addopts = "-m 'not extended'"` to default invocation. Verify pre-commit `./doxa_test` integration suite is unaffected (it uses its own runner, not pytest markers).
- [x] [P18-T31] Add `just test-extended` recipe → `uv run pytest -m extended -v`
- [-] [P18-T32] Add `--extended` flag to `doxa_test` runner (parses to a category column); wire to category filter — *superseded by `just test-extended` (P18-T31) + `.github/workflows/extended.yml` (P18-T33), which already provide pytest-level gating; runner-flag is duplicative*
...
- [x] [P18-T34] Update `README.md` with `--out` flag examples; document the `kind` field for user-defined modes; document `doxa-research cancel`; document `doxa-research modes --kind <immediate|background>` filter
...
- `./doxa_test -r --skip-interactive -q` — full suite green
...
- **(Reeval 2026-04-27)** `grep -rn "is_background_mode\|is_background_model" src/doxa_research/` returns ≤ the documented set in `planning/p18-call-site-audit.md` — no new resolution-path callers introduced

### Manual Verification
- `doxa-research deep_research "topic"` (or PR2's `doxa-research ask "topic" --mode deep_research`) behaves identically to today (background regression)
- `doxa-research thinking "what is X"` (or PR2's `doxa-research ask "what is X" --mode thinking`) streams to stdout in seconds, prints no operation ID, prints no `doxa-research resume` hint
- `doxa-research ask "..." --mode thinking --out answer.md` writes to `answer.md` only; rerun with `--append` appends; `--out -,answer.md` tees to both
- `doxa-research ask "..." --mode mini_research` prints a deprecation warning pointing at `quick_research`, then runs as background
- Edit user TOML, add `[modes.foo]` with provider/model but no `kind` → next doxa-research run prints a one-time warning at config load, then proceeds (warn-only in v3.1.0; v4.0.0 will error)
- **(Reeval 2026-04-27)** `doxa-research modes --kind immediate` filters the table to just immediate modes; `doxa-research modes --kind background` to just background; tab-completing the value offers `immediate` / `background`
- `doxa-research ask "..." --mode quick_research --model o3` (manual override forcing immediate kind on a background-only model) → fails fast with `ModeKindMismatchError` carrying `[modes.quick_research]` config-edit suggestion; no API call made
- `doxa-research cancel <op-id>` for a running background op → checkpoint updated to `cancelled`, OpenAI `responses.cancel` called, exit 0
- `doxa-research cancel <op-id>` for a completed op → exits with helpful "operation already completed" message
