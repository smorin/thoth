...
- **Code (current sources of "default truth"):**
  - `src/doxa_research/config.py:221` — `ConfigSchema.get_defaults()` (runtime defaults)
  - `src/doxa_research/commands.py:70` — `_build_starter_profiles()` (init starter profiles)
  - `src/doxa_research/commands.py:111` — `_build_starter_document()` (init full document)
...
**Goal**: Drive every default config value — `doxa-research init`'s starter document, `ConfigSchema.get_defaults()`, and the per-provider configuration surface — from a single typed source built on Pydantic v2. Eliminate the duplication between runtime defaults and the init template, and gain typechecker + diagnostics coverage for every key the project ships.

**Motivation (P21 follow-up)**: P21's tomlkit refactor made the init writer structurally sound but didn't unify the two sources of "default config truth": `ConfigSchema.get_defaults()` (used by `ConfigManager.load_all_layers`) and `_build_starter_document()` (used by `doxa-research init`). The init writer is intentionally a *subset* of runtime defaults (it omits `max_transient_errors`, `prompt_max_bytes`, `cancel_upstream_on_interrupt`, and the entire `clarification` section), but the subset is hand-curated rather than declared, so a typo like `prompy_prefix` in the init template still slips past lint/typecheck today. Schema-driven generation closes that gap and prepares the provider config surface for P23/P24/P28 fields that the schema will need to model.
...
|---|---|---|
| 1. **Schema** | Pydantic v2 models — types, defaults, per-field metadata, validation rules. | `src/doxa_research/config_schema.py` (new) |
| 2. **Seed data** | Example data with no validation rules of its own — the 6 starter profiles (`daily`, `quick`, `openai_deep`, `all_deep`, `interactive`, `deep_research`); the schema validates their *shape*, not their *values*. | `src/doxa_research/_starter_data.py` (new) — content frozen verbatim from today's `_build_starter_profiles()` |
| 3. **Writer** | tomlkit emitter; reads schema metadata to attach inline comments and pick the `in_starter` field set; reads writer-owned table for structural prose (e.g. `# Configuration profiles (P21)…`). | `src/doxa_research/commands.py` (rewritten thin wrapper around the schema) |
...
3. **Schema topology**: separate runtime defaults from overlay/user-file shape —
   - `DoxaConfig` — runtime default config only, all required-with-defaults. `ConfigSchema.get_defaults()` is derived from this model and must stay byte-identical to today's default dict.
   - `ConfigOverlay` — every runtime-default field optional; **auto-derived** from `DoxaConfig` via a `make_partial(Model)` helper, then extended with valid user-only overlay fields that are intentionally not emitted by `get_defaults()` (`general.default_profile`, `general.prompt_prefix`, mode-level prompt/config fields, etc.).
   ...
   - `PartialDoxaConfig` remains available as the mechanically-derived runtime partial used by CLI/profile overlay internals; a regression test (`TS03`) asserts field-for-field alignment with `DoxaConfig` so manual drift is impossible.
   - `UserConfigFile` — what `doxa-research.config.toml` actually looks like on disk: `ConfigOverlay` fields plus `profiles: dict[str, ProfileConfig]` plus an optional `experimental: dict[str, Any]` carve-out.
...
5. **Round-trip semantics for TS04**: hybrid —
   ...
   - **L3 (strict schema validation)**: parsed root tables validate through `DoxaConfig` projection and the full parsed document validates through `UserConfigFile` with zero warnings in strict mode.
   - **L1 (substring assertions)**: rendered TOML contains `# Doxa Research Configuration File`, `[profiles]`, and `[profiles.daily]` — catches dropped-comment regressions without a brittle golden file.
...
9. **Public API stability**:
   ...
   - **New** `ConfigSchema.model() -> type[DoxaConfig]` — escape hatch for typed callers; low priority.
...
- Touching `BUILTIN_MODES` (mode-specific data, not config defaults — see `src/doxa_research/config.py:53`).
...
1. **[Important]** Profile schema must accept P21-valid fields that are not runtime defaults: `general.default_profile`, `general.prompt_prefix`, profile-root `prompt_prefix`, and nested `profiles.<name>.modes.<mode>` prompt/config fields. Recommended action: add `ConfigOverlay`, `ProfileConfig`, and `ModeConfig` instead of using a raw `PartialDoxaConfig` for every user/profile shape.
...
- [x] [P33-TS00] Dependency metadata test: assert `pyproject.toml` declares `pydantic>=2.12.5,<3` directly (not only through OpenAI's transitive dependency), and `uv.lock` still resolves Pydantic 2.x.
- [x] [P33-TS01] Schema construction smoke test: instantiating `DoxaConfig()` with no overrides succeeds and every default value type-checks. Asserts `ConfigSchema.get_defaults()` equals `_ROOT_SCHEMA.model_dump(mode="python")` byte-identical.
- [x] [P33-TS02] Schema-coverage test: walk every leaf path in the historical `get_defaults()` (snapshot) and assert each one resolves to a `DoxaConfig` field. Separately assert valid user-only overlay paths (`general.default_profile`, `general.prompt_prefix`, `modes.<name>.system_prompt`, `modes.<name>.prompt_prefix`, and `profiles.<name>.prompt_prefix`) resolve through `ConfigOverlay` / `ProfileConfig`. **This is the test family that catches the `prompy_prefix` typo class without rejecting valid P21 fields.**
- [x] [P33-TS03] `make_partial(DoxaConfig)` regression: produced model has the same field set as `DoxaConfig`, all marked optional with `None` defaults. Field-for-field alignment makes manual drift impossible.
- [x] [P33-TS04] Round-trip test (Position C): `_build_starter_document()` output → parsed root tables excluding `profiles` equal `get_defaults()` projected to `ConfigSchema.starter_keys()`; parsed `profiles` equals `STARTER_PROFILES`; full parsed document validates strict-mode through `UserConfigFile` with zero warnings; rendered TOML contains the substring assertions for `# Doxa Research Configuration File`, `[profiles]`, `[profiles.daily]`.
- [x] [P33-TS05] Warn-only behavior: a config containing `[general] prompy_prefix = "x"` produces exactly one warning (with field path `general.prompy_prefix`) and does **not** raise. With `--no-validate`, the same config produces zero warnings.
- [x] [P33-TS06] `[experimental]` carve-out: arbitrary keys under `[experimental]` produce zero warnings even with `extra="forbid"` elsewhere.
- [x] [P33-TS07] Provider-specific schema fields: `providers.openai.temperature = 0.7` validates through provider schema; `providers.perplexity.search_context_size = "high"` validates; an unknown OpenAI field (`providers.openai.bogus = 1`) warns with the right field path; the Perplexity model rejects (warns on) OpenAI-specific fields. This test is schema acceptance only except where TS09 proves OpenAI runtime consumption.
- [x] [P33-TS08] Mode/profile prompt surface: configs containing `[modes.thinking] system_prompt = "..."`, `[modes.thinking] prompt_prefix = "..."`, `[profiles.fast] prompt_prefix = "..."`, and `[profiles.fast.modes.thinking] system_prompt = "..."` validate with zero warnings. Typos at the same levels (`system_prompy`, `prompy_prefix`) produce precise warnings.
- [x] [P33-TS09] OpenAI consumption regression: when `[providers.openai] temperature = 0.2` is set for an OpenAI model that accepts temperature, the value reaches the OpenAI request builder; when an active profile overlays `[profiles.fast.modes.thinking] system_prompt = "Profile prompt"`, the prompt passed to `OpenAIProvider.submit()` uses `"Profile prompt"` as the mode system prompt. If a modeled OpenAI field is not consumed today, either wire it in P33 or explicitly downgrade it to schema-only and remove the runtime claim.
- [x] [P33-T00] Promote Pydantic to a direct dependency in `pyproject.toml` as `pydantic>=2.12.5,<3` and refresh `uv.lock` without changing the resolved major version. (TS00 must pass.)
- [x] [P33-T01] Define `DoxaConfig` and per-section sub-models in `src/doxa_research/config_schema.py` with values byte-identical to today's `get_defaults()`. Include the `StarterField()` wrapper. (TS01, TS02 default-path assertions must pass.)
- [x] [P33-T02] Implement `make_partial()` helper, derive `PartialDoxaConfig`, define `ConfigOverlay`, `ProfileConfig`, and `UserConfigFile` (top-level overlay + `profiles` + `experimental`). (TS02 overlay assertions, TS03, TS08 must pass.)
- [x] [P33-T03] Implement `ConfigSchema.validate(data, *, strict=False) -> ValidationReport`. Define `ValidationReport` (a dataclass with `warnings: list[ValidationWarning]`) and `ValidationWarning` (`layer`, `path`, `message`, `value_preview`). Wire `--no-validate` into the CLI as a global flag that passes loader metadata (`validate=False`) rather than adding any key to the config override layer. Add a guard that `_no_validate` / `no_validate` is rejected if someone tries to pass it as user config. (TS05, TS06 must pass.)
- [x] [P33-T04] Refactor `ConfigSchema.get_defaults()` body to materialize `DoxaConfig().model_dump(mode="python")` on demand. Verify output remains schema-derived while default factories such as `paths.checkpoint_dir` still observe the active environment. (TS01 still passes; full test suite still passes.)
- [x] [P33-T05] Extract starter profile content to `STARTER_PROFILES` in `src/doxa_research/_starter_data.py` (content frozen verbatim). Refactor `_build_starter_profiles()` and `_build_starter_document()` to: (a) iterate the schema for in-starter fields, (b) read inline comments from field metadata, (c) read structural comments from a writer-owned `WRITER_COMMENTS` table. (TS04 must pass.)
- [x] [P33-T06] Define `ProviderConfigBase`, `OpenAIConfig`, `PerplexityConfig`, and a placeholder `GeminiConfig` with the agreed forward-looking field set (`api_key`, `model`, `temperature`, `max_tokens`, `timeout`, `base_url` on the base; provider-specific fields on subclasses). Mark non-OpenAI future fields as schema-only unless their owning provider project adds runtime pickup tests. (TS07 must pass.)
...
- [x] Regression test status after merging origin/main (P23 + P35 + P36 + P38): `uv run pytest -q` 1189/1189 green with 25 deselected; `./doxa_test -r --skip-interactive -q` 75/87 with 12 API-key-gated skips and 0 failures; `just check`, `just test-lint`, `just test-typecheck`, and `uv run ruff format --check src/ tests/` all pass. (BUG-001 fix made `get_defaults()` materialize on demand; the 11 pre-existing pollution failures we noted earlier are no longer reproducing post-merge — fixed by main's resume/snapshot work.)

### Deliverable
After P33 lands:
- `ConfigSchema.get_defaults()` returns the same schema-derived dict it does today, materialized fresh from `DoxaConfig()` so dynamic default factories are not frozen at import time.
- `doxa-research init` produces a TOML document semantically identical to today, generated from the same schema.
...
- `./doxa_test -r --skip-interactive -q` — no regressions in integration suite.
- `just check` — lint + typecheck clean (Pydantic models type-check under `ty`).

### Manual verification
- Run `doxa-research init --hidden` in a scratch dir; diff output against a pre-P33 capture — should be byte-identical (modulo the `version` line and any tomlkit-version-driven formatting).
- Add `[general] prompy_prefix = "x"` to a config; run `doxa-research status`; observe one warning with field path `general.prompy_prefix`. Add `--no-validate`; observe no warnings.
