...
- [x] [P33-TS01] Schema construction smoke test: instantiating `ThothConfig()` with no overrides succeeds and every default value type-checks. Asserts `ConfigSchema.get_defaults()` equals `_ROOT_SCHEMA.model_dump(mode="python")` byte-identical.
...
- [x] [P33-TS03] `make_partial(ThothConfig)` regression: produced model has the same field set as `ThothConfig`, all marked optional with `None` defaults. Field-for-field alignment makes manual drift impossible.
...
- [x] [P33-T01] Define `ThothConfig` and per-section sub-models in `src/thoth/config_schema.py` with values byte-identical to today's `get_defaults()`. Include the `StarterField()` wrapper. (TS01, TS02 default-path assertions must pass.)
- [x] [P33-T02] Implement `make_partial()` helper, derive `PartialThothConfig`, define `ConfigOverlay`, `ProfileConfig`, and `UserConfigFile` (top-level overlay + `profiles` + `experimental`). (TS02 overlay assertions, TS03, TS08 must pass.)
...
- [x] [P33-T04] Refactor `ConfigSchema.get_defaults()` body to materialize `ThothConfig().model_dump(mode="python")` on demand. Verify output remains schema-derived while default factories such as `paths.checkpoint_dir` still observe the active environment. (TS01 still passes; full test suite still passes.)
