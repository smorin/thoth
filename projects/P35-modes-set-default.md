...
- [x] [P35-TS02] Author tier-matrix tests parametrized over the 7-row matrix (no flag, `--project`, `--config PATH`, each combined with `--profile X`, plus the conflict row). Each row asserts (a) target file received the expected key, (b) other files untouched, (c) JSON envelope shape is correct.
- [x] [P35-TS03] Author **same-tier profile-existence rule** tests for `set-default`: X-only-in-user with `--project` → exit 1; X-only-in-project with default user target → exit 1; X-in-both → accepted; X-nowhere → exit 1; `--config PATH` to a file lacking `[profiles.X]` → exit 1. Error message includes target tier and remediation hint.
...
- [x] [P35-TS06] Author JSON envelope tests asserting the success and error shapes documented in the spec (with and without `--profile`).
...
- [x] [P35-T03] Add hand-written click leaves `modes_set_default` and `modes_unset_default` in `src/thoth/cli_subcommands/modes.py`. Update `_MODES_EPILOG` to mention them. Verify `thoth modes --help` lists both.
...
- [x] [P35-T07] Manual verification: run the seven worked examples from the spec by hand against tmp configs; confirm error messages are clear and actionable.
      Verified all 8 rows from spec § "Worked examples". Note: spec uses `--profile` as an inline flag; in practice it is a root-level option (`thoth --profile X modes set-default ...`). Row 5 in the spec uses NAME=`deep` which is not an actual builtin (real builtins: `default`, `deep_research`, `deep_dive`, `deep-with-stats`, etc.); verified the equivalent behavior with `deep_research`. All 8 rows produced the expected exit codes and error/success messages.
