...
- [ ] [P35-T01] Implement `ConfigDocument.set_default_mode(name, *, profile=None)`, `unset_default_mode(*, profile=None)`, and `default_mode_name(*, profile=None)` in `src/thoth/config_document.py`. Mirror the existing `set_default_profile` / `unset_default_profile` / `default_profile_name` shape and B17 prune-empty semantics.
