...
- [x] [P10-T02] XDG path helpers in src/doxa_research/paths.py with TDD
...
- [x] [P10-T11] Final verification (lint, typecheck, full pytest suite, doxa_test -r)

### Automated Verification
- `just check` passes
- `uv run pytest tests/` passes
- `./doxa_test -r` passes
- API key values masked by default in `doxa-research config get/list` output
