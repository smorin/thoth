"""CLI surface for the `doxa modes` subcommand.
...
from doxa_research._secrets import _mask_tree
from doxa_research.config import BUILTIN_MODES, ConfigManager, mode_kind
...
class _ModesOpSpec:
    """Static CLI-surface description of one `doxa modes <op>` mutator.
...
def _resolve_write_target(
    ...
    """
    from doxa_research.config_write_context import (
...
def get_modes_add_data(
    ...
    *,
    ...
    """Pure data function for `doxa modes add`. Returns a receipt dict.
    ...
    if existing is not None:
        ...
        if existing_model is None:
            return {
                ...
                "message": (
                    ...
                    f"(possibly mid-edit). Use `doxa modes set {name} model {model}` "
                    f"to repair, or `doxa modes remove {name}` to delete and re-add."
                ),
            }
        ...
        return {
            ...
            "message": (
                ...
                f"Use `doxa modes set {name} model {model}` to update."
            ),
        }
...
def get_modes_set_data(
    ...
    *,
    ...
    """Pure data function for `doxa modes set`. Returns a receipt dict.
    ...
    """
    from doxa_research._secrets import _is_secret_key, _mask_secret
    from doxa_research.config import BUILTIN_MODES
    from doxa_research.config_cmd import _parse_value
...
def get_modes_unset_data(
    ...
    *,
    ...
    """Pure data function for `doxa modes unset`. Returns receipt dict.
...
def get_modes_remove_data(
    ...
    *,
    ...
    """Pure data function for `doxa modes remove`. Returns receipt dict.
        ...
        return {
            **base,
            ...
            "message": (
                ...
                f"Drop any user-side override first (`doxa modes remove {old}`) "
                f"and create a new mode at {new!r} via `doxa modes add`."
            ),
        }
...
# ---------------------------------------------------------------------------
# `doxa modes set-default` / `unset-default` — P35
# ---------------------------------------------------------------------------


def _doxa_error_envelope(exc: Exception, *, op_name: str) -> dict:
    from doxa_research.errors import DoxaError
    from doxa_research.json_output import doxa_error_code

    if not isinstance(exc, DoxaError):
        raise exc
    envelope: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION,
        "op": op_name,
        "error": doxa_error_code(exc),
        "message": exc.message,
    }
    if exc.suggestion:
...
def get_modes_set_default_data(
    ...
    *,
    ...
    """Pure data function for `doxa modes set-default NAME` via modes dispatch."""
    from doxa_research.config_cmd import get_modes_set_default_data as _set_default

    try:
        data = _set_default(name, project=project, profile=profile, config_path=config_path)
    except Exception as exc:
        return _doxa_error_envelope(exc, op_name="set-default")
...
def get_modes_unset_default_data(
    *,
    ...
    """Pure data function for `doxa modes unset-default` via modes dispatch."""
    from doxa_research.config_cmd import get_modes_unset_default_data as _unset_default
...
def _derive_kind(cfg: dict[str, Any], warnings: list[str]) -> Kind:
    """Resolve a mode's display kind for the `doxa modes` table.
...
def get_modes_list_data(
    *,
    ...
    """Pure data function for `doxa modes list`.
...
def modes_command(op: str | None, args: list[str], *, config_path: str | None = None) -> int:
    """Dispatch `doxa modes <op>`. Returns a process exit code."""
    if op is None:
