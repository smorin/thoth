...
from doxa_research.paths import user_config_dir

LEGACY_USER_FILENAME = "config.toml"
LEGACY_PROJECT_PATHS: tuple[str, ...] = ("./doxa_research.toml", "./.doxa_research/config.toml")
...
def format_legacy_config_guidance(legacy_paths: list[Path] | None = None) -> str | None:
    ...
    lines.append(
        ...
        "doxa.config.toml (or .doxa.config.toml in the project root)."
    )
    return "\n".join(lines)
