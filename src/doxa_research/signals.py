...
(`doxa_research.signals`) directly rather than the re-export shim in `doxa_research.__main__`,
...
def handle_sigint(signum: int, frame: Any) -> None:
    ...
    if _current_checkpoint_manager and _current_operation:
        ...
        try:
            ...
            _console.print(
                f"[green]✓[/green] Checkpoint saved. Resume with: doxa resume {_current_operation.id}"
            )
        except Exception as e:
            _console.print(f"[red]Error saving checkpoint:[/red] {e}")

    if _current_operation is not None:
        print(f"\nResume later: doxa resume {_current_operation.id}", file=sys.stderr)
