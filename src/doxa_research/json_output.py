...
def doxa_error_code(exc: DoxaError) -> str:
    """Map Doxa Research exceptions to stable JSON error codes."""
...
def emit_doxa_error(exc: DoxaError) -> NoReturn:
    """Emit a DoxaError using the shared JSON error contract."""
    details = {"suggestion": exc.suggestion} if exc.suggestion else None
    emit_error(doxa_error_code(exc), exc.message, details, exit_code=exc.exit_code)


def run_json_doxa_boundary(build: Callable[[], T]) -> T:
    """Run JSON command work and convert DoxaError failures to envelopes."""
    try:
        return build()
