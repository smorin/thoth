"""CLI surface for the `doxa config` subcommand."""
...
from doxa_research._secrets import _mask_tree as _mask_in_tree
from doxa_research.config import ConfigManager
from doxa_research.config_write_context import ConfigWriteContext
...
def get_config_get_data(
    ...
    *,
    ...
    """Pure data function for `doxa config get KEY`.
...
def get_config_set_data(
    ...
    *,
    ...
    """Pure data function for `doxa config set KEY VALUE`."""
    if project and config_path is not None:
        return {
            "key": key,
            "value": None,
...
def get_config_unset_data(
    ...
    """Pure data function for `doxa config unset KEY`."""
    if project and config_path is not None:
        return {
            "key": key,
            "removed": False,
...
def get_config_list_data(
    *,
    ...
    """Pure data function for `doxa config list`."""
    cm = _load_manager(config_path, profile=profile)
    ...
    *,
    ...
    while i < len(args):
        ...
        if a == "--layer":
            ...
            console.print(
                "[red]Error:[/red] --raw is only supported on 'doxa config get'; "
                "use 'doxa config list --json' for machine-readable output"
            )
            return 2
        else:
...
def get_config_path_data(*, project: bool, config_path: str | Path | None = None) -> dict:
    """Pure data function for `doxa config path`."""
    if project and config_path is not None:
...
def _op_help(args: list[str], *, config_path: str | Path | None = None) -> int:
    if args:
        ...
        return 2
    from doxa_research.help import show_config_help
...
def get_config_edit_data(*, project: bool, config_path: str | Path | None) -> dict:
    """Pure data function for `doxa config edit`.
...
def config_command(
    ...
    *,
    ...
    """Dispatch `doxa config <op>`. Returns a process exit code."""
...
def get_config_profile_add_data(
    ...
    *,
    ...
    """Pure data function for `doxa config profiles add NAME`.
...
def get_config_profile_remove_data(
    ...
    *,
    ...
    """Pure data function for `doxa config profiles remove NAME`.
...
def get_config_profile_set_data(
    ...
    *,
    ...
    """Pure data function for `doxa config profiles set NAME KEY VALUE`."""
    if project and config_path is not None:
        return {
            "profile": name,
            "key": key,
...
def get_config_profile_unset_data(
    ...
    *,
    ...
    """Pure data function for `doxa config profiles unset NAME KEY`.
...
def get_config_profile_show_data(
    ...
    *,
    ...
    """Pure data function for `doxa config profiles show NAME`.
...
def get_config_profile_current_data(
    *,
    ...
    """Pure data function for `doxa config profiles current`.
...
def get_config_profile_set_default_data(
    ...
    *,
    ...
    """Pure data function for `doxa config profiles set-default NAME`.
    ...
    if name not in catalog_names:
        raise ConfigProfileError(
            ...
            source="doxa config profiles set-default",
        )
...
def get_config_profile_unset_default_data(
    *,
    ...
    """Pure data function for `doxa config profiles unset-default`.
...
def get_modes_set_default_data(
    ...
    *,
    ...
    """Pure data function for `doxa modes set-default NAME`.
    ...
    from doxa_research.errors import ConfigProfileError, ModeNotFoundError
...
def _same_tier_profile_suggestion(
    ...
    *,
    ...
    if project:
        fix = (
            f"Create it with `doxa config profiles add {profile} --project`, "
            "or remove `--project` if you meant the user-tier profile."
        )
    elif config_path is not None:
        fix = (
            f"Create it with `doxa --config {target_path} config profiles add {profile}`, "
            "or choose a config file that already defines the profile."
        )
    else:
        fix = (
            f"Create it with `doxa config profiles add {profile}`, "
...
def get_modes_unset_default_data(
    *,
    ...
    """Pure data function for `doxa modes unset-default`.
...
def get_config_profile_list_data(
    *,
    ...
    """Pure data function for `doxa config profiles list`.
