...
_DEFAULT_RC: dict[str, tuple[str, ...]] = {
    "bash": (".bashrc",),
    "zsh": (".zshrc",),
    "fish": (".config", "fish", "completions", "doxa_research.fish"),
}

_BLOCK_RE = re.compile(
    r"# >>> doxa completion >>>.*?# <<< doxa completion <<<\n?",
    re.DOTALL,
)
    ...
    *,
    ...
    if manual:
        msg = (
            ...
            f"{block}\n"
            "Remove with:\n"
            "  sed -i '/# >>> doxa completion >>>/,/# <<< doxa completion <<</d' "
    ...
    return InstallResult(
        ...
        path=target,
        message=(
            f"Wrote doxa completion block to {target}.\n"
