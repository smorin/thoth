"""`doxa ask "PROMPT"` Click subcommand.
...
so both `doxa ask --mode X "..."` and `doxa --mode X ask "..."` work.
"""
...
def ask(
    ...
    if interactive:
        raise click.UsageError(
            "--interactive does not apply to 'doxa ask'; use 'doxa -i' (interactive mode) instead."
        )
    if clarify:
        raise click.UsageError(
            "--clarify does not apply to 'doxa ask'; "
            "use 'doxa --clarify' (interactive mode with clarification) instead."
        )
    ...
    if pick_model:
        raise click.UsageError(
            "--pick-model does not apply to 'doxa ask'; "
            "it requires interactive mode. Use 'doxa -i --pick-model' instead."
        )
    ...
    if as_json:
        ...
        from doxa_research.completion.sources import operation_ids
        from doxa_research.config import BUILTIN_MODES, mode_kind
        from doxa_research.errors import DoxaError
        from doxa_research.json_output import (
            emit_doxa_error,
            emit_error,
            emit_json,
            run_json_doxa_boundary,
        )
        from doxa_research.run import get_resume_snapshot_data

        effective_mode, effective_project, effective_prompt = run_json_doxa_boundary(
            _prepare_request
        )
