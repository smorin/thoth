"""`doxa completion <shell>` Click subcommand.
...
Behavior:
  - `doxa completion <shell>` (no flags) — emit eval-able script to stdout.
  - `doxa completion <shell> --install` (TTY) — write fenced block; prompt on overwrite.
  - `doxa completion <shell> --install --force` — write/overwrite silently.
  - `doxa completion <shell> --install --manual` — print block + instructions; never write.
...
from doxa_research.completion.install import install as do_install
...
def completion(
    ...
    # stays raw for `eval "$(doxa completion zsh)"` use).
