"""`doxa modes` Click subgroup. PR2 ships `list` only.
...
flag is gated to its `doxa modes list <flag>` canonical via Q6-C1
...
from doxa_research.completion.sources import mode_kind as _mode_kind_completer
from doxa_research.completion.sources import mode_names as _mode_names_completer

_PASSTHROUGH_CONTEXT = {"ignore_unknown_options": True, "allow_extra_args": True}

_LEGACY_FLAG_TO_NEW_FORM: dict[str, str] = {
    "--json": "doxa modes list --json",
    "--show-secrets": "doxa modes list --show-secrets",
    "--full": "doxa modes list --full",
    "--name": "doxa modes list --name",
    "--source": "doxa modes list --source",
}


_MODES_EPILOG = """\b
Mutation operations:
  doxa modes add NAME --model MODEL [--description D] [--kind K]
  doxa modes set NAME KEY VALUE
  doxa modes unset NAME KEY
  doxa modes remove NAME
  doxa modes rename OLD NEW
  doxa modes copy SRC DST [--from-profile X] [--override]
  doxa modes set-default NAME
  doxa modes unset-default
...
def _make_modes_leaf(op_name: str):
    """Generate a click leaf for `doxa modes <op_name>` driven by
    ...
    @modes.command(
        name=op_name,
        context_settings=_PASSTHROUGH_CONTEXT,
        help=f"`doxa modes {op_name}` — see `doxa help modes` for examples.",
    )
    @click.argument("args", nargs=-1, type=click.UNPROCESSED)
    @click.option("--json", "as_json", is_flag=True, help="Emit JSON envelope")
    @click.pass_context
