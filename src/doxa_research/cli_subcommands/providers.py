"""`doxa providers` Click subgroup with leaves: list, models, check.
...
from doxa_research.completion.sources import provider_names as _provider_names_completer
...
_LEGACY_FLAG_TO_NEW_FORM: dict[str, str] = {
    "--list": "doxa providers list",
    "--models": "doxa providers models",
    "--keys": "doxa providers check",
    "--check": "doxa providers check",
    "--refresh-cache": "doxa providers models --refresh-cache",
    "--no-cache": "doxa providers models --no-cache",
}
...
def providers(ctx: click.Context) -> None:
    ...
    # Q5-A row 4: bare `doxa providers` exits 2 (Click default for required subgroup).
...
def providers_list_cmd(ctx: click.Context, filter_provider: str | None, as_json: bool) -> None:
    ...
    from doxa_research import commands as _commands
    from doxa_research.config import get_config
    from doxa_research.json_output import emit_error, emit_json, run_json_doxa_boundary

    effective_provider = pick_value(filter_provider, ctx, "provider")
    profile = inherited_value(ctx, "profile")

    if as_json:
        config_manager = run_json_doxa_boundary(lambda: get_config(profile=profile))
