# Doxa Research v2.5 Implementation Plan - Version 6.0

## Overview

This plan reflects the current state as of March 2026 and identifies the next
priorities for Doxa Research development. It supersedes plan v5.0 with updated milestone
...
**Root Cause**: `InteractiveSession._create_help_text()` (doxa-research:3795) reads
...
- [ ] **MA-T01**: `doxa-research -i --provider mock` launches without crashing [INT-smoke]
- [ ] **MA-T02**: Interactive mode shows "Doxa Research Interactive Mode" banner [INT-01 subset]
...
- [ ] **MA-04**: Run `./doxa_test -r --interactive` and verify all INT tests pass
- [ ] **MA-05**: Run `make check && make fix` and confirm no lint errors

### Acceptance Criteria

- `./doxa_test -r --interactive` passes all 16 INT tests
...
`doxa-research --provider perplexity "query"` returns a real response.

**Status**: Not started (skeleton with `NotImplementedError` exists at doxa-research:2286)
...
- [ ] **MB-T09**: `doxa-research providers -- --models` lists `sonar-deep-research`
...
- `./doxa_test -r --provider perplexity` passes all Perplexity tests (requires key)
- `./doxa_test -r --provider mock` still passes (no regression)
- `make check` passes
...
- [ ] **MC-T07**: `doxa-research providers -- --models` lists Gemini models
...
**Goal**: Implement `doxa-research update` and `doxa-research clean` commands (Milestones 23 and 24
...
- [ ] **MD-T01**: `doxa-research update` detects stale queued operations and marks them failed
- [ ] **MD-T02**: `doxa-research update --dry-run` shows changes without applying
- [ ] **MD-T03**: `doxa-research clean --completed` deletes completed checkpoints
- [ ] **MD-T04**: `doxa-research clean --failed` deletes failed checkpoints
- [ ] **MD-T05**: `doxa-research clean --days 7` deletes checkpoints older than 7 days
- [ ] **MD-T06**: `doxa-research clean --dry-run` shows what would be deleted
- [ ] **MD-T07**: `doxa-research clean` prompts for confirmation before deletion
- [ ] **MD-T08**: `doxa-research clean` with `--force` skips confirmation
- [ ] **MD-T09**: `doxa-research list --completed` shows only completed operations
- [ ] **MD-T10**: `doxa-research list --failed` shows only failed operations
...
- [ ] **ME-T01**: `./doxa_test -r --provider mock` passes 100% of mock tests
- [ ] **ME-T02**: `./doxa_test -r` (provider-agnostic) passes 100% of agnostic tests
