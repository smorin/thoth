# Product Requirements Document – Doxa Research v2.5
...
- Integrated UV export for extracting embedded dependencies from doxa-research script
...
- Added ModelCache class for managing cached model lists in ~/.doxa-research/model_cache/
...
- Shell completion shipped (`doxa-research completion bash|zsh|fish`) per P16 PR3.
...
Doxa Research is a command-line interface (CLI) tool that automates deep technical research using multiple LLM providers. **The primary use case is simple: just give it a prompt and get comprehensive research results in your current directory.** While Doxa Research supports advanced features like mode selection, project organization, and async operations, the default experience is optimized for immediate, zero-configuration research.

### Core Value Propositions
- **Instant research**: Just `doxa-research "your prompt"` – no mode selection needed
...
| Term | Definition |
|------|------------|
| Default mode | When no mode is specified, uses a special 'default' mode that passes prompts directly to the LLM without any system prompt. Files created with this mode include 'default' in the filename pattern. NOTE: Quick mode (`doxa-research "prompt"`) uses "default" mode, NOT "deep_research" |
...
| Model cache | Local storage of provider model lists to reduce API calls and improve performance. Stored in ~/.doxa-research/model_cache/ |
...
| F-02 | Accept prompt as single positional argument: `doxa-research "prompt"` | Must | T-CLI-01 |
...
| F-05 | Single config file `~/.config/doxa-research/doxa-research.config.toml` with all settings | Must | T-CFG-01 |
...
| ID | Requirement | Priority | Test ID |
|----|-------------|----------|---------|
| F-26 | `doxa-research "prompt"` executes in default mode in current directory | Must | T-QUICK-01 |
...
| F-113 | Show list of available providers with cache status when `doxa-research providers -- --list` is run | Must | T-PROV-13 |
...
| F-123 | Store model cache in ~/.doxa-research/model_cache/ directory | Must | T-CACHE-06 |
...
| F-141 | Implement `/status [operation-id]` interactive command. With no argument, reports the last operation submitted in the current session; with an operation ID, delegates to the same code path as the top-level `doxa-research status <ID>` command. | Must | T-INT-13 |
...
| F-148 | Support project-level configuration file (./doxa.config.toml or ./.doxa-research.config.toml) | Should | T-ARCH-06 |
...
| ID | Requirement | Priority | Test ID |
|----|-------------|----------|---------|
| F-177 | On a successful `--async` submission, stdout must print exactly the following three lines — and must not print them on submission failure:<br>`Research submitted`<br>`Operation ID: <operation-id>`<br>`Check later with: doxa-research status <operation-id>` | Must | T-ASYNC-08 |
...
doxa "PROMPT"                   # Deep research with output to current directory

# INTERACTIVE MODE - Visual Prompt Input
doxa -i                         # Enter interactive mode (Edit Mode)
doxa --interactive              # Same as above
doxa -i --clarify              # Start in Clarification Mode

# ADVANCED USAGE - Full Control
doxa MODE PROMPT [OPTIONS]      # Specify mode and options
...
doxa "test prompt" -P mock  # Short form
```
...
doxa --prompt-file ./complex_research_prompt.txt

# Pipe prompt from another command
echo "analyze security implications of WebAssembly" | doxa-research --prompt-file -
...
doxa config get providers.openai.model  # Get a config value
doxa config set providers.openai.model gpt-4o  # Set a config value

# List available models
doxa providers --models                      # List all models for all providers
doxa providers --models --provider openai    # List OpenAI models only
doxa providers --models -P perplexity        # List Perplexity models only

# Model cache management
doxa providers -- --list                     # Show providers with cache status
doxa providers -- --models --refresh-cache   # Force refresh model cache
doxa providers -- --models --provider openai --refresh-cache  # Refresh specific provider
doxa providers -- --models --no-cache        # Bypass cache completely without updating it
```
...
| providers | List available models | `doxa-research providers --models` |
...
USAGE:
    doxa-research "PROMPT"                   # Quick research (recommended)
    doxa-research MODE PROMPT [OPTIONS]      # Advanced usage
...
EXAMPLES:
    ...
    doxa-research "test prompt" --provider mock
...
✓ Configuration saved to ~/.config/doxa-research/doxa-research.config.toml
...
To see available models, use: doxa-research providers -- --models
To refresh model cache, use: doxa-research providers -- --models --refresh-cache
...
The system works with zero configuration if API keys are in environment variables. The config file at `~/.config/doxa-research/doxa-research.config.toml` provides additional control:
...
When you run `doxa-research "your prompt"`, files are created in your current directory:
...
- Note: Due to Click option parsing, providers command requires -- separator: `doxa-research providers -- --models`
...
## End of Doxa Research v2.1 PRD

This specification prioritizes the simplest possible user experience while maintaining all advanced capabilities. The primary innovation is making `doxa-research "prompt"` the default interaction pattern, removing friction for users who just want quick, comprehensive research results in their current directory.
