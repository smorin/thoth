# Doxa Research `config` Subcommand + XDG Layout — Design v1

**Status:** Approved design (2026-04-17). Implementation via TDD.
**Scope:** New `doxa-research config` subcommand (get/set/unset/list/path/edit/help) and switch of all user-writable filesystem paths to XDG Base Directory Specification. No migration of legacy paths.
...
Doxa Research's config is layered (defaults → user TOML → project TOML → env → CLI) but there is no in-CLI way to inspect or modify it. Users must hand-edit TOML. The current user-writable directory is chosen by `platformdirs`, which returns `~/Library/Application Support/doxa-research` on macOS — this is neither XDG-compliant nor consistent with the Linux path users expect. Additionally, checkpoints (state) and the model cache are stored under the config dir, mixing three XDG categories into one.

Goals:

- Give users `doxa-research config {get,set,list,...}` for introspection and edits without hand-editing TOML.
...
| Purpose | Env var | Default | Doxa Research subpath |
|---|---|---|---|
| Config | `XDG_CONFIG_HOME` | `~/.config` | `doxa-research/doxa-research.config.toml` |
| State (checkpoints) | `XDG_STATE_HOME` | `~/.local/state` | `doxa-research/checkpoints/` |
| Cache (model cache) | `XDG_CACHE_HOME` | `~/.cache` | `doxa-research/model_cache/` |
...
### New module: `src/doxa_research/paths.py`

```python
def user_config_dir() -> Path:  # $XDG_CONFIG_HOME/doxa-research or ~/.config/doxa-research
def user_state_dir() -> Path:   # $XDG_STATE_HOME/doxa-research or ~/.local/state/doxa-research
def user_cache_dir() -> Path:   # $XDG_CACHE_HOME/doxa-research or ~/.cache/doxa-research

def user_config_file() -> Path:       # user_config_dir() / "doxa-research.config.toml"
...
- `src/doxa_research/config.py` (L20, L147, L197) — defaults + `ConfigManager.__init__`.
- `src/doxa_research/models.py` (L19, L108) — `ModelCache` default cache dir.
- `src/doxa_research/commands.py` (L18, L72) — `init_command` default config path.
- `src/doxa_research/help.py` (L12, L106) — `show_init_help` displayed path.
...
Same pattern as existing subcommands — `cli.py` recognizes `"config"` as `args[0]` and routes to `config_command(op, rest, ...)` in a new `src/doxa_research/config_cmd.py` module. `config.py` stays the schema/loader; `config_cmd.py` owns the CLI surface.

Operations: `get`, `set`, `unset`, `list`, `path`, `edit`, `help`.

## 4. Per-op semantics

### 4.1 `doxa-research config get <KEY>`
...
### 4.2 `doxa-research config set <KEY> <VALUE>`

Writes `<KEY>=<VALUE>` to the user config file (creating the file if missing).

Flags:
- `--project` — target `./doxa.config.toml` instead.
...
### 4.3 `doxa-research config unset <KEY>`
...
Flags:
- `--project` — target `./doxa.config.toml`.

### 4.4 `doxa-research config list`
...
### 4.5 `doxa-research config path`

Prints the target config file path (absolute).

Flags:
- `--project` — print `./doxa.config.toml` (prints the would-be path if not yet created).

No flag: prints the user config file path.

### 4.6 `doxa-research config edit`

Opens the target config file in `$EDITOR` (fallback: `vi`). Creates the file with a minimal header (`# Doxa Research Configuration File\nversion = "2.0"\n`) if it doesn't exist.

Flags:
- `--project` — edit `./doxa.config.toml`.

### 4.7 `doxa-research config help`

Renders `show_config_help()` from `help.py`. Also reachable via `doxa-research help config` and `doxa-research --help config` (existing dispatch patterns in `DoxaCommand.parse_args` and the `help` subcommand in `cli.py`).
...
- `DoxaCommand.parse_args` and the `help` subcommand in `cli.py` recognize `config`.
...
- `user_config_dir()` returns `$XDG_CONFIG_HOME/doxa-research` when env set; `~/.config/doxa-research` when unset; `~/.config/doxa-research` when empty string (per spec).
...
- `set --project general.default_mode deep_dive` writes to `./doxa.config.toml`; user TOML untouched.
...
- `config path` prints the user path; `--project` prints `./doxa.config.toml`.
...
- `config help`, `doxa-research help config`, and `doxa-research --help config` all print identical text.
...
**Modified:**
- `src/doxa_research/config.py` — swap `platformdirs.user_config_dir("doxa-research")` for `paths.user_config_dir()`; update `get_defaults()["paths"]["checkpoint_dir"]`.
- `src/doxa_research/models.py` — swap to `paths.user_cache_dir() / "model_cache"`.
- `src/doxa_research/commands.py` — swap `init_command` default config path; register `config` op in `CommandHandler.commands`.
- `src/doxa_research/cli.py` — dispatch `"config"` in the subcommand chain; extend `--help config` interception.
- `src/doxa_research/help.py` — `show_config_help()`, `build_epilog()`, `show_general_help`.
- `pyproject.toml` — add `tomlkit`; drop `platformdirs` if no remaining consumers.

## 10. Out of scope (explicitly excluded)

- Migration of existing `~/Library/Application Support/doxa-research` or prior `~/.config/doxa-research` content.
- Read-through fallback to legacy locations.
- Interactive `doxa-research init` wizard rewrite.
...
4. New module `src/doxa_research/config_cmd.py` keeps `config.py` focused on schema/loading.
