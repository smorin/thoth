# Doxa Research v2.5 Implementation Plan - Version 5.0
...
- [x] **M1-03**: Add query parsing for quick mode (`doxa-research "query"`)
...
- [x] **M1T-01**: Verify `doxa-research --version` displays version ✅ [T-CLI-01]
- [x] **M1T-02**: Verify `doxa-research "test query"` creates output file ✅ *Works with --provider mock* [T-QUICK-01]
...
- [x] **M2-05**: Implement config file path resolution (`~/.config/doxa-research/doxa-research.config.toml`)
...
- [x] **M2-09**: Add config file location option ✅ *--config flag implemented*
...
- [x] **M3T-01**: Verify mock provider completes after delay ✅ *Works with --provider mock* [T-MOCK-01]
...
- Quick mode (`doxa-research "query"`) uses "default" mode (no system prompt), NOT "deep_research"
...
- [x] **M4T-06**: Verify `doxa-research "query"` uses default mode, not deep_research ✅ *Confirmed* [T-QUICK-01]
...
- [x] **M6-08**: Implement --combined flag ✅ *Opt-in flag implemented; combined reports not generated by default*
...
- [x] **M6T-03**: Verify combined reports merge all providers ✅ *Code implemented with generate_combined_report()* [T-OUT-03]
- [x] **M6T-04**: Verify duplicate filenames are handled ✅ *Dedup counter logic in generate_filename()* [T-OUT-04]
...
**Goal**: Implement `doxa-research update` command to fix stale operation statuses
...
**Goal**: Implement `doxa-research clean` command with comprehensive filtering options
...
- [ ] **M27T-01**: Verify `doxa-research providers -- --list` shows all providers [T-PROV-13]
...
- [ ] **M35-07**: Add Gemini to `doxa-research providers -- --models` listing
...
- [ ] **M35T-07**: Provider listed in `doxa-research providers`
