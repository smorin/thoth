...
**Goal:** Fix the four confirmed defects in `doxa-research`:
...
**Scope:** `src/doxa_research/__main__.py`, `doxa_test`, documentation (plan, PRD, README, CONTRIBUTING, RELEASE, CHANGELOG), and minimal supporting test files under `tests/`.
...
- **[P07-TS02]** — Regression proving `doxa-research status <ID>` after async submission reports provider state, not a permanently empty queued shell.
  PRD refs: F-13 / T-CMD-02.
- **[P07-TS03]** — Regression proving the exact async success output contract from F-177 / T-ASYNC-08 (`Research submitted` + `Operation ID: <id>` + `Check later with: doxa-research status <id>`) appears **only** after a provider job exists.
...
### Acceptance Criteria
- `doxa-research "prompt" --provider mock --api-key-mock dummy --async` prints the F-177 success block only after a provider job exists.
- `doxa-research status <ID>` shows provider details for the async operation.
...
### Test Design
- **[P07-TS04]** — Regression covering `doxa-research providers -- --list` so a not-implemented provider is not shown as `✓ Ready`.
...
- [ ] **[P07-T08]** Convert the current `NotImplementedError` path into a user-facing `DoxaError` / `ProviderError` with a clear message and suggestion.
...
### Acceptance Criteria
- `doxa-research providers -- --list` no longer reports Perplexity as `✓ Ready`.
- `doxa-research "prompt" --provider perplexity ...` fails cleanly with an intentional message.
...
- **[P07-TS08]** — Regression proving `/status <operation-id>` delegates to the same code path as the top-level `doxa-research status <ID>` command.
...
- `/status <id>` works for any existing operation, matching `doxa-research status <id>`.
...
### Implementation Tasks
- [ ] **[P07-T15]** Update `planning/doxa-research.prd.v24.md`:
...
- Run only the new or affected `doxa_test` cases for each task first.
...
8. Re-run `just check`
9. Re-run `./doxa_test -r`

### Additional Spot Checks
- `./doxa "async smoke" --provider mock --api-key-mock dummy --async`
- `./doxa status <operation-id>`
- `./doxa providers -- --list`
