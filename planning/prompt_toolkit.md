...
* Modular architecture to allow:
  ...
  2. A Doxa Research-specific version integrated into its CLI ecosystem

I'll provide working prototype code, implementation notes, and guidance on integration with Doxa Research. I’ll update you once the research is ready.


# Designing an Interactive Prompt Toolkit Input Box for Doxa Research CLI

## Overview

To build a **pure CLI interactive input** for Doxa Research, we will leverage the [Python Prompt Toolkit](https://github.com/prompt-toolkit/python-prompt-toolkit) library. Prompt Toolkit provides rich text-based UI components and key handling, allowing us to create a **bordered text input box with multi-line editing, custom key bindings, and dynamic slash commands**. According to the Doxa Research v2.3 requirements, the interactive mode should include: a bordered input box with multi-line support, slash commands for option configuration, help text above the input, placeholder text, Unix-style shortcuts (e.g. Ctrl+A/E/K), and should exit after submitting one query. We will design an architecture meeting these needs and outline two implementations:

1. **Prototype & Tutorial:** a self-contained example demonstrating the key features (easy to integrate into Doxa Research).
2. **Doxa Research-Integrated Version:** tailored to Doxa Research’s internal commands and workflow (reusing Doxa Research’s CLI logic).
...
* **Single Query Submission:** Once the user finalizes their prompt (and presses Enter to submit), the interactive UI should **exit** and hand off the query to Doxa Research’s normal execution flow. In other words, the interactive mode is a one-shot composer for a query (not a persistent REPL).
...
* **Running External Code (for slash commands):** Prompt Toolkit allows running code that prints to the terminal **without breaking the UI** via `run_in_terminal()`. This will suspend the prompt rendering, run the given function (we can print output or even invoke Rich console printing), then resume the prompt. We will use this to call Doxa Research’s existing CLI logic for commands. For example, when the user enters `/models`, we can call Doxa Research’s `providers_command(..., show_models=True)` which prints a table of models to stdout. Using `run_in_terminal` ensures the output (Rich table in this case) is printed above/outside the prompt cleanly.
...
* When Enter is pressed:
  ...
  * Retrieve the input text. If it begins with `/`, do **not exit** the Application yet. Instead, consume that command:
    ...
    * **`/help`:** We will display help info. This could be a static message listing available slash commands or even reuse Doxa Research’s help. Doxa Research CLI has a help text accessible via `ctx.get_help()` or the `show_*_help()` functions. We might simply print a short custom help for interactive mode (since the commands are different). We can use `run_in_terminal()` to safely print to stdout. For example:
      ...
      This will clear the input box and print the help above (once the app resumes, the printed text will remain above the now-empty prompt box). We can similarly call `show_providers_help()` from Doxa Research for consistency.
    ...
    * **`/provider`:** This command likely means the user wants to choose one or multiple providers. There are a couple of ways to implement selection:
      ...
      For clarity, let’s assume single selection (since Doxa Research’s `--provider` option only takes one, and by default it runs both if not specified). So `/provider` -> user picks one provider to focus on. Implementation:
...
### 4. Submitting the Query to Doxa Research

Once the user has configured settings via slash commands and finally enters a prompt (that is not a command), the Application will exit with that text. We then use the collected state (provider, model, mode, etc.) to call Doxa Research’s internal execution function.

Doxa Research’s CLI code, when not handling a special subcommand, eventually calls `run_research(mode, query, ...)` to perform the operation. In interactive mode, we should mimic the same. For example:
...
* Use `current_provider` and `current_model` state: If a specific provider was chosen, pass `--provider`. If a specific model was chosen, we might need to tell Doxa Research to use that model. Doxa Research CLI doesn’t have a direct `--model` flag; instead, if a provider’s default model is overridden, one way is to create a mode config or use the provider’s own selection mechanism. (Since Doxa Research’s `create_provider()` likely accepts a config specifying which model if not default). We may have to extend Doxa Research to accept a model override in interactive mode, or simply set an environment variable for the chosen model (not ideal). For now, assume choosing a model sets the default for that provider (maybe via config).
* Then call `run_research` (or simulate CLI arguments). Essentially, we'd integrate like:
  ...
  This way, after the interactive prompt, Doxa Research continues as if the user provided those options via CLI. All the rich output generation and file writing will happen as usual.
...
help_text = ("<b>Doxa Research Interactive Mode</b>\n"
...
def _(event):
    ...
    if text.startswith("/"):
        ...
        if cmd == "/help":
            ...
            def list_providers():
                # Imagine this uses Doxa Research's providers_command(show_list=True)
...
## Integrating with Doxa Research’s Architecture

With the prototype validated, integrating into Doxa Research involves a few adjustments:

* **Embedding in Doxa Research Codebase:** We likely create an `interactive_mode()` function within the Doxa Research CLI script (or a separate module). When the `-i/--interactive` flag is passed (detected in Click options), we invoke `interactive_mode()` instead of the normal query execution path. E.g., in the `cli()` function, add:
...
* **Reusing CLI Logic:** We should call existing functions to avoid duplication:
  ...
  * For help text, Doxa Research’s `show_providers_help()` (and similar for init, status, etc.) print detailed usage. We might have an interactive-specific help, but could also piggyback on these. Perhaps simpler: define our own short help as shown above.
...
* Doxa Research CLI Implementation – **Providers listing and model fetching** (for reuse in slash commands)
* Doxa Research CLI Implementation – **Command help and dispatch logic** (reused for help output and command detection)
...
https://github.com/smorin/doxa-research/blob/f3aacc45c9b8a6a9451d5b4431f8da11e24124f2/planning/doxa-research.prd.v23.md#L15-L23
GitHub
doxa.prd.v23.md

https://github.com/smorin/doxa-research/blob/f3aacc45c9b8a6a9451d5b4431f8da11e24124f2/planning/doxa-research.prd.v23.md#L20-L23
...
https://github.com/smorin/doxa-research/blob/f3aacc45c9b8a6a9451d5b4431f8da11e24124f2/doxa-research#L1929-L1938
GitHub
doxa

https://github.com/smorin/doxa-research/blob/f3aacc45c9b8a6a9451d5b4431f8da11e24124f2/doxa-research#L1947-L1955
...
https://github.com/smorin/doxa-research/blob/f3aacc45c9b8a6a9451d5b4431f8da11e24124f2/doxa-research#L536-L545
GitHub
doxa

https://github.com/smorin/doxa-research/blob/f3aacc45c9b8a6a9451d5b4431f8da11e24124f2/doxa-research#L718-L727
GitHub
doxa

https://github.com/smorin/doxa-research/blob/f3aacc45c9b8a6a9451d5b4431f8da11e24124f2/doxa-research#L536-L544
GitHub
doxa

https://github.com/smorin/doxa-research/blob/f3aacc45c9b8a6a9451d5b4431f8da11e24124f2/doxa-research#L1866-L1875
GitHub
doxa

https://github.com/smorin/doxa-research/blob/f3aacc45c9b8a6a9451d5b4431f8da11e24124f2/doxa-research#L1876-L1884
...
https://github.com/smorin/doxa-research/blob/f3aacc45c9b8a6a9451d5b4431f8da11e24124f2/doxa-research#L1948-L1956
GitHub
doxa

https://github.com/smorin/doxa-research/blob/f3aacc45c9b8a6a9451d5b4431f8da11e24124f2/doxa-research#L622-L632
GitHub
doxa

https://github.com/smorin/doxa-research/blob/f3aacc45c9b8a6a9451d5b4431f8da11e24124f2/doxa-research#L630-L639
...
https://github.com/smorin/doxa-research/blob/f3aacc45c9b8a6a9451d5b4431f8da11e24124f2/planning/doxa-research.prd.v23.md#L19-L22
GitHub
doxa.prd.v23.md

https://github.com/smorin/doxa-research/blob/f3aacc45c9b8a6a9451d5b4431f8da11e24124f2/planning/doxa-research.prd.v23.md#L15-L19
GitHub
doxa

https://github.com/smorin/doxa-research/blob/f3aacc45c9b8a6a9451d5b4431f8da11e24124f2/doxa-research#L1866-L1874
GitHub
doxa.prd.v23.md

https://github.com/smorin/doxa-research/blob/f3aacc45c9b8a6a9451d5b4431f8da11e24124f2/planning/doxa-research.prd.v23.md#L22-L24
