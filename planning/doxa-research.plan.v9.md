# Doxa Research Ergonomics v1 — Implementation Plan (v9, supersedes v8)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Related:**
- Spec: `docs/superpowers/specs/2026-04-24-doxa-research-ergonomics-design.md`
- Already-landed dependency: P11 (`doxa-research modes` + `is_background_mode`) → commit `4cbc5ec`
- Already-landed dependency: P13 (`is_background_model(str)` primitive) → commit `89498ef`

**Goal:** Land the ergonomic improvements from the approved spec on top of the P11/P13 foundation. Net surface change: `providers list/models/check` subcommands + deprecation shim, sync-deep-research progress spinner, restructured help with workflow ladder + worked examples + `-v` example, API-key documentation pass with `doxa-research help auth`, `--input-file`/`--auto` clearer help, `--pick-model` flag (immediate-modes only), and config-path-on-errors.

**Architecture:** Reuse the P11/P13 helper `is_background_model(model: str | None) -> bool` from `doxa-research.config` everywhere we need a quick/deep classification. Two new shared helpers: `format_config_context()` in `errors.py` (consistent error bodies) and a `render_auth_help()` renderer in `help.py` (in-CLI auth guidance). The `providers` command becomes a real Click subcommand group with a one-release deprecation shim for the old `providers -- --list` form. Spinner integration is a small `src/doxa_research/progress.py` module + a wrapper in `run.py`.

**Tech Stack:** Python 3.11+, click, rich, `thothspinner` (new dep), pytest + `click.testing.CliRunner` + `./doxa_test`, ruff + ty.

**Diff from v8:**
- ❌ **Drop v8 Task 1** — `is_deep_research_model` is unnecessary; `is_background_model` already exists at `src/doxa_research/config.py:136`. All v8 references rewritten to use the existing name.
- ❌ **Drop v8 Task 6** — `doxa-research workflow` command. User direction; `doxa-research modes` already shows kind.
- ✏️ **Simplify Task 4 (help epilog)** — `doxa-research modes` already lists modes by kind. Limit epilog edit to: add the workflow chain line + new worked examples (`--auto` chain, `--resume`, `-v` debug). Skip the quick/deep grouping inside the epilog.
- ✏️ **Task 7 (providers subgroup)** — leverage the existing `COMMANDS` tuple at `src/doxa_research/help.py` introduced by commit `2e14333`.
...
| `doxa-research help auth` ordering | `tests/test_help_auth.py` (new) | env-vars first, config-file second, CLI-flags last with deprecation hint |
| `doxa-research providers list/models/check` | `tests/test_providers_subcommand.py` (new) | Each subcommand exits 0 with expected output shape |
...
| Regression | `./doxa_test -r --provider mock --skip-interactive` | 63/64 pass, 1 skip, 0 fail |

**Per-task iteration command (fast):** `uv run pytest tests/<file>::<test> -x -v`

**End-of-project gate:** `make env-check && just fix && just check && ./doxa_test -r && just test-fix && just test-lint && just test-typecheck`
...
|---|---|---|
| `src/doxa_research/errors.py` | Modify | `format_config_context()`; enrich `APIKeyError` body |
| `src/doxa_research/help.py` | Modify | Workflow chain + examples + `-v` block in epilog; `render_auth_help()`; `show_auth_help()`; soften `--api-key-*` help via existing renderers |
| `src/doxa_research/cli.py` | Modify | Reword `--input-file`/`--auto`; soften `--api-key-*` help; add `--pick-model/-M`; route `providers <sub>`; add `auth` to help-topic dispatch |
| `src/doxa_research/commands.py` | Modify | `providers_list`, `providers_models`, `providers_check` handlers |
| `src/doxa_research/run.py` | Modify | Apply `--pick-model` override; wrap sync-poll with spinner via gate |
| `src/doxa_research/signals.py` | Modify | Print "Resume later" hint on SIGINT when an op is in flight |
| `src/doxa_research/progress.py` | Create | `should_show_spinner()` gate + `run_with_spinner()` context manager |
| `src/doxa_research/interactive_picker.py` | Create | Model picker UI used by `--pick-model` (immediate modes only) |
...
| `PROJECTS.md` | Modify | Add `P14: Doxa Research CLI Ergonomics v1` block |
| `tests/test_error_context.py` | Create | Helper + APIKeyError tests |
| `tests/test_cli_help.py` | Create | Help-string assertions |
| `tests/test_help_auth.py` | Create | `doxa-research help auth` ordering |
...
from doxa-research.errors import format_config_context
...
Add to `src/doxa_research/errors.py`:
...
```bash
git add tests/test_error_context.py src/doxa_research/errors.py
...
Replace the body in `src/doxa_research/errors.py`:
...
class APIKeyError(DoxaError):
    ...
    def __init__(self, provider: str):
        from doxa-research.paths import user_config_file  # local import to avoid cycles
...
def test_input_file_help_mentions_advanced_usage():
    assert "non-doxa-research document" in _help()
```
...
In `src/doxa_research/cli.py`, replace the existing `--input-file` and `--auto` decorators:
...
```bash
git add tests/test_cli_help.py src/doxa_research/cli.py
...
def test_help_has_async_chain_example():
    out = _help()
    assert "doxa-research deep_research --auto" in out
    assert "--async" in out
```
...
In `src/doxa_research/help.py`, edit `build_epilog()` so the section after `Research Modes:` includes the workflow chain + new examples. Keep the `doxa-research modes` reference for full per-mode detail. Replace the existing `Research Modes` and `Examples` sections with:
...
```python
    ...
    lines.append("  # Quick")
    lines.append('  $ doxa "how does DNS work"')
    lines.append("")
    lines.append("  # Sharpen, then research (chain with --auto)")
    lines.append('  $ doxa clarification "k8s networking" --project k8s')
    lines.append("  $ doxa exploration --auto --project k8s")
    lines.append("  $ doxa deep_research --auto --project k8s --async")
    lines.append("")
    lines.append("  # Resume a backgrounded job")
    lines.append("  $ doxa --resume op_abc123")
    lines.append("")
    lines.append("  # Debug API issues — show model, provider, timeouts, retries")
    lines.append('  $ doxa deep_research "topic" -v')
    lines.append("")
    lines.append("For detailed command help: doxa-research help [COMMAND]")
```
...
```bash
git add tests/test_cli_help.py src/doxa_research/help.py
git commit -m "docs(help): add workflow chain + chained/async/resume/-v examples"
```

---

## Task 5: `doxa-research help auth` + README authentication section

**Files:**
- Create: `tests/test_help_auth.py`
- Modify: `src/doxa_research/help.py`
- Modify: `src/doxa_research/cli.py` (soften `--api-key-*` help)
- Modify: `README.md`
...
Append to `src/doxa_research/help.py`:
...
def render_auth_help() -> str:
    return (
        ...
        "\n"
        "2. Config file (persistent, per-machine): ~/.doxa-research/config.toml\n"
        "     [providers.openai]\n"
        '     api_key = "sk-..."\n'
        "\n"
...
In `DoxaCommand.parse_args` (existing chain in `help.py`), add an `auth` branch:
...
If the existing `HELP_TOPICS` tuple in `help.py` is the source of truth for valid `doxa-research help <topic>` names, append `"auth"` (and add a corresponding entry to `COMMANDS` only if the auth helper should also appear under `Commands:`; per the spec it should not — auth is help-only).
...
Replace the Authentication section of `README.md` with the exact text from `render_auth_help()`, plus a one-line note pointing readers at `doxa-research help auth` for the in-CLI version.

- [ ] **Step 7: Commit**

```bash
git add tests/test_help_auth.py src/doxa_research/help.py src/doxa_research/cli.py README.md
git commit -m "docs(auth): add 'doxa-research help auth' and authentication ordering pass"
```
...
- Modify: `src/doxa_research/cli.py`
- Modify: `src/doxa_research/help.py` (deprecation message hooks; keep `providers` in `COMMANDS`)
...
In `src/doxa_research/commands.py` add (or replace, if there is an existing `providers` handler) three handlers. Reuse existing key-resolution helpers if present in `commands.py` or `config.py`; the snippet below is a self-contained reference to keep this plan complete.
...
Find the existing block in `src/doxa_research/cli.py` that catches commands by name (the block that uses `COMMAND_NAMES`). Where it currently dispatches `providers` to the legacy handler, replace with subcommand routing:
...
if first_arg == "providers":
    ...
    if sub == "list":
        ...
        click.echo(
            "warning: 'doxa-research providers -- --list' is deprecated; "
            "use 'doxa-research providers list'",
            err=True,
        )
        ctx.exit(providers_list(cfg))
    elif sub in (None, "--help", "help"):
        from doxa-research.help import show_providers_help
        show_providers_help()
        ctx.exit(0)
    else:
...
- [ ] **Step 6: Run doxa_test regression**

```bash
./doxa_test -r --provider mock --skip-interactive 2>&1 | tail -10
```
...
```bash
git add tests/test_providers_subcommand.py src/doxa_research/commands.py src/doxa_research/cli.py src/doxa_research/help.py
...
- Create: `src/doxa_research/progress.py`
...
from doxa-research.progress import should_show_spinner
...
```bash
git add tests/test_progress_spinner.py src/doxa_research/progress.py
...
def test_maybe_spinner_calls_spinner_when_gate_passes(monkeypatch):
    ...
    monkeypatch.setattr("doxa-research.run.run_with_spinner", fake_spinner)
    monkeypatch.setattr("doxa-research.run.should_show_spinner", lambda **kw: True)

    from doxa-research.run import _maybe_spinner
...
def test_maybe_spinner_is_noop_when_gate_fails(monkeypatch):
    ...
    monkeypatch.setattr("doxa-research.run.run_with_spinner", fake_spinner)
    monkeypatch.setattr("doxa-research.run.should_show_spinner", lambda **kw: False)
...
Add near the top of `src/doxa_research/run.py`:

```python
from contextlib import contextmanager

from doxa-research.progress import run_with_spinner, should_show_spinner
...
```bash
git add tests/test_progress_spinner.py src/doxa_research/run.py
...
**Files:**
- Modify: `tests/test_progress_spinner.py`
- Modify: `src/doxa_research/signals.py`
...
def test_sigint_prints_resume_hint(capsys):
    import doxa-research.signals as sig
...
If the SIGINT handler in `src/doxa_research/signals.py` uses different function names, run `grep -n "def.*sigint\|signal.SIGINT\|handler" src/doxa_research/signals.py` and adjust the test accordingly. The assertion (output contains the hint) is the part that must hold.
...
In `src/doxa_research/signals.py`, locate the SIGINT handler (the function that calls the checkpoint save). After the save and before `sys.exit(...)`, add:
...
if _current_operation is not None:
    click.echo(
        f"\nBackgrounded. Resume later: doxa-research --resume {_current_operation.id}"
    )
```
...
```bash
git add tests/test_progress_spinner.py src/doxa_research/signals.py
...
In `src/doxa_research/cli.py`, add the option:
...
```python
if pick_model:
    from doxa-research.config import BUILTIN_MODES, is_background_model
    ...
    if is_background_model(mode_cfg.get("model")):
        click.echo(
            ...
            "       the research quality and cost profile; edit ~/.doxa-research/config.toml\n"
...
```bash
git add tests/test_pick_model.py src/doxa_research/cli.py
...
**Files:**
- Create: `src/doxa_research/interactive_picker.py`
...
def test_pick_model_quick_mode_uses_picker(monkeypatch):
    ...
    monkeypatch.setattr("doxa-research.interactive_picker.pick_model", fake_pick)
    ...
    monkeypatch.setattr("doxa-research.run.run_research", fake_run)
...
```python
    ...
    from doxa-research.interactive_picker import pick_model as _pick, immediate_models_for_provider
...
```bash
git add tests/test_pick_model.py src/doxa_research/interactive_picker.py src/doxa_research/cli.py src/doxa_research/run.py
...
```markdown
## [x] Project P14: Doxa Research CLI Ergonomics v1 (v2.12.0)
**Goal**: Reduce first-time-user friction in the doxa-research CLI.
...
- [x] [P14-T04] doxa-research help auth + README authentication ordering pass
...
Expected: every step green; `./doxa_test` reports 63 passed / 1 skipped / 0 failed.

- [ ] **Step 2: Manual smoke checks**

```bash
doxa --help | head -60                          # workflow chain + examples visible
doxa --help auth                                # auth ordering visible
doxa providers list                             # exits 0
doxa providers -- --list                        # works + deprecation line on stderr
doxa --pick-model deep_research "x"             # exit 2, clear error
doxa --pick-model default "x"                   # picker prompt (pipe `echo 1 |` to auto-pick)
doxa deep_research "hello" --provider mock      # spinner appears
```
...
| 3.4 `doxa-research workflow` | **Dropped** — covered by existing `doxa-research modes` + Task 4 ladder line |
