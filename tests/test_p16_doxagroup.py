...
from doxa_research.help import DoxaGroup
...
def test_invoke_routes_mode_positional(fake_group, monkeypatch):
    ...
    monkeypatch.setattr("doxa_research.help._run_research_default", fake_run)
    monkeypatch.setattr("doxa_research.config.BUILTIN_MODES", {"deep_research", "default"})
...
def test_init_subcommand_invokes_handler(monkeypatch):
    """P16-TS07: doxa init dispatches through Click to CommandHandler.init_command."""
    from doxa_research.cli import cli
    ...
    monkeypatch.setattr("doxa_research.commands.CommandHandler.init_command", fake_init)
...
def test_status_requires_op_id():
    """P16-TS08: doxa status (no OP_ID) → op-required error (Click default exit 2)."""
    from doxa_research.cli import cli
...
def test_status_invokes_handler(monkeypatch):
    ...
    monkeypatch.setattr("doxa_research.commands.CommandHandler.status_command", fake_status)
    runner = CliRunner()
...
def test_list_all_flag(monkeypatch):
    ...
    monkeypatch.setattr("doxa_research.commands.CommandHandler.list_command", fake_list)
    runner = CliRunner()
...
def test_providers_list_invokes_correct_function(monkeypatch):
    ...
    monkeypatch.setattr("doxa_research.commands.providers_command", fake_command)
    runner = CliRunner()
...
def test_config_list_invokes_handler(monkeypatch):
    ...
    monkeypatch.setattr("doxa_research.config_cmd.config_command", fake_config_command)
    runner = CliRunner()
...
def test_modes_list_invokes_handler(monkeypatch):
    """PR2 (Q5-A row 5): bare `doxa modes` exits 2 with help; `doxa modes
    ...
    monkeypatch.setattr("doxa_research.modes_cmd.modes_command", fake_modes_command)
    runner = CliRunner()
...
def test_help_auth_topic_removed():
    """P16 PR2 T8 / Q5-A row 13.ii: `doxa help auth` virtual topic is dropped.
...
def test_help_has_two_sections():
    """P16-TS09: doxa --help has 'Run research' and 'Manage doxa_research' sections."""
    from doxa_research.cli import cli
    ...
    assert "Manage doxa_research" in result.output
...
def test_help_run_section_contains_research_verbs():
    ...
    # "Run research:" / "Manage doxa-research:" — we want the rendered section.
    out = result.output
    run_idx = out.rfind("Run research")
    manage_idx = out.rfind("Manage doxa_research")
...
def test_help_manage_section_contains_admin_verbs():
    ...
    result = runner.invoke(cli, ["--help"])
    out = result.output
    # rfind: docstring also mentions "Manage doxa-research:" — want the rendered section.
    manage_idx = out.rfind("Manage doxa_research")
