...
The full run_research call is exercised by the doxa_test integration suite.
"""
...
def test_no_prefix_returns_prompt_unchanged(isolated_doxa_home: Path) -> None:
    cm = _cm(isolated_doxa_home, 'version = "2.0"\n')
    result = assemble_prompt_with_prefix(cm, "default", "hello world")
    assert result == "hello world"


def test_general_prefix_prepended_with_blank_line(isolated_doxa_home: Path) -> None:
    cm = _cm(
        isolated_doxa_home,
...
def test_active_profile_prefix_replaces_general(isolated_doxa_home: Path) -> None:
    cm = _cm(
        isolated_doxa_home,
        """version = "2.0"
[general]
...
def test_profile_mode_specific_prefix_wins(isolated_doxa_home: Path) -> None:
    cm = _cm(
        isolated_doxa_home,
        """version = "2.0"
[profiles.deep]
...
def test_empty_user_prompt_still_assembles(isolated_doxa_home: Path) -> None:
    cm = _cm(
        isolated_doxa_home,
...
def test_user_prompt_preserved_verbatim(isolated_doxa_home: Path, user_prompt: str) -> None:
    cm = _cm(
        isolated_doxa_home,
        'version = "2.0"\n[general]\nprompt_prefix = "PREFIX"\n',
    )
