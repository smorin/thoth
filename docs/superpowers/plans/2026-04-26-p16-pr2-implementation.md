...
**Create:**
- `src/doxa_research/cli_subcommands/_options.py` — `_research_options` decorator (~30 LOC; Task 1)
- `src/doxa_research/cli_subcommands/ask.py` — `ask` Click subcommand wrapping `_run_research_default` (~50 LOC; Task 2)
- `src/doxa_research/cli_subcommands/resume.py` — `resume` Click subcommand wrapping `resume_operation` (~40 LOC; Task 3)
...
**Modify (production):**
- `src/doxa_research/cli.py` — apply `_research_options` decorator to the cli group (Task 1); remove `--resume`/`-R` global option declaration + the resume_id keys in `_extract_fallback_options` and `_version_conflicts` (Task 5); remove the `if opts.get("resume_id"):` block in `_dispatch_click_fallback` (Task 5); register `ask` (Task 2) and `resume` (Task 3) subcommands; small touches in Tasks 6–10
- `src/doxa_research/help.py` — Task 5 adds `--resume`/`-R` early-intercept gating to `DoxaGroup.parse_args`; Task 8 removes the `--help <topic>` auth hijack from the SAME method; Task 8 also removes `"completion"` from `ADMIN_COMMANDS`; Task 4 updates the `doxa-research --resume op_abc123` example string at line 134
- `src/doxa_research/cli_subcommands/providers.py` — Task 6 removes the in-group flag shim (lines 39–62, 109–173) and adds Q6-C1 `ctx.fail` gating for every legacy flag form; adds `--refresh-cache` and `--no-cache` options to `providers_models_cmd` with mutex per Q5-A row 1
- `src/doxa_research/cli_subcommands/modes.py` — Task 7 removes `ModesGroup` class entirely + the bare-`modes` shortcut + every hidden `--json/--show-secrets/--full/--name/--source` legacy command; adds Q6-C1 `ctx.fail` gating for those 5 flags; adds Q5-A row 5 (no-leaf exit 2 — natural Click default once `invoke_without_command=False`)
- `src/doxa_research/modes_cmd.py` — Task 7 removes the early-return at line 261 so `--name` and `--source` intersect (Q5-A row 11.i)
- `src/doxa_research/cli_subcommands/config.py` — Task 9 promotes `get` to a typed Click command with `--show-secrets` and `--raw` (kept passthrough on `set/unset/list/path/edit/help` to preserve PR1.5 behavior); Task 10 promotes `--layer` to `click.Choice` per Q5-A row 8
- `src/doxa_research/config_cmd.py` — Task 9 changes `_op_get` line 104 from `if _is_secret_key(key) and not show_secrets and not raw:` to `if _is_secret_key(key) and not show_secrets:` (Q4-D split). The `elif raw:` data-source-merge branch at lines 90–95 is PRESERVED unchanged — Q4-D only addresses masking
- `src/doxa_research/cli_subcommands/help_cmd.py` — Task 8 removes the `topic == "auth"` virtual-topic branch (lines 25–28); the `auth` topic now only works through `doxa-research help auth` if a real `auth` subcommand were added — for PR2, drop the parse-time hijack and the help-leaf shortcut. The literal `auth` string in the "Available topics" listing at line 36 is dropped too
- `src/doxa_research/cli_subcommands/status.py` — Task 10 changes `ctx.exit(1)` to `ctx.exit(2)` per Q5-A row 6
- `src/doxa_research/run.py` — Task 3 widens `resume_operation` signature with `quiet`, `no_metadata`, `timeout_override`, `cli_api_keys` params (Q1-PR2-C honor-set); Task 4 updates 4 emitter strings at lines 629, 654, 827, 854
- `src/doxa_research/signals.py` — Task 4 updates 2 emitter strings at lines 93, 99
- `src/doxa_research/commands.py` — Task 4 updates 2 hint emitters at lines 227, 238; Task 6 (with mutex) — note the `--refresh-cache + --no-cache` precedence is moved up to the leaf via Click mutex, so commands.py change in Task 6 is just keeping behavior consistent
- `src/doxa_research/providers/openai.py` — Task 4 updates the legacy hint string at line 69 from `'doxa-research providers -- --models --provider openai'` → `'doxa-research providers models --provider openai'`
- `src/doxa_research/cli.py` — Task 7 also removes `--clarify`-without-`--interactive` silent no-op per Q5-A row 7 (raises `BadParameter("--clarify requires --interactive")`)

**Modify (tests):**
- `tests/_fixture_helpers.py:65` — Task 4 updates `extract_resume_id` regex from `r"doxa-research --resume\s+(...)"` → `r"doxa-research resume\s+(...)"`
...
- `tests/test_cli_help.py:26` — Task 4 updates assertion `"doxa-research --resume" in out` → `"doxa-research resume" in out`
- `tests/test_progress_spinner.py:152` — Task 4 updates `"Resume later: doxa-research --resume op_abc123"` → `"Resume later: doxa-research resume op_abc123"`
- `tests/test_providers_subcommand.py:23-27` — Task 6 flips `test_old_form_deprecated_but_works` to assert `r.exit_code == 2` and `"(use 'doxa-research providers list')"` in stderr
...
- `doxa_test` lines 2170, 2216, 2238 — Task 4 updates `r"...doxa-research --resume"` patterns
- `doxa_test` lines 2260, 2269, 2290–2297, 2307 — Task 6 updates `providers -- --models...` argv to `providers models...`
- `README.md` line 218 — Task 4 updates `doxa-research --resume research-…` example
- `README.md` lines 98, 224, 227, 230, 233, 234, 565, 572, 573, 574, 575 — Task 11 updates remaining `doxa-research providers --` references
...
- `help.py` — `auth` hijack in `DoxaGroup.parse_args` lines 51–54 (Task 8)
...
1. **`LEFTHOOK=0` discipline.** Tasks 4–9 produce transitional `./doxa_test` states (the integration suite asserts on `doxa-research --resume` literal text and `providers -- --list` argv). For each of those tasks, the per-task gate documents exactly which manual checks (`just check`, file-targeted pytest) MUST pass before invoking `LEFTHOOK=0 git commit -m '...'`. **Task 12 (final commit) MUST go through the full hook set without bypass** per CLAUDE.md "Hook discipline".
2. **Click 8.x `protected_args` deprecation.** `cli.py::_click_remainder_args` (lines 74–81) and `help.py::DoxaGroup.invoke` (lines 71–77) already wrap `protected_args` reads in narrow `warnings.catch_warnings()` suppression. Task 5's `--resume` early-intercept does NOT touch `ctx.protected_args` — it scans the raw `args` list parameter to `parse_args(ctx, args)` BEFORE delegating to `super().parse_args(...)`. No new `protected_args` usage is introduced in PR2.
3. **`DoxaGroup.parse_args` is the only custom dispatch override added/modified in PR2.** Tasks 5 and 8 both touch this method. Task 5 adds the `--resume`/`-R` early-intercept; Task 8 removes the `--help auth` hijack. The end state is shown explicitly in Task 8's Step 8.4 code block; verify the file contents match before committing Task 8.
...
7. **Test category bundling per spec §9.2.** Each task lists which test categories (A through H) get added in that commit. Final PR2 gate (post-Task 12) MUST show pytest count ≥ baseline + ~95 PR2-new = ~407 and `./doxa_test -r` 63+ passing.
...
**Files:**
- Create: `src/doxa_research/cli_subcommands/_options.py`
- Modify: `src/doxa_research/cli.py` — replace the 15 inline `@click.option(...)` decorators on the `@click.group(...)` block (lines 467–528) with `@_research_options`
...
the historical pre-PR2 order on `cli` so that `doxa-research --help` output is
    ...
    (
        ("--input-file",),
        {
            "help": (
                ...
                "non-doxa-research document, an older run, or a file from a different project."
            ),
        },
    ),
    (
...
from doxa-research.cli_subcommands._options import _research_options
...
In `src/doxa_research/cli.py`, locate the `@click.group(cls=DoxaGroup, …)` block at line 467 and the 15 `@click.option(...)` decorators at lines 473–528 (note: `--version` / `-V` at line 502 is NOT a research option — keep it inline). Replace the 15 research options with one `@_research_options` call.
...
def cli(
    ...
    Quick usage: doxa-research "PROMPT"
...
```bash
uv run pytest tests/test_p16_pr2_options_decorator.py tests/test_p16_dispatch_parity.py tests/test_p16_doxagroup.py tests/test_cli_help.py -v
```
...
```bash
just check
git add src/doxa_research/cli_subcommands/_options.py src/doxa_research/cli.py tests/test_p16_pr2_options_decorator.py
...
**Files:**
- Create: `src/doxa_research/cli_subcommands/ask.py`
- Create: `tests/test_p16_pr2_ask.py`
- Modify: `src/doxa_research/cli.py` — add `cli.add_command(_ask_mod.ask)` registration
...
def _stub_run_research(monkeypatch):
    ...
    monkeypatch.setattr("doxa-research.run.run_research", fake)
    return captured
...
def test_ask_via_group_level_flags(monkeypatch):
    """Q3-PR2-C: `doxa-research --mode X ask "..."` works (group form)."""
    captured = _stub_run_research(monkeypatch)
...
```python
"""`doxa-research ask "PROMPT"` Click subcommand.
...
so both `doxa-research ask --mode X "..."` and `doxa-research --mode X ask "..."` work.
"""
...
def ask(
    ...
    from doxa-research.cli import _read_prompt_input, _run_research_default, _apply_config_path
    from doxa-research.cli import _prompt_max_bytes
...
In `src/doxa_research/cli.py`, after the existing `cli.add_command(_init_mod.init)` line (line 634), add the `ask` registration. Since `RUN_COMMANDS` order is `("ask", "resume", "status", "list")` per `help.py:14`, register `ask` first so the help renderer's two-section output places it at the top of the Run-research section:

```python
from doxa-research.cli_subcommands import ask as _ask_mod  # noqa: E402

cli.add_command(_ask_mod.ask)
```

Place this import block BEFORE `from doxa-research.cli_subcommands import init as _init_mod`.
...
```bash
uv run pytest tests/test_p16_dispatch_parity.py tests/test_p16_doxagroup.py tests/test_cli_help.py tests/test_cli_regressions.py -v
just check
```
...
```bash
git add src/doxa_research/cli_subcommands/ask.py src/doxa_research/cli.py tests/test_p16_pr2_ask.py
...
**Files:**
- Create: `src/doxa_research/cli_subcommands/resume.py`
...
- Modify: `src/doxa_research/run.py` — widen `resume_operation` signature
- Modify: `src/doxa_research/cli.py` — register `resume` subcommand
...
Expected: only `src/doxa_research/cli.py:351` (the OLD `--resume` flow), `src/doxa_research/run.py` (the def itself), and any test stubs. No other callers — confirms Task 3 + Task 5 sequencing is safe.
...
def _stub_resume(monkeypatch):
    ...
    monkeypatch.setattr("doxa-research.run.resume_operation", fake)
    return captured
...
def test_resume_honors_config_path(monkeypatch, tmp_path):
    captured = _stub_resume(monkeypatch)
    cfg = tmp_path / "doxa-research.toml"
...
In `src/doxa_research/run.py`, modify the `resume_operation` definition starting at line 703. Add keyword-only defaulted parameters per Q1-PR2-C:
...
```python
"""`doxa-research resume OP_ID` Click subcommand.
...
def resume(
    ...
    from doxa-research.cli import _apply_config_path, _build_app_context, _run_maybe_async
...
In `src/doxa_research/cli.py`, after the `ask` registration added in Task 2:

```python
from doxa-research.cli_subcommands import resume as _resume_mod  # noqa: E402
...
```bash
git add src/doxa_research/cli_subcommands/resume.py src/doxa_research/cli.py src/doxa_research/run.py tests/test_p16_pr2_resume.py tests/test_cli_regressions.py
...
## Task 4: Update `--resume` emitter strings to `doxa-research resume`

**Why fourth:** Once both subcommands exist (Tasks 2 + 3), every help/hint string referencing `doxa-research --resume` should print the new canonical form. Doing this BEFORE removing the legacy flag (Task 5) means the new emitter strings are live but still copy-paste compatible with the legacy flag — users who hit the new hint mid-Task-4 still get a working command. Task 5 then enforces.

**Files:**
- Modify: `src/doxa_research/run.py` lines 629, 654, 827, 854 (4 emitters)
- Modify: `src/doxa_research/signals.py` lines 93, 99 (2 emitters)
- Modify: `src/doxa_research/commands.py` lines 227, 238 (2 print_hint emitters)
- Modify: `src/doxa_research/help.py` line 134 (epilog example)
- Modify: `src/doxa_research/providers/openai.py` line 69 (legacy hint string)
...
- Modify: `doxa_test` lines 2170, 2216, 2238 (regex patterns)
...
In `src/doxa_research/run.py`, change the four occurrences of `Resume with: [bold]doxa-research --resume {operation.id}[/bold]` to `Resume with: [bold]doxa-research resume {operation.id}[/bold]` at lines 629, 654, 827, 854.

- [ ] **Step 4.2: Update `signals.py` emitters (2 sites)**

In `src/doxa_research/signals.py`:
- Line 93: `f"[green]✓[/green] Checkpoint saved. Resume with: doxa-research --resume {_current_operation.id}"` → `f"[green]✓[/green] Checkpoint saved. Resume with: doxa-research resume {_current_operation.id}"`
- Line 99: `f"\nResume later: doxa-research --resume {_current_operation.id}"` → `f"\nResume later: doxa-research resume {_current_operation.id}"`

- [ ] **Step 4.3: Update `commands.py` emitters (2 sites)**

In `src/doxa_research/commands.py`:
- Line 227: `print_hint(f"doxa-research --resume {op_id}", "Pick up where Ctrl-C left off")` → `print_hint(f"doxa-research resume {op_id}", "Pick up where Ctrl-C left off")`
- Line 238: `print_hint(f"doxa-research --resume {op_id}", "Retry from checkpoint")` → `print_hint(f"doxa-research resume {op_id}", "Retry from checkpoint")`

- [ ] **Step 4.4: Update `help.py` epilog example**

In `src/doxa_research/help.py:134`, change:
```python
formatter.write_text("doxa-research --resume op_abc123")
```
to:
```python
formatter.write_text("doxa-research resume op_abc123")
...
In `src/doxa_research/providers/openai.py:68-69`:
```python
f"Model '{model}' not found. Please check available models with "
f"'doxa-research providers -- --models --provider openai'",
```
to:
```python
f"Model '{model}' not found. Please check available models with "
...
```python
match = re.search(r"doxa-research --resume\s+(research-\d{8}-\d{6}-[a-f0-9]{16})", output)
```
to:
```python
match = re.search(r"doxa-research resume\s+(research-\d{8}-\d{6}-[a-f0-9]{16})", output)
...
```python
assert "doxa-research --resume" in out
```
to:
```python
assert "doxa-research resume" in out
...
```python
assert "Resume later: doxa-research --resume op_abc123" in output
```
to:
```python
assert "Resume later: doxa-research resume op_abc123" in output
...
- [ ] **Step 4.7: Update `doxa_test` patterns (3 sites)**

Read `doxa_test` around lines 2170, 2216, 2238 to confirm exact pattern strings, then update:
- Line ~2170: `r"Checkpoint saved\. Resume with: doxa-research --resume"` → `r"Checkpoint saved\. Resume with: doxa-research resume"`
- Line ~2216: `r"Resume with: .*doxa-research --resume"` → `r"Resume with: .*doxa-research resume"`
...
```bash
DOXA_TEST_MODE=1 uv run python -c "from tests.baselines import capture_baselines as c; print(c.capture('help', ['--help']))" > /tmp/new_help.json
# Confirm only the resume example line changed, then:
mv /tmp/new_help.json tests/baselines/help.json
```

(If `tests/baselines/capture_baselines.py` was deleted in PR1's Task 15, recapture by running the live binary: `doxa-research --help > /tmp/help.txt`, then update the JSON's `stdout` field manually with the new content.)

- [ ] **Step 4.10: Run doxa_test for the affected cases**

```bash
./doxa_test -r --provider mock --skip-interactive -q -t TR-02
./doxa_test -r --provider mock --skip-interactive -q -t TR-03
./doxa_test -r --provider mock --skip-interactive -q -t TS-09
```
...
```bash
git add src/doxa_research/run.py src/doxa_research/signals.py src/doxa_research/commands.py src/doxa_research/help.py src/doxa_research/providers/openai.py tests/_fixture_helpers.py tests/test_cli_help.py tests/test_progress_spinner.py tests/baselines/help.json doxa_test README.md
git commit -m "refactor(cli): update --resume emitter strings to doxa-research resume (P16 PR2)"
```
...
`(use 'doxa-research NEW_FORM')` migration substring on stderr per Q6-PR2-C1.
"""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["--resume", "op_x"], "doxa-research resume"),
        (["-R", "op_x"], "doxa-research resume"),
    ],
)
...
- [ ] **Step 5.2: Add `--resume` early-intercept to `DoxaGroup.parse_args`**

In `src/doxa_research/help.py`, modify `DoxaGroup.parse_args` (lines 51–55):
...
def parse_args(self, ctx: click.Context, args: list[str]):
    ...
    for token in args:
        if token in ("--resume", "-R") or token.startswith("--resume="):
            ctx.fail(
                "no such option: --resume (use 'doxa-research resume OP_ID')"
            )
...
Expected: PASS — these were already updated in Task 4 to expect the new `doxa-research resume` text in emitter output, and the new subcommand is now the path that produces that text.

- [ ] **Step 5.9: Commit**

```bash
git add src/doxa_research/help.py src/doxa_research/cli.py tests/test_p16_pr2_gating.py tests/test_p16_pr2_resume.py tests/test_resume.py tests/test_pick_model.py tests/test_cli_regressions.py
git commit -m "refactor(cli): remove --resume global flag, gate legacy form (P16 PR2)"
```

`LEFTHOOK=0` is **NOT** needed if the per-task gate (Step 5.7 + 5.8) is GREEN. If the pre-commit hook fails on a doxa_test case not in the targeted list, run the full quiet suite first to identify it: `./doxa_test -r --provider mock --skip-interactive -q`.
...
- Modify: `doxa_test` lines 2260, 2269, 2290–2297, 2307 — migrate argv
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["providers", "--", "--list"], "doxa-research providers list"),
        (["providers", "--", "--models"], "doxa-research providers models"),
        (["providers", "--", "--keys"], "doxa-research providers check"),
        (["providers", "--", "--refresh-cache"], "doxa-research providers models --refresh-cache"),
        (["providers", "--", "--no-cache"], "doxa-research providers models --no-cache"),
        # providers in-group hidden flag form (PR1.5 shim)
        (["providers", "--list"], "doxa-research providers list"),
        (["providers", "--models"], "doxa-research providers models"),
        (["providers", "--keys"], "doxa-research providers check"),
        (["providers", "--check"], "doxa-research providers check"),
    ],
)
...
Replace the body of `src/doxa_research/cli_subcommands/providers.py` with:
...
_LEGACY_FLAG_TO_NEW_FORM: dict[str, str] = {
    "--list": "doxa-research providers list",
    "--models": "doxa-research providers models",
    "--keys": "doxa-research providers check",
    "--check": "doxa-research providers check",
    "--refresh-cache": "doxa-research providers models --refresh-cache",
    "--no-cache": "doxa-research providers models --no-cache",
}
...
def providers(ctx: click.Context) -> None:
    ...
    # Q5-A row 4: bare `doxa-research providers` exits 2 (Click default for required subgroup).
...
```bash
grep -n "def providers_models" /Users/stevemorin/c/doxa-research/src/doxa_research/commands.py
```
...
def test_old_form_gated_with_migration_hint():
    ...
    assert r.exit_code == 2
    combined = (r.output or "") + (r.stderr or "")
    assert "doxa-research providers list" in combined
```
...
- [ ] **Step 6.5: Migrate doxa_test cases T-PROV-07/08/10 + P07-M2-01 (Category H)**
...
Read the relevant `doxa_test` lines first to capture the exact patterns before editing.
...
just check
./doxa_test -r --provider mock --skip-interactive -q -t T-PROV
./doxa_test -r --provider mock --skip-interactive -q -t P07-M2-01
```

Expected: ALL PASS. Note: `T-PROV-06` asserts the bare `doxa-research providers` help output mentions `--list|--models|list|models` — verify that pattern still matches the new help epilog (the `epilog` field can be removed from the `@click.group(name="providers", ...)` decoration; if the `T-PROV-06` regex needs adjustment, update it to match what the new help renders).

- [ ] **Step 6.7: Commit**

```bash
git add src/doxa_research/cli_subcommands/providers.py tests/test_providers_subcommand.py tests/test_p16_pr2_gating.py tests/test_p16_pr2_cleanup.py doxa_test
...
**Files:**
- Modify: `src/doxa_research/cli_subcommands/modes.py` — remove `ModesGroup` class entirely; remove the `if ctx.invoked_subcommand is None` shortcut; remove the five hidden `modes_legacy_*` commands; add Q6-C1 gating in the group callback for the five flags
- Modify: `src/doxa_research/modes_cmd.py:243-261` — remove the `return 0` after the detail render so `--source` filter is honored when `--name` matches; pass both `name` and `source` to the filter logic
- Modify: `src/doxa_research/cli.py` — add `--clarify` requires `--interactive` rejection in the cli group callback (Q5-A row 7)
...
@pytest.mark.parametrize(
    "argv,migration_hint",
    [
        (["modes", "--json"], "doxa-research modes list --json"),
        (["modes", "--show-secrets"], "doxa-research modes list --show-secrets"),
        (["modes", "--full"], "doxa-research modes list --full"),
        (["modes", "--name", "deep_research"], "doxa-research modes list --name"),
        (["modes", "--source", "user"], "doxa-research modes list --source"),
    ],
)
...
```python
def test_modes_no_leaf_exits_2(isolated_doxa_home):
    """Q5-A row 5: bare `doxa-research modes` exits 2 (no leaf default)."""
    r = CliRunner().invoke(cli, ["modes"])
    assert r.exit_code == 2, r.output


def test_modes_list_name_and_source_intersect(isolated_doxa_home):
...
The `isolated_doxa_home` fixture comes from the existing `conftest.py`.
...
Replace the body of `src/doxa_research/cli_subcommands/modes.py` with:

```python
"""`doxa-research modes` Click subgroup. PR2 ships `list` only.
...
flag is gated to its `doxa-research modes list <flag>` canonical via Q6-C1.
...
_LEGACY_FLAG_TO_NEW_FORM: dict[str, str] = {
    "--json": "doxa-research modes list --json",
    "--show-secrets": "doxa-research modes list --show-secrets",
    "--full": "doxa-research modes list --full",
    "--name": "doxa-research modes list --name",
    "--source": "doxa-research modes list --source",
}
...
def modes(ctx: click.Context) -> None:
    ...
    # Q5-A row 5: bare `doxa-research modes` (or unknown-arg) exits 2.
    if args:
...
In `src/doxa_research/modes_cmd.py`, modify `_op_list` starting at line 234. The current behavior (lines 243–261) is:
...
In `src/doxa_research/cli.py`, in the `def cli(...)` callback body, after the existing `--input-file`/`--auto` mutex check at lines 615–616, add:
...
```bash
uv run pytest tests/test_p16_pr2_gating.py tests/test_p16_pr2_cleanup.py tests/test_p16_doxagroup.py tests/test_cli_regressions.py -v
just check
```

Expected: PASS. Note: `tests/test_p16_doxagroup.py:223` previously asserted bare-`modes` lists modes — that assertion will need updating to expect exit 2 + help text. If the test exists in that form, update it in this same commit.

- [ ] **Step 7.6: Run doxa_test for the modes case**

```bash
./doxa_test -r --provider mock --skip-interactive -q -t M
```

Expected: PASS. The `M8T-…` and modes-related cases use `modes list` directly (verified via `grep`); the `doxa-research modes bogus_op` test at doxa_test:2128 will see Click's default `"No such command 'bogus_op'"` error instead of `"unknown modes op"` — update the pattern in `doxa_test` to `r"No such command|unknown.*op"` (regex alternation accepts both).

- [ ] **Step 7.7: Commit**

```bash
git add src/doxa_research/cli_subcommands/modes.py src/doxa_research/modes_cmd.py src/doxa_research/cli.py tests/test_p16_pr2_gating.py tests/test_p16_pr2_cleanup.py tests/test_p16_doxagroup.py doxa_test
git commit -m "refactor(cli/modes): remove flag-style shim + gate (P16 PR2)"
```

`LEFTHOOK=0` is **NOT** needed if Step 7.5 + 7.6 are GREEN. If pre-commit hits an unrelated doxa_test failure, diagnose with `./doxa_test -r --last-failed -q` first.

---

## Task 8: Drop `--help <topic>` parse-time hijack (Q5-A row 13.ii) + remove `completion` from `ADMIN_COMMANDS`

**Why eighth:** Tasks 5–7 added gating for runtime forms. The remaining shim is the parse-time `--help auth` hijack in `DoxaGroup.parse_args` (lines 51–54 in PR1.5; Task 5 already added the `--resume` early-intercept above it). Drop it now so `parse_args` ends up with ONLY the `--resume` early-intercept.

**Files:**
- Modify: `src/doxa_research/help.py` — remove the `--help auth` hijack from `DoxaGroup.parse_args`; remove `"completion"` from `ADMIN_COMMANDS`
- Modify: `src/doxa_research/cli_subcommands/help_cmd.py` — remove the `topic == "auth"` virtual-topic branch; remove `, auth` from the "Available topics" listing
...
def test_help_auth_parse_time_hijack_removed():
    """Q5-A row 13.ii: `doxa-research --help auth` is no longer hijacked at parse time."""
...
def test_help_subcommand_topic_still_works():
    """`doxa-research help status` still forwards to `doxa-research status --help`."""
...
def test_help_auth_topic_via_help_subcommand_removed():
    """The `auth` virtual topic on `doxa-research help auth` is also dropped per Q5-A row 13.ii."""
    r = CliRunner(mix_stderr=False).invoke(cli, ["help", "auth"])
    assert r.exit_code == 2, r.output
    combined = (r.output or "") + (r.stderr or "")
...
In `src/doxa_research/help.py`, modify `DoxaGroup.parse_args` to ONLY contain the `--resume` early-intercept added by Task 5. Final state:
...
def parse_args(self, ctx: click.Context, args: list[str]):
    ...
    for token in args:
        if token in ("--resume", "-R") or token.startswith("--resume="):
            ctx.fail("no such option: --resume (use 'doxa-research resume OP_ID')")
    return super().parse_args(ctx, args)
```
...
In `src/doxa_research/help.py:15-22`:
...
In `src/doxa_research/cli_subcommands/help_cmd.py`, remove lines 25–28:
...
def test_bug_01_auth_help_interception_is_root_only() -> None:
    ...
    assert "Initialize doxa-research configuration" in result.output
```
...
```bash
uv run pytest tests/test_p16_pr2_gating.py tests/test_p16_doxagroup.py tests/test_cli_help.py tests/test_cli_regressions.py tests/test_p16_dispatch_parity.py -v
just check
```

Expected: ALL PASS. The `help_auth.json` baseline in `tests/baselines/` (captured in PR1's Task 1) may need recapture if its content changed — verify and recapture if needed (`doxa-research help auth` now exits 2; the baseline previously expected exit 0 with the auth-help text).

- [ ] **Step 8.7: Commit**

```bash
git add src/doxa_research/help.py src/doxa_research/cli_subcommands/help_cmd.py tests/test_p16_pr2_gating.py tests/test_cli_regressions.py tests/baselines/help_auth.json
...
**Files:**
- Modify: `src/doxa_research/cli_subcommands/config.py` — promote `get` from passthrough to a typed Click command with `--show-secrets`, `--raw`, `--json`, `--layer` options. (Other leaves stay passthrough; that's a PR3 concern.)
- Modify: `src/doxa_research/config_cmd.py:104` — change masking predicate to `not show_secrets` only
...
In `src/doxa_research/cli_subcommands/config.py`, replace the passthrough `config_get`:
...
In `src/doxa_research/config_cmd.py:104`:
...
```bash
git add src/doxa_research/cli_subcommands/config.py src/doxa_research/config_cmd.py tests/test_p16_pr2_config_secrets.py
...
- Row 4: bare `doxa-research providers` no leaf → exit 2 (already in Task 6)
- Row 6: `doxa-research status` no arg → exit 2 (currently exit 1; baseline recapture needed)
...
**Files:**
- Modify: `src/doxa_research/cli_subcommands/status.py:18` — change `ctx.exit(1)` to `ctx.exit(2)`
...
def test_status_no_arg_exits_2():
    """Q5-A row 6: bare `doxa-research status` exits 2 (Click default for missing required arg)."""
    r = CliRunner().invoke(cli, ["status"])
    assert r.exit_code == 2, r.output


def test_config_get_invalid_layer_exits_2(isolated_doxa_home):
...
def test_providers_no_leaf_exits_2():
    """Q5-A row 4: bare `doxa-research providers` exits 2 (no-leaf default)."""
...
In `src/doxa_research/cli_subcommands/status.py:14-18`:
...
```bash
DOXA_TEST_MODE=1 doxa-research status > /tmp/status_no_args.stdout 2> /tmp/status_no_args.stderr; echo "EXIT=$?"
```
...
```bash
grep -n "status_no_args\|status.*exit" /Users/stevemorin/c/doxa-research/tests/test_p16_dispatch_parity.py
```
...
```bash
git add src/doxa_research/cli_subcommands/status.py tests/baselines/status_no_args.json tests/test_p16_pr2_cleanup.py tests/test_p16_dispatch_parity.py
...
- Modify: `README.md` — sweep all remaining `doxa-research providers --` and `doxa-research --resume` references not already updated (lines 98, 224, 227, 230, 233, 234, 565, 572, 573, 574, 575)
...
- `doxa-research config get KEY --raw` no longer bypasses secret masking. To reveal a secret value, use `--show-secrets` (with or without `--raw`). `--raw` now controls only output formatting.
...
In `README.md`, update each remaining occurrence of `doxa-research --resume` and `doxa-research providers --` to the new form. Cited locations from the audit + Step 11 prep grep:

- Line 98: `doxa-research providers -- --list` → `doxa-research providers list`
- Line 224: `doxa-research providers -- --list` → `doxa-research providers list`
- Line 227: `doxa-research providers -- --keys` → `doxa-research providers check`
- Line 230: `doxa-research providers -- --models` → `doxa-research providers models`
- Line 233: `doxa-research providers -- --models --provider openai` → `doxa-research providers models --provider openai`
- Line 234: `doxa-research providers -- --models -P perplexity` → `doxa-research providers models -P perplexity`
- Line 565 (table row): `doxa-research providers -- --list` → `doxa-research providers list`
- Line 572 (table row): `doxa-research providers -- --list` → `doxa-research providers list`
- Line 573 (table row): `doxa-research providers -- --models` → `doxa-research providers models`
- Line 574 (table row): `doxa-research providers -- --keys` → `doxa-research providers check`
- Line 575 (table row): `doxa-research providers -- --models -P openai` → `doxa-research providers models -P openai`
...
```bash
git grep "doxa-research --resume" -- 'src/' 'tests/' 'README.md' ':!CHANGELOG.md' ':!docs/superpowers/'
git grep "doxa-research providers --" -- 'src/' 'tests/' 'README.md' ':!CHANGELOG.md' ':!docs/superpowers/'
git grep "doxa-research modes --" -- 'src/' 'tests/' 'README.md' ':!CHANGELOG.md' ':!docs/superpowers/'
```
...
Expected: ALL GREEN. Counts: pytest ≥ 407, doxa_test ≥ 63.
...
**doxa_test count:** XX passing (PR1.5 baseline preserved)
...
- File path `src/doxa_research/cli_subcommands/_options.py` — used identically in Tasks 1, 2 imports. ✓
...
- **`doxa_test:2128` `unknown modes op` pattern** — Task 7 Step 7.6 calls out that this pattern needs widening to `r"No such command|unknown.*op"` because Click's default error replaces the custom wording.
- **`tests/test_p16_doxagroup.py:223`** — Per the audit, this asserts bare-`modes` lists modes. Task 7 Step 7.5 calls this out and includes the test in the commit's `git add`.
...
- **Pre-commit `./doxa_test` failures during transitional commits** — Per the per-task gates in Tasks 4–10, run targeted `./doxa_test -t <id>` cases before each commit. If the full quiet suite reveals an unrelated transitional failure that's expected to clear in a later task, document the failure ID in the commit message body and proceed; otherwise diagnose. **Task 12 MUST pass the full hook set.**
- **`isolated_doxa_home` fixture** — Used in several tests across `tests/test_p16_pr2_*.py`. The fixture is defined in `tests/conftest.py`; verify availability before relying on it. If it's not exported globally, import it explicitly or move tests to a directory that picks up the conftest.
