...
**Tech stack:** Python 3.11+, Click 8.x, pytest, `respx`/`vcr` cassettes, existing `isolated_doxa_home` test fixture, `./doxa_test` integration runner.
...
- P11 (`doxa-research modes` discovery — established the rendering vocabulary; v2.11.0)
...
- P16 PR1+PR2+PR3 (Click-native CLI + `doxa-research ask`/`resume` + `--json` everywhere + `completion/sources.py:79 mode_kind` dead code; landed in `f8b62f2`, awaiting v3.0.0 release tag)
...
| `cli.py:284` | `_doxa_config.is_background_model(model_name)` | `mode_kind(mode_cfg) == "background"` | C |
...
| G | `feat(providers): add provider.cancel() and 'doxa-research cancel' subcommand` |
...
- [ ] `./doxa_test -r --skip-interactive -q` passes (integration)
...
- Commit `f8b62f2` shipped P16 PR2 + PR3 to `main`: `cli_subcommands/` directory, `_research_options` shared decorator, `doxa-research ask`/`doxa-research resume` subcommands, `--json` everywhere, `completion/sources.py`, `progress.py:should_show_spinner`. `feat!` queues v3.0.0 from release-please.
...
- `doxa-research ask --json` already implements **Option E** (background → submit envelope; immediate → snapshot from checkpoint) at `ask.py:158-231`. The kind-aware split exists for the JSON path; P18 brings the same split to the human-readable path *and* adds streaming.
...
4. **Phase D grows slightly**: wire `--kind` filter on `doxa-research modes` using the dead-code `mode_kind` completer in `completion/sources.py:79`.
5. **Phase E placement**: `--out`/`--append` flags go in **`cli_subcommands/_options.py:_RESEARCH_OPTIONS`**, not `cli.py`. Both top-level CLI and `doxa-research ask` inherit them via the existing decorator stack.
