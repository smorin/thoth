# doxa_test Claude-Code Ergonomics Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add machine-readable output, precise selectors, and a self-rerun loop to `doxa_test` so Claude Code and hooks can consume it without shell-scraping.

**Architecture:** Single-file additions to the existing `doxa_test` uv-shebang script. Introduce a pure-Python `resolve_selection()` helper and JSON cache I/O. Add `pytest` coverage via `tests/test_doxa_test_cli.py` — unit tests import `doxa_test` as a module; integration tests subprocess-invoke `./doxa_test`.

**Tech Stack:** Python 3.11+, Click (already a dep), Rich (already a dep), pytest, uv shebang.

**Spec:** `docs/superpowers/specs/2026-04-24-doxa-research-test-claude-code-ergonomics-design.md`
...
**Modify:**
- `doxa_test` — add flags, resolver, cache I/O, list printers, fenced-failure printer, `-q` branch.
- `.gitignore` — add `.doxa_test_cache/` and `test_outputs/`.
- `CLAUDE.md` — replace workaround guidance with new flags (Task 8).

**Create:**
- `tests/test_doxa_test_cli.py` — pytest coverage for the new CLI surface.

No new production modules. `doxa_test` stays a single file.

### Module-level additions to `doxa_test`

Near existing constants (line ~60):
```python
CACHE_DIR = Path(".doxa_test_cache")
...
`doxa_test` is currently a shebang script with no `.py` extension. Pytest tests import it via a helper:
```python
import importlib.util, pathlib
spec = importlib.util.spec_from_loader(
    "doxa_test_mod",
    importlib.machinery.SourceFileLoader("doxa_test_mod", str(pathlib.Path(__file__).resolve().parent.parent / "doxa_test"))
)
doxa_test_mod = importlib.util.module_from_spec(spec)
spec.loader.exec_module(doxa_test_mod)
```
...
Run: `git ls-files | grep -E '(doxa_test_cache|test_outputs)' || echo "clean"`
Expected: `clean`
...
```bash
git add .gitignore
git commit -m "chore: gitignore .doxa_test_cache/ and test_outputs/"
```
...
**Files:**
- Modify: `doxa_test` (new function near line ~460, call site in `main()` near line ~2759)
- Test: `tests/test_doxa_test_cli.py` (new file)

- [ ] **Step 1: Write the failing test**

Create `tests/test_doxa_test_cli.py`:
...
Run: `uv run pytest tests/test_doxa_test_cli.py -v`
Expected: FAIL with `AttributeError: module 'doxa_test_mod' has no attribute 'resolve_selection'`.

- [ ] **Step 3: Add the helper**

Add to `doxa_test`, immediately before the `# Test Runner` section banner (around line ~703):
...
In `doxa_test`, replace the filter block inside `main()` (lines ~2759 to the line immediately before `if verbose:`, i.e., just after "Show breakdown" block) with:
...
Run: `./doxa_test -r --provider mock --skip-interactive`
Expected: same PASS/FAIL counts as before the refactor; exit 0.

- [ ] **Step 7: Commit**

```bash
git add doxa_test tests/test_doxa_test_cli.py
git commit -m "refactor(doxa_test): extract resolve_selection() helper"
```
...
**Files:**
- Modify: `doxa_test` (add list printers near line ~700; add CLI options; early-exit in `main()`)
- Test: `tests/test_doxa_test_cli.py`

- [ ] **Step 1: Write failing tests**

Append to `tests/test_doxa_test_cli.py`:
...
def _run_doxa_test(*args: str) -> subprocess.CompletedProcess:
...
Run: `uv run pytest tests/test_doxa_test_cli.py -k "list" -v`
Expected: FAIL — "no such option: --list" or similar.

- [ ] **Step 3: Add list printers**

Add to `doxa_test` near line ~700 (before the Selection Resolver banner):
...
In `doxa_test`, inside the `@click.command()` decorator chain above `def main(...)`, add:
...
```bash
git add doxa_test tests/test_doxa_test_cli.py
git commit -m "feat(doxa_test): add --list and --list-json for test discovery"
```
...
**Files:**
- Modify: `doxa_test` — extend `resolve_selection` to honour `ids`.
- Test: `tests/test_doxa_test_cli.py`
...
Run: `uv run pytest tests/test_doxa_test_cli.py -k "id_" -v`
Expected: FAIL — `ids` not honoured; `_requested_but_filtered` attr missing.

- [ ] **Step 3: Extend `resolve_selection`**

Replace the body of `resolve_selection` in `doxa_test` with:
...
def test_id_integration_runs_exactly_one_test():
    proc = _run_doxa_test("-r", "--id", "M1T-01", "-q")
    assert proc.returncode == 0, proc.stdout + proc.stderr
    assert "1 passed" in proc.stdout
```

Run: `uv run pytest tests/test_doxa_test_cli.py::test_id_integration_runs_exactly_one_test -v`
...
```bash
git add doxa_test tests/test_doxa_test_cli.py
git commit -m "feat(doxa_test): add --id for exact-match test selection"
```
...
**Goal:** Every `-r` invocation writes `.doxa_test_cache/last_run.json` atomically, containing the full v1 schema (stdout/stderr full for failures, null for passes).

**Files:**
- Modify: `doxa_test` — add serializer + atomic write; call from `generate_report`.
- Test: `tests/test_doxa_test_cli.py`
...
def test_cache_written_on_every_run(tmp_path, monkeypatch):
    ...
    cache_file = REPO_ROOT / ".doxa_test_cache" / "last_run.json"
    if cache_file.exists():
        cache_file.unlink()
    proc = _run_doxa_test("-r", "--id", "M1T-01")
    assert proc.returncode == 0, proc.stdout + proc.stderr
...
Run: `uv run pytest tests/test_doxa_test_cli.py -k "cache" -v`
Expected: FAIL — `serialize_run_report` missing; cache file not written.

- [ ] **Step 3: Add serializer**

Add to `doxa_test` near line ~700 (next to the list printers):
...
Run: `./doxa_test -r --provider mock --skip-interactive`
Expected: same counts as before; `.doxa_test_cache/last_run.json` exists and parses as JSON.

- [ ] **Step 7: Commit**

```bash
git add doxa_test tests/test_doxa_test_cli.py
git commit -m "feat(doxa_test): write last-run cache on every run"
```
...
**Files:**
- Modify: `doxa_test` — add cache reader; wire flag into resolver; exit 2 on missing/empty.
- Test: `tests/test_doxa_test_cli.py`
...
def test_last_failed_exits_2_when_no_cache(tmp_path):
    ...
    if cache_file.exists():
        cache_file.unlink()
    proc = _run_doxa_test("-r", "--last-failed")
    assert proc.returncode == 2
...
def test_last_failed_reads_cache_and_reruns(tmp_path):
    ...
    proc2 = _run_doxa_test("-r", "--last-failed")
...
Run: `uv run pytest tests/test_doxa_test_cli.py -k "last_failed" -v`
Expected: FAIL — `read_last_failed` and `--last-failed` flag don't exist.

- [ ] **Step 3: Add the cache reader**

Add to `doxa_test` near `write_cache_atomic`:
...
```python
    if last_failed:
        try:
            ...
            click.echo(
                "No prior failures to rerun (.doxa_test_cache/last_run.json missing or empty)",
                err=True,
            )
            sys.exit(2)
        if not failed_ids:
...
```bash
git add doxa_test tests/test_doxa_test_cli.py
git commit -m "feat(doxa_test): add --last-failed to rerun prior failures"
```
...
**Files:**
- Modify: `doxa_test` — accept `--report-json` and write the same payload atomically to the chosen path.
- Test: `tests/test_doxa_test_cli.py`
...
def test_report_json_writes_to_given_path(tmp_path):
    ...
    proc = _run_doxa_test("-r", "--id", "M1T-01", "--report-json", str(target))
    assert proc.returncode == 0
...
Run: `uv run pytest tests/test_doxa_test_cli.py -k "report_json" -v`
...
```bash
git add doxa_test tests/test_doxa_test_cli.py
git commit -m "feat(doxa_test): add --report-json for on-demand JSON export"
```
...
**Files:**
- Modify: `doxa_test` — quiet branch in `run_all_tests` and `generate_report`; fenced printer.
- Test: `tests/test_doxa_test_cli.py` (also unskip the Task 4 integration test)
...
def test_quiet_emits_fenced_failure_for_failing_test(doxa_test_mod, capsys):
...
Run: `uv run pytest tests/test_doxa_test_cli.py -k "quiet" -v`
Expected: FAIL — `quiet` kwarg missing; `-q` not recognized.

- [ ] **Step 3: Add the fenced printer**

Add to `doxa_test` near the list printers:
...
```python
    def _generate_quiet_report(self):
        ...
        sys.stdout.write(
            f"doxa_test: {passed} passed, {failed} failed, {skipped} skipped "
...
Run: `./doxa_test -r --provider mock --skip-interactive -q`
...
```bash
git add doxa_test tests/test_doxa_test_cli.py
git commit -m "feat(doxa_test): add -q/--quiet mode with fenced failure blocks"
```
...
````markdown
When a pre-commit hook fails on `./doxa_test`, grep to skip the 64-row
noise table:

```bash
./doxa_test -r --provider mock --skip-interactive 2>&1 | grep -A 30 "Failed Test Details"
```
````
...
```bash
git add CLAUDE.md
git commit -m "docs(claude): use new doxa_test flags instead of grep workarounds"
```
...
```bash
uv run pytest tests/test_doxa_test_cli.py -v
```
...
```bash
./doxa_test --list | head -5
./doxa_test --list-json | python -m json.tool | head -20
./doxa_test -r --id M1T-01 -q
./doxa_test -r --last-failed -q   # expect exit 2 with helpful message
./doxa_test -r --id M1T-01 --report-json /tmp/report.json && cat /tmp/report.json | python -m json.tool | head
```
