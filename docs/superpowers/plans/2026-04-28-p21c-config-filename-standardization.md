...
**Goal:** Standardize Doxa Research's config filename to `doxa-research.config.toml`. One user-tier location (`$XDG_CONFIG_HOME/doxa-research/doxa-research.config.toml`) and two project-tier locations (`./doxa.config.toml` or `./.doxa-research.config.toml`, mutually exclusive — both present is a hard error). Legacy filenames (`config.toml` in user dir, `doxa-research.toml`, `.doxa-research/config.toml`) are no longer read. A "config not found" error path detects legacy files only to enrich the suggestion with rename guidance.

**Architecture:** `paths.py` returns the canonical user path. `ConfigManager` loads exactly the canonical filenames; `_load_project_config_with_path` raises `ConfigAmbiguousError` if both project-tier files exist. A new `detect_legacy_paths()` helper lives in `config.py` and is called *only* by the "config not found" error formatter — never on the happy load path. `doxa-research init` gains `--user` and `--hidden` flags (mutually exclusive); `--force` semantics extend to `--user`.
...
- Modify: `src/doxa_research/paths.py` — `user_config_file()` returns the canonical path.
- Modify: `src/doxa_research/config.py` — `project_config_paths` shrinks to the two canonical names; `_load_project_config_with_path` raises on ambiguity; user-tier load skips legacy; new `detect_legacy_paths()` helper.
- Modify: `src/doxa_research/errors.py` — add `ConfigNotFoundError`, `ConfigAmbiguousError`.
- Modify: `src/doxa_research/commands.py` — `init_command` and `get_init_data` accept `user`, `hidden`, `force` parameters; respect mutual exclusion; write to the chosen target.
- Modify: `src/doxa_research/cli_subcommands/init.py` — add `--user`, `--hidden`, `--force` Click options; enforce mutual exclusion at the Click layer; thread through to `init_command` / `get_init_data`.
- Modify: `src/doxa_research/help.py`, `src/doxa_research/cli_subcommands/config.py` — string sweep for canonical filename.
- Modify: `README.md`, `manual_testing_instructions.md` — canonical filename + short "Migrating from earlier Doxa Research versions" note.
...
|---|---|---|
| A1 | `~/.config/doxa-research/doxa-research.config.toml` exists | Loads it; `cm.user_config_path` points to it. |
| A2 | Only legacy `~/.config/doxa-research/config.toml` exists (canonical absent) | Treated as no user config (existing optional-user-config behavior). Legacy file is **not** loaded. `cm.user_config_path` is the canonical path (which doesn't exist). |
...
| A5 | Same as A4 but legacy `config.toml` is on disk | `ConfigNotFoundError` message additionally names the legacy path with rename target (`doxa-research.config.toml`). |

### B. Project-tier loading

| ID | Scenario | Expected |
|---|---|---|
| B1 | Only `./doxa.config.toml` exists | Loads it; `cm.project_config_path` points to it. |
| B2 | Only `./.doxa-research.config.toml` exists | Loads it; `cm.project_config_path` points to it. |
| B3 | Both `./doxa.config.toml` and `./.doxa-research.config.toml` exist | `ConfigAmbiguousError` raised. Message names both files and tells the user to delete one. No precedence. |
| B4 | Neither canonical project file exists | Returns no project config (existing optional-project-config behavior). |
| B5 | Only legacy `./doxa.toml` exists | Treated as no project config. Legacy file is **not** loaded. |
| B6 | Only legacy `./.doxa-research/config.toml` exists | Treated as no project config. Legacy file is **not** loaded. |
| B7 | `ConfigNotFoundError` path with legacy `./doxa.toml` on disk | Error suggestion names the legacy file and the rename target. |
| B8 | `ConfigNotFoundError` path with legacy `./.doxa-research/config.toml` on disk | Error suggestion names the legacy file and the rename target. |
...
### D. `doxa-research init` flag combinations

| ID | Invocation | Target | Notes |
|---|---|---|---|
| D1 | `doxa-research init` | `./doxa.config.toml` | Default project write. |
| D2 | `doxa-research init --hidden` | `./.doxa-research.config.toml` | Dotfile form. |
| D3 | `doxa-research init --user` | `$XDG_CONFIG_HOME/doxa-research/doxa-research.config.toml` | User-tier write. Creates parent dir if missing. |
| D4 | `doxa-research init --user --hidden` | ERROR | Mutual exclusion enforced at Click layer (UsageError). |
| D5 | `doxa-research init` when `./doxa.config.toml` already exists, no `--force` | Refuses to overwrite; error. |
| D6 | `doxa-research init --force` when `./doxa.config.toml` exists | Overwrites. |
| D7 | `doxa-research init --user` when user file exists, no `--force` | Refuses to overwrite; error. |
| D8 | `doxa-research init --user --force` when user file exists | Overwrites. *(Confirmed in spec §9 Q2.)* |
| D9 | `doxa-research init --hidden` when `./doxa.config.toml` already exists (no `--force`) | Writes `./.doxa-research.config.toml`. The next `doxa-research` invocation hits B3 ambiguity — this is documented but is **not** prevented at `init` time (B3 catches it on next load, with a clearer error than `init` could give pre-write). |
| D10 | `doxa-research init --hidden --force` when `./.doxa-research.config.toml` exists | Overwrites `./.doxa-research.config.toml`. |
| D11 | `doxa-research init --json --non-interactive` | Honors `--user` and `--hidden` selection in the JSON envelope (`config_path` field reflects target). |
| D12 | `doxa-research init --user` when both project config filenames exist | Writes `$XDG_CONFIG_HOME/doxa-research/doxa-research.config.toml`. `init --user` is a user-tier target write and does not pre-load project config. |
| D13 | `doxa-research init --json --non-interactive` when `./doxa.config.toml` exists, no `--force` | Refuses to overwrite and emits a JSON error envelope. |
| D14 | `doxa-research init --json --non-interactive --force` when `./doxa.config.toml` exists | Overwrites the visible project file and emits a success envelope with `created=false`. |
...
| E2 | `grep -r "doxa-research.toml" src/ tests/` excluding the legacy detector | Only canonical references remain. |
| E3 | `grep -r "\.doxa-research/config" src/ tests/` excluding the legacy detector | Only canonical references remain. |
...
from doxa-research.config import ConfigManager, detect_legacy_paths
from doxa-research.errors import ConfigAmbiguousError, ConfigNotFoundError
...
- [ ] **Step 4: Write C1–C3 legacy-detection helper tests.** C2 uses `monkeypatch.setattr` on `doxa-research.config.detect_legacy_paths` to assert zero calls during a successful load.
...
```python
# src/doxa_research/errors.py
class ConfigNotFoundError(DoxaError):
    """No canonical Doxa Research config file found at any expected location."""

class ConfigAmbiguousError(DoxaError):
    """Both canonical project-tier config files exist; user must pick one."""
```

Match the existing `DoxaError` constructor pattern (message + suggestion) used by `ConfigProfileError`.
...
```bash
git add src/doxa_research/paths.py src/doxa_research/errors.py
...
```python
# src/doxa_research/config.py — module level, not on ConfigManager.
LEGACY_USER_FILENAME = "config.toml"
LEGACY_PROJECT_PATHS: tuple[str, ...] = ("./doxa.toml", "./.doxa-research/config.toml")
...
def _format_config_not_found() -> ConfigNotFoundError:
    ...
    if legacy:
        ...
        lines.append(
            ...
            "doxa-research.config.toml (or .doxa-research.config.toml in the project root)."
        )
...
## Task 4: `doxa-research init` — `--user`, `--hidden`, `--force`
...
def _resolve_init_target(
    ...
    *,
    ...
    if hidden:
        return Path("./.doxa-research.config.toml").resolve()
...
```python
# src/doxa_research/cli_subcommands/init.py
...
@click.option("--hidden", "hidden", is_flag=True, help="Write to ./.doxa-research.config.toml instead of ./doxa.config.toml")
...
```bash
git add src/doxa_research/cli_subcommands/init.py src/doxa_research/commands.py
git commit -m "feat(p21c): doxa-research init --user, --hidden, --force flags"
```
...
- Modify: `src/doxa_research/cli_subcommands/config.py`
- Modify: any other `src/**/*.py` containing legacy filename literals (audit via `grep`).

- [ ] **Step 1: Inventory**

```bash
grep -rn "config.toml\|doxa-research.toml\|\.doxa-research/" src/doxa_research/ \
...
```bash
git add src/
git commit -m "refactor(p21c): sweep source strings to canonical doxa-research.config.toml"
```
...
```bash
grep -rn "config.toml\|doxa-research.toml\|\.doxa-research/config" \
...
Add a short "Migrating from earlier Doxa Research versions" callout in the config section:
...
The old filenames are no longer read. Rename them with `mv`. If both `./doxa.config.toml` and `./.doxa-research.config.toml` exist in the same project, Doxa Research will refuse to start until one is deleted.
```
...
- `./doxa.toml` → `./doxa.config.toml`
- `./.doxa-research/config.toml` → `./.doxa-research.config.toml`
- `~/.config/doxa-research/config.toml` → `~/.config/doxa-research/doxa-research.config.toml`

If the P21 spec's "the catalog records the actual project config path (`./doxa.toml` or `./.doxa-research/config.toml`)" line still exists, replace with the canonical pair `(./doxa.config.toml or ./.doxa-research.config.toml)`.

- [ ] **Step 4: manual_testing_instructions.md**

Update every smoke test that writes or names a config path. Pay special attention to any test that creates `./doxa.toml` as a fixture — those become `./doxa.config.toml`.

- [ ] **Step 5: Commit**

```bash
git add README.md manual_testing_instructions.md docs/ planning/ CLAUDE.md AGENTS.md
git commit -m "docs(p21c): sweep documentation to canonical doxa-research.config.toml"
```
...
- [ ] Manual smoke (one fresh shell):
  - `doxa-research init` in an empty dir → file at `./doxa.config.toml`.
  - `doxa-research init --hidden` in another empty dir → file at `./.doxa-research.config.toml`.
  - Touch both files in the same dir → next `doxa-research config get general.default_mode` (or any config-loading command) errors with `ConfigAmbiguousError`.
  - `doxa-research init --user` → file at `~/.config/doxa-research/doxa-research.config.toml`, even when the current directory contains both project config filenames.
  - `doxa-research init --user --hidden` → `UsageError: --user and --hidden are mutually exclusive`.
  - Place a legacy `./doxa.toml`, no canonical → command requiring config errors with the canonical paths listed AND the legacy file named in the suggestion.
