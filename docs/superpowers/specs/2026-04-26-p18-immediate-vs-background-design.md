...
**Predecessors:**
- **P11** (`doxa-research modes` Discovery, v2.11.0) — established `Kind = Literal["immediate", "background"]` at the rendering layer (`modes_cmd.py:22`). P18 promotes that vocabulary to a *declared* data field on every mode.
...
- **P16 PR1+PR2+PR3** (Click-native CLI, v3.0.0 — landed in commit `f8b62f2`) — established `cli_subcommands/` directory, `DoxaGroup` dispatcher, shared `_research_options` decorator stack (`cli_subcommands/_options.py`), `--json` envelope contract via `json_output.py`, `completion/sources.py` data sources, and `doxa-research ask`/`doxa-research resume` subcommands. P18 plugs into these structures; no PR2/PR3 changes needed.
...
- **P12** (CLI Mode Editing — `doxa-research modes set/add`) — once P18 ships, mode-editing commands must surface `kind` as a required field for new user modes. P12 should be drafted with awareness of the P18 contract.
...
5. **Naming overloads invite confusion.** `mini_research` is described as "Fast, lightweight" but uses `o4-mini-deep-research` (background-only). `thinking` is the truly synchronous one. The mismatch is invisible from `doxa-research modes list` until you correlate `Kind` against the description text by eye.
...
- A new `doxa-research ask` subcommand. That is P16 PR2's responsibility; P18's `_execute_immediate` is a behavior layer that PR2's `ask` will benefit from automatically.
...
| Q9 | Cancel: implement uniformly, or research first? | **Research first per provider.** Three explicit research tasks (T18/T19/T20 in PROJECTS.md). OpenAI almost certainly supports `responses.cancel`; Perplexity/Gemini TBD. Providers without upstream cancel raise `NotImplementedError`; the `doxa-research cancel` CLI catches and reports "upstream cancel not supported, local checkpoint marked cancelled". |
| Q10 | Extended test gating | **`pytest -m extended`** + `addopts = "-m 'not extended'"` in default config. Mirror in `./doxa_test --extended` and `just test-extended`. Nightly CI gate (new workflow, gated on repo secret). Never on PR CI. |
...
|---|---|---|
| `src/doxa_research/config.py` | Modified | Add `kind` to all 12 `BUILTIN_MODES` entries (`config.py:41-133`); add `mode_kind(cfg)`; thin `is_background_mode` wrapper kept (still called by 6 sites); warn-once on user modes missing `kind` in `_validate_config` (`config.py:367`) |
| `src/doxa_research/models.py` | Modified | Add `ModelSpec` NamedTuple + `derive_known_models()` + `KNOWN_MODELS` module constant. Cross-mode kind conflict raises at import. |
| `src/doxa_research/errors.py` | Modified | Add `ModeKindMismatchError(DoxaError)` carrying `mode_name`/`model`/`declared_kind`/`required_kind` |
| `src/doxa_research/providers/base.py` | Modified | Add `StreamEvent` dataclass; add `stream()` and `cancel()` raising `NotImplementedError` |
| `src/doxa_research/providers/__init__.py` | Modified | Thread `mode_config["kind"]` into `provider_config["kind"]` in `create_provider` (currently `providers/__init__.py:107-112`); replace `is_background_mode(provider_config)` call with `mode_kind(provider_config) == "background"` |
| `src/doxa_research/providers/openai.py` | Modified | Add `_validate_kind_for_model(mode)` (called first in `submit()`); implement `stream()` via `client.responses.stream`; implement `cancel()` via `client.responses.cancel`; remove the `if not background: return completed` shortcut in `check_status` (`providers/openai.py:232-233`, Phase J cleanup) |
| `src/doxa_research/providers/mock.py` | Modified | Implement `stream()` (deterministic chunks) and `cancel()` (pop from internal jobs dict, mark cancelled) |
| `src/doxa_research/providers/perplexity.py` | Modified | Optional: implement `cancel()` *only if* Phase F research confirms upstream support; otherwise leave base `NotImplementedError` |
| `src/doxa_research/progress.py` | Modified | Migrate `should_show_spinner` (`progress.py:16-36`) to call `mode_kind(mode_cfg) == "background"` once a `mode_cfg` is plumbed through; for now keep `is_background_model(model)` as the model-level helper. Extend the gate so the **Progress bar is also suppressed** for immediate runs (currently spinner is gated, Progress bar is not). |
| `src/doxa_research/run.py` | Modified | Rename `_execute_research` (`run.py:550`) → `_execute_background`; add top-level `execute()` dispatcher; add `_execute_immediate` (no `Progress`, no polling, streams to `MultiSink`); gate every `doxa-research resume {id}` / `doxa-research status {id}` hint (`run.py:629, 654, 691, 692`) on `mode_kind == "background"`; conditionally suppress `Operation ID` echo (`run.py:199, 311, 691`) for immediate runs unless `--project` or `--out FILE` set |
| `src/doxa_research/cli.py` | Modified | Replace `_doxa_config.is_background_model(model_name)` (`cli.py:284`) call site with `mode_kind` once a mode_cfg is in scope |
| `src/doxa_research/cli_subcommands/_options.py` | Modified | Add `--out PATH` (repeatable, accepts `-`, comma-list accepted) and `--append` flags to `_RESEARCH_OPTIONS` so they are inherited by both top-level CLI and `doxa-research ask`. Update help string for `--pick-model` from "(immediate modes only)" to reference declared `kind`. |
| `src/doxa_research/cli_subcommands/_option_policy.py` | Modified | Add `out` and `append` entries to the option-policy mapping so inheritance + validation work consistently |
| `src/doxa_research/cli_subcommands/ask.py` | Modified | Wire `--out`/`--append` arg through to `_run_research_default` and the `--json` envelope path. Replace `is_background_mode(mode_config)` (`ask.py:176`) with `mode_kind(mode_config) == "background"`. |
| `src/doxa_research/cli_subcommands/cancel.py` | NEW | `@click.command("cancel")` + `OP_ID` positional + `--json`, delegates to `cancel_operation()`; mirrors `cli_subcommands/resume.py` pattern |
| `src/doxa_research/cli_subcommands/modes.py` | Modified | Add `--kind <immediate\|background>` filter; the completer is **already wired** as dead-code in `completion/sources.py:79-81 mode_kind` (left as P18 forward-compat by PR3) — Phase D plugs it in |
| `src/doxa_research/interactive_picker.py` | Modified | Replace `is_background_model(model)` filter (`interactive_picker.py:35,44`) with `mode_kind(mode_cfg) == "immediate"` once a mode_cfg is in scope; the user-facing UX (filter immediate-only models for `--pick-model`) is unchanged |
| `src/doxa_research/sinks.py` | NEW | `MultiSink` class — fans `write(chunk)` to a list of `IO[str]` handles, lazy file open, ordered close in `finally` |
| `src/doxa_research/commands.py` | Modified | Add `cancel_operation(op_id, ctx)`; gate `doxa-research resume` hints on background kind |
| `src/doxa_research/modes_cmd.py` | Modified | Replace substring fallback in `_derive_kind` (`modes_cmd.py:46-50`) with: read `cfg["kind"]` first; fall back to legacy heuristic only with a warning. Wire the `--kind` filter into `_op_list`. |
| `src/doxa_research/signals.py` | Modified | Ctrl-C path calls `cancel_operation` best-effort with 5s timeout before exit |
...
| `doxa_test` | Modified | Add `--extended` flag and category column |
...
| `README.md` | Modified | Document `--out` flag; document `kind` field for user-defined modes; document `doxa-research cancel`; document the immediate-vs-background contract briefly |
| `manual_testing_instructions.md` | Modified | Streaming, cancel, mismatch, alias scenarios |
| `.release-please-manifest.json` / `pyproject.toml` / `src/doxa_research/__init__.py` | NOT modified | Release-please manages all version bumps. Currently `2.5.0` (manifest); next release tag will be `v3.0.0` from the queued `feat!` bump in #24. Land P18 after v3.0.0 tag for v3.1.0 release. |
...
- **CLI surface**: `--out PATH` (repeatable, `-` for stdout, comma-list); `--append`; `doxa-research cancel <op-id>` subcommand.
- **Mode aliases**: `mini_research → quick_research` (alias kept).
- **Test infrastructure**: `extended` pytest marker; `tests/extended/` directory; `just test-extended`; `./doxa_test --extended`; nightly CI workflow.
...
```
doxa ask "what is X" --mode thinking --out -
  → cli dispatch
...
| G | Cancel impls + `doxa-research cancel <op-id>` | No |
...
| `tests/test_cancel_subcommand.py` | G | `doxa-research cancel <op-id>` updates checkpoint, calls `provider.cancel`, exits 0; missing op exits 6 |
...
Gated: `addopts = "-m 'not extended'"` in default config; run via `pytest -m extended` / `just test-extended` / `./doxa_test --extended`. Nightly CI workflow.

### 7.3 Integration (`doxa_test`)

The existing `doxa_test` integration runner is already exhaustive for background-mode flows. P18 adds:
...
### 8.1 P16 — Click-native CLI + `doxa-research ask` (v3.0.0 — SHIPPED to `main` 2026-04-27)

PR1 + PR2 + PR3 all merged in commit `f8b62f2`. The structures P18 plugs into now exist:

- **`doxa-research ask PROMPT`** (`cli_subcommands/ask.py`) — canonical scripted research entrypoint, inheriting the 21-flag research-options stack from `cli_subcommands/_options.py:_research_options`.
- **`doxa-research resume OP_ID`** (`cli_subcommands/resume.py`) — replaces the removed `--resume` flag.
- **`--json` everywhere** via `json_output.py:emit_json` / `emit_error`. `doxa-research ask --json` already implements **Option E**: "background mode → submit envelope; immediate → run synchronously and emit snapshot from latest checkpoint" (`ask.py:158-231`). This is the kind-aware split applied to the JSON path; P18 brings the same split to the human-readable path and adds streaming.
...
### 8.2 P12 — `doxa-research modes set/add/unset`
...
P12 can absorb these requirements into its existing scope; no separate cross-project task needed. The `cli_subcommands/modes.py` file already exists (`doxa-research modes list/json` shipped in P11) — P12 grows it.
...
- **Cancel race conditions** — `doxa-research cancel` issued just as a background job transitions to `completed` upstream may produce inconsistent local state. Mitigation: re-poll status immediately after `cancel()` returns; treat post-cancel `completed` as success (job finished before the cancel landed).
...
- `--out` supports stdout, file, tee, and append for immediate runs (added to `_RESEARCH_OPTIONS` so both top-level CLI and `doxa-research ask` inherit it)
- `doxa-research modes --kind <immediate|background>` filters output (wires the dead-code `mode_kind` completer in `completion/sources.py:79`)
- `provider.cancel()` exists on the base; OpenAI + Mock implement it; Perplexity/Gemini status reflects research findings
- `doxa-research cancel <op-id>` cancels in-flight background ops upstream and updates the checkpoint
...
**New sub-task added by reevaluation**: **`doxa-research modes --kind <immediate|background>` filter** (Phase D in PROJECTS.md). The completer is already committed dead-code in `completion/sources.py:79-81`; this finishes the round-trip the PR3 author intentionally left open.
...
- Task: `--kind` filter on `doxa-research modes` (Phase D)
