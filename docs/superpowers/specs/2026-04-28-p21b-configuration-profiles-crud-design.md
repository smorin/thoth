...
Add `doxa-research config profiles ...` CLI commands so users can manage configuration profiles without hand-editing TOML. P21b is purely additive: profile *behavior* (selection, overlay, error semantics) is fully delivered by P21 and stays unchanged. P21b adds the management UI on top.

The new subgroup:

```text
doxa config profiles list [--json] [--show-shadowed]
doxa config profiles show NAME [--json] [--show-secrets]
doxa config profiles current [--json]
doxa config profiles set-default NAME [--project] [--json]
doxa config profiles unset-default [--project] [--json]
doxa config profiles add NAME [--project] [--json]
doxa config profiles set NAME KEY VALUE [--project] [--string] [--json]
doxa config profiles unset NAME KEY [--project] [--json]
doxa config profiles remove NAME [--project] [--json]
```

Default writes target the user config. `--project` targets `./doxa.config.toml`, matching `doxa-research config set --project`. `--config PATH` and `--project` remain mutually exclusive.

## 2. Motivation

P21 ships profile resolution and overlay. Users can run `doxa-research --profile fast "topic"` and persist `general.default_profile = "fast"` — but only by hand-editing TOML. This is fine for v1 but cumbersome for day-to-day workflows: adding a new profile means opening an editor; persisting a selection means knowing the exact key path.

P21b removes that friction. The data functions and Click leaves use the same `tomlkit` patterns as the existing `doxa-research config set/unset/list` commands, so the surface is familiar and the implementation reuses existing helpers (`_target_path`, `_reject_config_project_conflict`, `_load_toml_doc`, `_parse_value`, `_mask_in_tree`, `_to_plain`). Profile `unset` and `unset-default` must use a no-prune leaf-removal helper rather than `_unset_in_doc`, because empty parent tables and their comments are part of the P21b contract.

## 3. Decisions

### Q1. Where do the new commands live in the command tree?

Under `doxa-research config profiles`. Not at the top level. Reasons:

- `doxa-research config` is already the established home for config-management subcommands (`get`, `set`, `unset`, `list`).
- A top-level `doxa-research profiles` group would collide with the future research-options surface and create a second discovery surface.
- `doxa-research config profiles` is consistent with `doxa-research modes` (which lives at top level because modes are a research concept, not a config concept).
...
**Raw lookup and mutator leaves** reject the inherited root `--profile`. `show NAME` is a raw profile-definition lookup; the positional `NAME` is the only profile being inspected, so accepting a second profile selector such as `doxa-research --profile fast config profiles show potato` is ambiguous. Mutators (`add`, `set`, `unset`, `remove`, `set-default`, `unset-default`) operate on the positional `NAME` argument only; the runtime active profile is irrelevant to a mutation target. These leaves omit `"profile"` from `honored_options`. Click rejects inherited `--profile` with the standard "no such option" error.

### Q3. How does `set-default NAME` validate the target?

Before persisting `general.default_profile = NAME`, build a transient `ConfigManager` for the same target view that the command will write. When inherited `--config PATH` is present, pass that path to the constructor (`ConfigManager(PATH)`) so profiles defined only in the custom config participate in `cm.profile_catalog`; do not pass `config_path` through `load_all_layers(...)`. Then call `load_all_layers({})` and check `NAME` against `cm.profile_catalog` (the union of the target user/custom tier plus the current project tier, computed by P21). If `NAME` is absent from the catalog, raise `ConfigProfileError(f"Profile {NAME!r} not found", available_profiles=..., source="doxa-research config profiles set-default")`.

Cross-tier resolution is allowed: `doxa-research config profiles set-default prod` succeeds when `prod` is defined only in the project tier even though the pointer is being written to the user config. `doxa-research --config /tmp/doxa-research.config.toml config profiles set-default fast` succeeds when `fast` exists only in `/tmp/doxa-research.config.toml`. The next load resolves the pointer via the same catalog entry.
...
`doxa-research config profiles current` reports the runtime active selection plus its source (`flag`, `env`, `config`, or `none`). Honors `--profile` and `DOXA_PROFILE` so:

```bash
$ doxa --profile bar config profiles current
Active profile: bar (from --profile flag)

$ DOXA_PROFILE=fast doxa-research config profiles current --json
{"status": "ok", "data": {"active_profile": "fast", "selection_source": "env", ...}}
```

This is the runtime view. It can disagree with `doxa-research config get general.default_profile` (which is the persisted view). See P21 §4.Q5 for the read-only-runtime-input invariant.

### Q6. What does `list --show-shadowed` report?

P21 profile resolution already defines the shadowing rule: when user and project tiers both define the same profile name, the project profile shadows the user profile wholesale. `doxa-research config profiles list` uses that same rule.
...
```json
{
  "name": "prod",
  "tier": "project",
  "path": "/repo/doxa-research.config.toml",
  "active": true,
  "shadowed": false,
  "shadowed_by": null
}
...
```json
{
  "name": "prod",
  "tier": "user",
  "path": "/home/user/.config/doxa-research/doxa-research.config.toml",
  "active": false,
  "shadowed": true,
  "shadowed_by": {
    "tier": "project",
...
### 4.1 `src/doxa_research/config_cmd.py`
...
### 4.2 `src/doxa_research/cli_subcommands/config.py`
...
- `show`, `add`, `set`, `unset`, `remove`, `set-default`, `unset-default`: `validate_inherited_options(ctx, ..., honored_options={"config_path"})` — drop `profile` so Click rejects forms like `doxa-research --profile foo config profiles show bar` and `doxa-research --profile foo config profiles add bar`.
...
- `--project` writes `./doxa.config.toml`; `--config PATH` writes the custom file; `--project` + `--config` errors with `PROJECT_CONFIG_CONFLICT`.
...
- `manual_testing_instructions.md`: `doxa-research config profiles ...` smoke checks, including expected error cases (`set-default ghost`, `--profile foo add bar`).
- `src/doxa_research/help.py`: `doxa-research help config` includes the new `config profiles` subgroup.
...
- All nine Click leaves under `doxa-research config profiles` exist and are wired into the `config` group.
