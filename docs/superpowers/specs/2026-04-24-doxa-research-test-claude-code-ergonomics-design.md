# Design: doxa_test Claude-Code Ergonomics Bundle

**Date:** 2026-04-24
**Owner:** Steve Morin
**Status:** Draft — awaiting user review
**Scope:** `doxa_test` only (the uv-shebang integration runner at repo root). No changes to `./doxa` executable or pytest suite under `tests/`.

## Motivation

`doxa_test` is driven both by humans and by Claude Code (and CI hooks that invoke it on Claude Code's behalf). The human ergonomics are good; the automation ergonomics force shell-scraping workarounds that are already documented in `CLAUDE.md`:
...
- No change to the `./doxa` CLI, to pytest tests, or to lefthook pre-commit configuration.
...
|------|-------|---------|---------|
| `--report-json PATH` | option | (off) | Write an additional JSON report to `PATH`. The cache file at `.doxa_test_cache/last_run.json` is always written regardless. |
...
| `--last-failed` | flag | off | Rerun only tests that failed in the most recent run (reads `.doxa_test_cache/last_run.json`). Implies `-r`. |
...
- `--last-failed` with no cache file or empty prior failures: exit `2` with stderr message `No prior failures to rerun (.doxa_test_cache/last_run.json missing or empty)`. This distinguishes it from exit `1` (real failures) and exit `0` (success).
...
## Data Contract: `.doxa_test_cache/last_run.json`
...
```json
{
  ...
  "doxa_test_version": "git:sha",
  "invocation": ["./doxa_test", "-r", "--provider", "mock"],
...
- The file is written **atomically** (write to `.doxa_test_cache/last_run.json.tmp`, then `os.replace`). Partial writes from an interrupted run must not corrupt the next `--last-failed`.

### Cache directory

- `.doxa_test_cache/` at repo root.
...
- **At end:** one summary line:
  ...
  ```
  doxa_test: 60 passed, 2 failed, 2 skipped in 57.3s (cache: .doxa_test_cache/last_run.json)
  ```
...
### Code organization within `doxa_test`
...
Per `CLAUDE.md`, tests come first. Because `doxa_test` is both the runner and (historically) not self-tested in pytest, we add pytest coverage under `tests/test_doxa_test_cli.py` that imports helpers from the shebang script via the existing `sys.path.insert(src)` pattern, or — if easier — factors the pure-Python helpers into a small `src/doxa_test_support.py` module that the shebang imports. **Preferred: keep everything in the shebang, add pytest tests that subprocess-invoke `./doxa_test`** to match the existing black-box convention.

### Test matrix

1. **`--list` output shape**
   - `./doxa_test --list` exits 0, emits ≥1 TSV line, each line has exactly 3 tabs.
   - `./doxa_test --list-json` exits 0, emits valid JSON array, each element has the required keys.
   - Both are silent (no spinner, no table), regardless of whether `-r` is passed.

2. **`--id` exact match**
   - `./doxa_test -r --id M1T-01` runs exactly one test; JSON cache has one entry.
   - `./doxa_test -r --id M1T-01 --id M1T-04` runs exactly two tests.
   - `./doxa_test -r --id NONEXISTENT` warns on stderr, exits 0 with no tests run.
   - `./doxa_test -r --id M1T-01 --provider openai` runs M1T-01 (no openai key needed since test is provider-agnostic) with `requested_but_filtered: false` (it actually matches the filter trivially). Use an id+provider combination that genuinely mismatches to exercise `requested_but_filtered: true`.
...
6. **Cache always written**
   - Even without `--report-json`, `.doxa_test_cache/last_run.json` is written after every run.
   - Atomic write: no partial files on interrupt.

7. **Backwards compatibility**
   - Every existing invocation in `CLAUDE.md` (e.g., `./doxa_test -r --provider mock --skip-interactive`) produces the **same exit code** and **same human-visible table** as before (byte-for-byte comparison not required; structural: "contains final table, contains `Passed: N`").

### Pre-existing test suite

- `./doxa_test -r` must still pass end-to-end after the change. Lefthook already enforces this. No new test cases added to `all_tests` unless an existing behaviour needs coverage we lacked.
...
| `.doxa_test_cache/` accidentally committed | Add to `.gitignore` in the same PR as the feature. |
...
1. `feat(doxa_test): add --list and --list-json for test discovery`
2. `feat(doxa_test): add --id for exact-match test selection`
3. `feat(doxa_test): write last-run cache and add --last-failed`
4. `feat(doxa_test): add --report-json for on-demand JSON export`
5. `feat(doxa_test): add -q/--quiet mode with fenced failure blocks`
6. `test(doxa_test): pytest coverage for new CLI flags`
7. `docs(claude): update CLAUDE.md to use new flags instead of grep workarounds`

Each commit keeps the suite green (`./doxa_test -r` passes; pytest passes).
...
| 2 | Default persistence | B — always write to `.doxa_test_cache/last_run.json` |
