# Doxa Research CLI Ergonomics — Design

**Date:** 2026-04-24
**Owner:** steve.morin@gmail.com
**Status:** Approved (pending implementation plan)
**Scope project name (PROJECTS.md):** `doxa-research-ergonomics-v1`
...
| **3.4** | **`doxa-research workflow` / `doxa-research guide` command** | **Dropped — superseded by `doxa-research modes` (P11)** | Decision: `planning/doxa-research.plan.v9.md:18`. Rationale: `doxa-research modes` already lists every mode with provider/model/`kind=immediate\|background`/source/description in one table, making a separate workflow-ladder command redundant. |
| 3.5 | API-key documentation pass + `doxa-research help auth` | ✅ Shipped | P14-T05 (`v2.13.0`) |
...
Reduce the friction a first-time doxa-research user hits between installation and a
...
The intended outcome is a CLI where a new user can run `doxa-research --help`, see
...
- Finishing the `doxa-research init` interactive wizard (separate project, later)
...
| (no equivalent) | `doxa-research providers models` |
| (no equivalent) | `doxa-research providers check` |

`doxa-research providers -- --list` continues to work for one release and prints a
deprecation notice: `providers -- --list is deprecated; use 'doxa-research providers list'`.

**Files:** `src/doxa_research/cli.py`, `src/doxa_research/commands.py`, `src/doxa_research/help.py`.

### 3.2 Progress indicator via thothspinner

**Change:** add `thothspinner` as a dependency. Wrap the sync-poll loop in
`src/doxa_research/run.py` so any deep-research mode run without `--async` or `-v`
...
**Ctrl-C behavior:** the existing SIGINT handler in `src/doxa_research/signals.py`
already persists a checkpoint. The spinner wrapper prints a one-line hint:

```
Backgrounded. Resume later: doxa-research --resume op_abc123
```

**User-visible:**

```
$ doxa deep_research "k8s networking"
...
**Files:** `pyproject.toml`, `src/doxa_research/run.py`, `src/doxa_research/signals.py`
...
**Change:** restructure the `--help` epilog (`src/doxa_research/help.py:build_epilog`)
...
Examples:
  # Quick
  $ doxa "how does DNS work"

  # Sharpen, then research
  $ doxa clarification "k8s networking" --project k8s
  $ doxa exploration --auto --project k8s
  $ doxa deep_research --auto --project k8s --async

  # Resume a backgrounded job
  $ doxa --resume op_abc123
```
...
**Files:** `src/doxa_research/help.py`.

### 3.4 `doxa-research workflow` command

> **Status:** Dropped per `planning/doxa-research.plan.v9.md:18` — superseded by `doxa-research modes` (P11).

**Change:** add a new `doxa-research workflow` (alias `doxa-research guide`) top-level
...
Recommended research workflow:
  ...
  1. clarification   Sharpen the question before researching
                     $ doxa clarification "..." --project NAME
  2. exploration     Survey options, trade-offs
                     $ doxa exploration --auto --project NAME
  3. deep_dive       Focus on one technology
                     $ doxa deep_dive --auto --project NAME
  4. tutorial        Concrete examples and code
                     $ doxa tutorial --auto --project NAME
  5. solution        Design a specific solution
                     $ doxa solution --auto --project NAME
  6. prd             Product requirements document
                     $ doxa prd --auto --project NAME
  7. tdd             Technical design document
                     $ doxa tdd --auto --project NAME

For long-running steps use --async and later: doxa-research --resume OP_ID
```

**Files:** `src/doxa_research/cli.py`, `src/doxa_research/help.py`, `src/doxa_research/commands.py`.
...
`doxa-research help auth` renderer. Soften `--api-key-openai` / `--api-key-perplexity`
...
**Files:** `README.md`, `src/doxa_research/help.py`, `src/doxa_research/cli.py` (help-string
text only).

### 3.6 `--input-file` vs `--auto` clearer help

**Change:** rewrite the help strings for both flags at
`src/doxa_research/cli.py:91-92` and add a section in `doxa-research help` plus the README
...
--input-file PATH    Use the file at PATH as input for this mode. Use
                     when you want to feed a non-doxa-research document, an older
                     run, or a file from a different project.
```

**Files:** `src/doxa_research/cli.py`, `README.md`, `src/doxa_research/help.py`.
...
$ doxa deep_research "topic" -v
[doxa-research] Operation ID: op_abc123
[doxa-research] Provider: openai · Model: o3-deep-research · Timeout: 1800s
[doxa-research] POST /v1/responses (attempt 1/3) → 200 in 482ms
[doxa-research] Polling op_abc123 every 30s…
```
...
**Change:** update `src/doxa_research/errors.py` to append a "Resolved from:"
...
Error: OPENAI_API_KEY not set
  Suggestion: export OPENAI_API_KEY=sk-... (or edit ~/.doxa-research/config.toml)
  Config file: /Users/steve/.doxa-research/config.toml  (does not exist)
  Env checked: OPENAI_API_KEY (unset)
```

**Files:** `src/doxa_research/errors.py`, `src/doxa_research/paths.py` (may expose a helper).
...
in `src/doxa_research/config.py`. Rule: model string contains `-deep-research`.
The helper is also used by 3.2 (spinner gate) and 3.3 (help grouping) so
the rule lives in one place.

**Files:** `src/doxa_research/config.py` (helper), `src/doxa_research/cli.py` (flag),
`src/doxa_research/run.py` (apply override), `src/doxa_research/commands.py` (model list
for picker), new module or extension of `src/doxa_research/interactive.py` for the
picker UI.
...
1. **`is_deep_research_model(model)` in `src/doxa_research/config.py`** — single
...
2. **Help rendering helpers in `src/doxa_research/help.py`** — shared between
   `--help` epilog (3.3), `doxa-research workflow` (3.4), and `doxa-research help auth`
   (3.5). Each renderer is a small pure function that returns a string,
   so they are snapshot-testable.
3. **Error-context helper in `src/doxa_research/errors.py`** — a
...
- `doxa-research providers list/models/check` → exit 0 + expected output shape
- `doxa-research providers -- --list` → exit 0 + deprecation notice on stderr
- `doxa-research workflow` → lists all 7 ladder steps in order
- `doxa-research --pick-model default "..."` with mocked picker → runs, uses picked model
- `doxa-research --pick-model deep_research "..."` → exit non-zero, error text matches
...
`./doxa_test -r` full integration suite remains green. New doxa_test
cases added for:
...
4. PROJECTS.md tracks tasks under `doxa-research-ergonomics-v1`.
...
- Current CLI: `src/doxa_research/cli.py`
- Current mode table: `src/doxa_research/config.py:38` (`BUILTIN_MODES`)
- Current help: `src/doxa_research/help.py`
- Current run loop: `src/doxa_research/run.py`
- Current errors: `src/doxa_research/errors.py`
