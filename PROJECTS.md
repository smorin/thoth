...
- Read-only leaves (`list`, `show`, `current`) honor inherited `--profile`; mutator leaves (`add`/`set`/`unset`/`remove`/`set-default`/`unset-default`) reject it.
...
- [x] [P21b-T02] Add `config profiles` Click subgroup to `src/thoth/cli_subcommands/config.py` with leaves for `list`, `show`, `current`, `set-default`, `unset-default`, `add`, `set`, `unset`, `remove`. Mutator leaves omit `"profile"` from `honored_options`; read-only leaves include it. Add typed `--show-shadowed` on `list`. JSON output via existing envelope helpers. (Commit `6126486`.)
