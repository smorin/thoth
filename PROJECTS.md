...
- [x] P21b — Configuration Profile CRUD Commands (depends on P21)
...
## [x] Project P21b: Configuration Profile CRUD Commands
...
## [ ] Project P12: CLI Mode Editing — `thoth modes` mutations (v2.12.0)
...
- **JSON envelope**: every mutator accepts `--json` and emits `{schema_version: "1", op, mode, target: {file, tier}, …op-specific keys}`. Default human output is a one-line confirmation (e.g. `Set my_brief.temperature = 0.2 → ~/.thoth/thoth.config.toml [modes.my_brief]`). Users who want the full effective view run `thoth modes --name NAME` afterward.
...
- **Effective view after writes**: not auto-printed. The write receipt is intentionally minimal; users run `thoth modes --name NAME [--profile X]` for the full ModeInfo. Diverges from the original P12 design (which echoed `_render_detail` after every write) — chosen for parity with `config profiles` and to avoid double-rendering.
...
`rename` and `copy` additionally refuse if the destination name is a builtin or already exists in the destination tier (`DST_NAME_TAKEN`).
...
- [ ] [P12-TS02d] Setting on a builtin-only name implicitly creates override in the chosen tier (post: `thoth modes --name NAME [--profile X]` → `source=overridden`)
- [ ] [P12-TS02e] Absent NAME in the chosen tier → `MODE_NOT_FOUND` exit 1
- [ ] [P12-TS02f] Targeting matrix (same shape as TS01h)
- [ ] [P12-TS02g] `--json` envelope: `{schema_version, op: "set", mode, target, key, value, wrote}`
...
- [ ] [P12-TS03c] On overridden mode: pruning fully reverts the override; post `thoth modes --name NAME` → `source=builtin`
...
- [ ] [P12-TS03g] `--json` envelope: `{schema_version, op: "unset", mode, target, key, removed, table_pruned}`
...
- [ ] [P12-TS04a] Drops a user-only mode in the chosen tier; gone from `thoth modes [--profile X]`
...
- [ ] [P12-TS04f] `--json` envelope: `{schema_version, op: "remove", mode, target, removed, reverted_to_builtin}`
...
{ "schema_version": "1", "op": "set", "mode": "deep_research",
...
- `--json` envelopes share `schema_version: "1"` and a uniform `target` shape
- Builtin-name guard blocks `add` / `remove` / `rename` of builtins regardless of `--profile`
- `--project + --config PATH` rejected with `USAGE_ERROR` exit 2 on every mutator

### Manual Verification
- After `thoth modes add my_brief --model gpt-4o-mini`, `thoth modes` shows it with `source=user`
- After `thoth modes set deep_research parallel false`, `thoth modes --name deep_research` shows `source=overridden` with `parallel: true → false`
- `thoth modes remove deep_research` reverts the override; `--name` shows `source=builtin`
...
## [x] Project P01: Developer Tooling & Automation (v2.6.0)
