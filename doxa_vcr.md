# VCR Cassette Integration for Doxa Research
...
Default output: `../doxa/doxa_test_cassettes/<provider>/<scenario>.yaml`.
...
Two approaches, depending on whether doxa-research stays with the custom `doxa_test`
runner or adds pytest alongside it.

### Option A: Fixture tests inside `doxa_test`

Add `vcrpy` to the inline script dependencies at the top of `doxa_test`:
...
CASSETTE_DIR = Path(__file__).parent / "doxa_test_cassettes"


def _vcr_openai_submit() -> None:
    """Fixture test: submit via cassette, get a job ID back."""
    from doxa-research.__main__ import OpenAIProvider
...
**Pros:** Consistent with how doxa-research tests work today. No new test runner.
**Cons:** `doxa_test` is already ~3600 lines. vcrpy's httpx interception
...
add per-provider test files as cassettes land. Doesn't bloat `doxa_test`.
**Cons:** Introduces pytest as a second test runner alongside `doxa_test`.
